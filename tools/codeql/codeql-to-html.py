...
Usage: codeql-to-html.py [--no-exclude] <input.sarif> <output.html>
"""
import json
import sys
import os
...
def main():
    ...
    if len(args) != 2:
        print(f"Usage: {sys.argv[0]} [--no-exclude] <input.sarif> <output.html>", file=sys.stderr)
        sys.exit(1)
            ...
            for file_path in sorted(sev_files.keys()):
                ...
                for i, r in enumerate(results):
                    ...
                    if has_detail:
                        ...
                        w(f'<tr class="{row}">')
                    w(f'<td class="line priority-cell {sev_cls}">{escape(rng)}</td>')
