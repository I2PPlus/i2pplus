...
# Check if google-java-format.jar exists in tools/
if [ ! -f "${PROJECT_ROOT}/tools/google-java-format.jar" ]; then
  echo " ! Error: google-java-format.jar not found in tools/."
...
find "$directory" -type f \( ! -name "*.class" \) \( ! -name "Strings.java" \) -name "*.java" -print0 | xargs -0 "$JAVA_EXECUTABLE" -jar "${PROJECT_ROOT}/tools/google-java-format.jar" --replace --fix-imports-only --skip-sorting-imports | grep -c "^import"
