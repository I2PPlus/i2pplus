...
SPOTBUGS_DIR="${TOOLS_DIR}/spotbugs/spotbugs-${SPOTBUGS_VERSION}"
SPOTBUGS_JAR="${SPOTBUGS_DIR}/lib/spotbugs.jar"
SPOTBUGS_HOME="${SPOTBUGS_DIR}"
SPOTBUGS_SCRIPT="${SPOTBUGS_DIR}/bin/spotbugs"
SPOTBUGS_TGZ="${TOOLS_DIR}/spotbugs/spotbugs-${SPOTBUGS_VERSION}.tgz"
...
if [ ! -f "$SPOTBUGS_SCRIPT" ]; then
    ...
    tar -xzf "$SPOTBUGS_TGZ" -C "${TOOLS_DIR}/spotbugs/"
...
if [ -f "config/spotbugs.xsl" ] && [ -f "$OUTPUT" ]; then
    echo "Transforming SpotBugs XML to HTML using custom stylesheet..."
    xsltproc -o "dist/spotbugs.html" "config/spotbugs.xsl" "$OUTPUT"
    echo "SpotBugs HTML report written to dist/spotbugs.html"
elif [ -f "config/spotbugs.xsl" ]; then
