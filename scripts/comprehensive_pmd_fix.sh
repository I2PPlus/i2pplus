...
mkdir -p "$PROJECT_ROOT/pmd_fix_results"
results_file="$PROJECT_ROOT/pmd_fix_results/fix_summary.txt"
echo "PMD Fix Summary - $(date)" > "$results_file"
echo "=================================" >> "$results_file"

# Find all Java files
echo "Finding all Java files..."
find "$PROJECT_ROOT" -name "*.java" -type f ! -path "*/.git/*" ! -path "*/build/*" ! -path "*/.gradle/*" > java_files.txt
...
"$SCRIPT_DIR/ensure-pmd.sh"
PMD_BIN="${PROJECT_ROOT}/tools/pmd-bin-7.7.0/bin/pmd"
if [ -f "$PMD_BIN" ]; then
    "$PMD_BIN" check -d "$PROJECT_ROOT" -R "${PROJECT_ROOT}/tools/pmd-bin-7.7.0/pmd-ruleset.xml" -f text -r "${results_file%/*}/pmd_report_after.txt" 2>/dev/null || \
    "$PMD_BIN" check -d "$PROJECT_ROOT" -R "${PROJECT_ROOT}/rulesets/java/quickstart.xml" -f text -r "${results_file%/*}/pmd_report_after.txt" 2>/dev/null || \
    echo "PMD analysis failed - check PMD installation"

    if [ -f "${results_file%/*}/pmd_report_after.txt" ]; then
        violations_after=$(grep -c "Violation" "${results_file%/*}/pmd_report_after.txt" 2>/dev/null || echo "0")
        echo "Violations after fix: $violations_after" | tee -a "$results_file"
    fi
else
    echo "PMD binary not found at ${PMD_BIN}" | tee -a "$results_file"
fi
