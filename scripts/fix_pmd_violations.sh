...
# Find all Java files
find "$PROJECT_ROOT" -name "*.java" -type f > java_files.txt
...
"${PROJECT_ROOT}/tools/pmd-bin-7.7.0/bin/pmd" check -d "$PROJECT_ROOT" -R "${PROJECT_ROOT}/rulesets/java/quickstart.xml" -f text -r "${PROJECT_ROOT}/pmd-report-after.txt" | head -50
