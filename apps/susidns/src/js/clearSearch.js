...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function handleClear() {
    ...
    let noFilterURL = `addressbook?book=${activeBook}&filter=none`;
    const isIframed = document.documentElement.classList.contains("iframed");
    if (isIframed) {noFilterURL = `/dns?book=${activeBook}&filter=none`;}
  ...
  clearSearch.addEventListener("click", function (event) {
    searchInput.value = "";
    if (window.top !== window.self) {window.top.location.href = this.href;}
