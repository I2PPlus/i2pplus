  ...
  elements.forEach(function(element) {
    ...
    if (style) {
      ...
      if (style.toLowerCase().indexOf("!important") !== -1) {
        var newStyle = style.replace(/!important/gi, "");
    ...
    if (element.tagName.toLowerCase() === "img") {
      if (!element.getAttribute("src").startsWith("data:")) {
        ...
        if (iframe && !iframe.classList.contains("showBlockedImages")) {
          var remoteImages = iframe.contentWindow.document.querySelectorAll("img");
          ...
          const blockedImages = document.getElementById("blockedImages") || window.parent.document.getElementById("blockedImages");
          const blockedImgCount = document.getElementById("blockedImgCount") || window.parent.document.getElementById("blockedImgCount");
