...
class Sorters {
    ...
    public static class ToSorter extends SorterBase {
        ...
        protected int compare(Mail a, Mail b) {
            ...
            if (a.to.length > minsz) {return 1;}
            if (b.to.length > minsz) {return -1;}
            return 0;
        }
    }
    ...
    public static class SubjectSorter extends SorterBase {
        ...
        protected int compare(Mail a, Mail b) {
            ...
            if (aslc.startsWith("re:") || aslc.startsWith("fw:")) {as = as.substring(3).trim();}
            else if (aslc.startsWith("fwd:")) {as = as.substring(4).trim();}
            else {
                if (aslc.startsWith(xre)) {as = as.substring(xre.length()).trim();}
                else {
                    if (aslc.startsWith(xfwd)) {as = as.substring(xfwd.length()).trim();}
                }
            }
            if (bslc.startsWith("re:") || bslc.startsWith("fw:")) {bs = bs.substring(3).trim();}
            else if (bslc.startsWith("fwd:")) {bs = bs.substring(4).trim();}
            else {
                if (bslc.startsWith(xre)) {bs = bs.substring(xre.length()).trim();}
                else {
                    if (bslc.startsWith(xfwd)) {bs = bs.substring(xfwd.length()).trim();}
                }
            }
            return collator.compare(as, bs);
        }
