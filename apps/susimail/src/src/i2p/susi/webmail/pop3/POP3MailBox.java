...
public class POP3MailBox implements NewMailListener {
    ...
    public POP3MailBox(String host, int port, String user, String pass) {
        ...
        if (_log.shouldDebug()) {_log.debug("Mailbox(" + host + "," + port + "," + user + ",password)");}
        this.host = host;
        this.port = port;
        this.user = user;
        this.pass = pass;
    ...
    public void getBodies(Collection<FetchRequest> requests) {
        ...
        synchronized(synchronizer) {
            ...
            for (FetchRequest fr : requests) {
                ...
                if (id < 0) {continue;}
                SendRecv sr;
                if (fr.getHeaderOnly() && supportsTOP) {sr = new SendRecv("TOP " + id + " 0", fr.getBuffer());}
                else {
    ...
    public void queueForDeletion(Collection<String> uidls) {
        for (String uidl : uidls) {queueForDeletion(uidl);}
    }
        ...
        synchronized(synchronizer) {
            try {
                if (isConnected()) {doDelete(noWait);}
                    ...
                    checkConnection(); // calls connect() which calls doDelete()
                    sendCmd1a("QUIT");
                    if (socket != null) {
                        try {socket.close();}
                        catch (IOException e) { /* ignored */ }
    ...
    private void checkConnection() throws IOException {
        ...
        if (!isConnected()) {
            connect();
            if (!isConnected()) {throw new IOException(_t("Cannot connect"));}
        }
    }
    ...
    private void updateMailCount(String response) {
        ...
        try {
            ...
            mails = Integer.parseInt(i != -1 ? response.substring(4, i)
                                             : response.substring(4));
        } catch (NumberFormatException nfe) {mails = 0;}
    }
    ...
    private void updateUIDLs(List<String> lines) {
        ...
        } else if (_log.shouldDebug()) {_log.debug("Error getting UIDL list from server.");}
    }
    ...
    public boolean connectToServer(NewMailListener nml) {
        ...
        synchronized(synchronizer) {
            ...
            else {t = new I2PAppThread(new ConnectRunner(nml), "POP3Conn");}
        }
        try {t.start();}
    ...
    private boolean doHandshake() throws IOException {
        ...
        if (rv && capa != null) {
            ...
            gotCAPA = true;
            if (_log.shouldDebug()) {
                _log.debug("POP3 server caps: pipelining? " + supportsPipelining + " UIDL? " + supportsUIDL +
                           " TOP? " + supportsTOP);
            }
        }
        return rv;
    ...
    private boolean doCheckMail() throws IOException {
        ...
        else if (_log.shouldDebug()) {_log.debug("STAT failed");}
        if (uidl.result) {updateUIDLs(uidl.ls);}
        else if (_log.shouldDebug()) {_log.debug("UIDL failed");}
        if (list.result) {updateSizes(list.ls);}
        else if (_log.shouldDebug()) {_log.debug("LIST failed");}
    ...
    private boolean sendCmds(List<SendRecv> cmds) throws IOException {
        ...
        for (SendRecv sr : cmds) {
            ...
            if (!foo.startsWith("+OK")) {
                ...
                if (result) {lastError = foo;} // actually the first error, for better info to the user
                result = false;
                sr.result = false;
            } else {
    ...
    private Buffer sendCmdN(String cmd, Buffer buffer) {
        synchronized (synchronizer) {
            try {return sendCmdNa(cmd, buffer);}
            catch (IOException e) {
            ...
            } else if (_log.shouldDebug()) {_log.debug("not connected after reconnect");}
        }
        return null;
    }
    ...
    private void getResultNa(Buffer buffer) throws IOException {
        ...
        try {
            ...
            while (DataHelper.readLine(input, buf)) {
                ...
                if (buf.charAt(len - 1) != '\r') {out.write('\n');}
                out.write('\n');
                buf.setLength(0);
            }
            success = true;
    ...
    private List<String> getResultNl() throws IOException {
        ...
        while (DataHelper.readLine(input, buf)) {
            ...
            if (len == 0) {break;} // huh? no \r?
            if (len == 2 && buf.charAt(0) == '.' && buf.charAt(1) == '\r') {break;}
            if (System.currentTimeMillis() - startTime > timeOut) {throw new IOException(_t("No response from server"));}
            String line;
            // RFC 1939 sec. 3 de-byte-stuffing
            if (buf.charAt(0) == '.') {line = buf.substring(1);}
            else {line = buf.toString();}
    ...
    public String lastError() {
        ...
        String e = lastError;
        if (e.startsWith("-ERR ") && e.length() > 5) {e = e.substring(5);}
        // translate this common error
        if (e.trim().equals("Login failed.")) {e = _t("Login failed");}
        else if (e.startsWith("Login failed.")) {e = _t("Login failed") + e.substring(13);}
        return e;
    }
    ...
    private int getIDfromUIDL(String uidl) {
        ...
        if (intObject != null) {result = intObject.intValue();}
        return result;
    }
