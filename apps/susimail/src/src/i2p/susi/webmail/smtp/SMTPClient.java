...
public class SMTPClient {
    ...
    public boolean sendMail(String host, int port, String user, String pass, String sender,
        ...
        try {
            ...
            if (ok) {
                ...
                if (result != 220) {
                    ...
                    if (result != 0) {error += _t("Server refused connection") + " (" + result + ")\n";}
                    else {error += _t("Cannot connect") + " (" + host + ':' + port + ")\n";}
                    ok = false;
                }
            }
            if (ok) {
            ...
            if (ok && maxSize < DEFAULT_MAX_SIZE) {
                ...
                if (attachments != null && !attachments.isEmpty()) {
                    for (Attachment a : attachments) {total += a.getSize();}
                }
        ...
        if (!mailSent && lastResponse.length() > 0) {
            ...
            for (int i = 0; i < lines.length; i++) {errorSb.append(lines[i]).append('\n');}
    ...
    public static void writeMail(Writer out, StringBuilder body,
        ...
        if (attachments != null && !attachments.isEmpty()) {
            for (Attachment attachment : attachments) {
                ...
                if (encoding == null) {throw new EncodingException(_t("No Encoding found for {0}", encodeTo));}
