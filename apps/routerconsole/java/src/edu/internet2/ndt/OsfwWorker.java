 ...
 * sufficient to determine if the server-&gt;client direction has a fire-wall.</p>
 *
 * <p>As part of the simple firewall test, the Server must try to connect to the
 ...
 * connection. This class implements this functionality.</p>
 *
...
public class OsfwWorker implements Runnable {
     ...
     * Constructor
     *
     * @param srvSocketParam Socket used to transmit protocol messages
     *
     * @param iParamTestTime Test time duration to wait for message from server
     */
     ...
     *
     * @param srvSocketParam Socket on which to accept connections
     * @param iParamTestTime Test time duration to wait for message from server
     * @param _localParam Applet object used to set the result of the S-&gt;C firewall test
     */
    ...
    public void run() {
        ...
        try {
            if (socketObj != null) {socketObj.close();}
            _srvSocket.close();
        } catch (IOException e) {
