...
class PluginUpdateRunner extends UpdateRunner {
    ...
    public void transferComplete(long alreadyTransferred, long bytesTransferred, long bytesRemaining, String url, String outputFile, boolean notModified) {
        if (!(_xpi2pURL.startsWith("file:") || _method == UpdateMethod.FILE)) updateStatus("<b>" + _t("Plugin downloaded") + ": " + _appDisplayName + "</b>");
    ...
    private void processFinal(File to, File appDir, String url, Properties props, String sudVersion, String pubkey, String signer) {
        ...
        if (destDir.exists()) {
            ...
            if (oldVersion == null || VersionComparator.comp(oldVersion, version) >= 0) {
                to.delete();
                statusDone("<b>" + _t("Downloaded plugin version {0} is not newer than installed plugin", version).replace("Downloaded plugin version", "") + "</b>");
                return;
            }
        ...
        if (Boolean.parseBoolean(props.getProperty("dont-start-at-install"))) {
            ...
            try {
                if (PluginStarter.startPlugin(_context, appName)) {
                    ...
                    String link;
                    if (linkName != null && linkURL != null) link = "<a target=_blank href=\"" + linkURL + "\"/>" + linkName + ' ' + version + "</a>";
                    else link = appName + ' ' + version;
                    statusDone("<b>" + _t("Plugin {0} installed and started", link) + "</b>");
                } else statusDone("<b>" + _t("Plugin {0} installed but failed to start, check logs", appName + ' ' + version) + "</b>");
            } catch (Throwable e) {
