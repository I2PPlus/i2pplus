...
class PluginUpdateRunner extends UpdateRunner {
    ...
    public PluginUpdateRunner(
        ...
        if (uris.isEmpty()) throw new IllegalArgumentException("uri cannot be empty");
        else _uri = uris.get(0);
        ...
        if (_xpi2pURL.startsWith("file:") || _method == UpdateMethod.FILE) {
            ...
            if (xpi2pfile == null || xpi2pfile.length() == 0) {
                ...
                long alreadyTransferred =
                        (new File(xpi2pfile)).getAbsoluteFile().length();
            ...
            // boolean shouldProxy = _context.getProperty(ConfigUpdateHandler.PROP_SHOULD_PROXY,
            // ConfigUpdateHandler.DEFAULT_SHOULD_PROXY);
            // always proxy, or else FIXME
            boolean shouldProxy = true;
            String proxyHost =
                    _context.getProperty(ConfigUpdateHandler.PROP_PROXY_HOST, ConfigUpdateHandler.DEFAULT_PROXY_HOST);
        ...
        if (up.haveKey(pubkey)) {
            ...
            if ((!ok) || !(signer.equals(signingKeyName) || signingKeyName != null && signingKeyName.isEmpty())) {
                ...
                if (signingKeyName == null)
                    _log.error(
                            "Failed to verify plugin signature, corrupt plugin or bad signature, signed by: " + signer);
                else
                    _log.error("Plugin signer \"" + signer
                            + "\" does not match existing signer in plugin.config file \"" + signingKeyName + "\"");
            ...
            if (!up.addKey(pubkey, signer)) {
                ...
                f.delete();
                to.delete();
                _log.error("Bad key or key mismatch - Failed to add plugin key \"" + pubkey + "\" for plugin signer \""
                        + signer + "\"");
                statusDone("<b>" + _t("Plugin signature verification of {0} failed", url) + "</b>");
                return;
            }
            ...
            if (!signer.equals(signingKeyName)) {
                ...
                if (signingKeyName == null)
                    ...
                    // shouldn't happen
                    _log.error("Plugin signer \"" + signer + "\" does not match new signer in plugin.config file \""
                            + signingKeyName + "\"");
                statusDone("<b>" + _t("Plugin signature verification of {0} failed", url) + "</b>");
                return;
            }
    ...
    private Properties getPluginConfig(File f, File to, String url) {
        ...
        if (!FileUtil.extractZip(to, tempDir, Log.ERROR)) {
            ...
            if (!FileUtil.isPack200Supported()) msg += " (or requires Pack200 which is unsupported in this JVM)";
            statusDone("<b>" + msg + "</b>");
            return null;
        }
    ...
    private void processFinal(
        ...
        if (appName == null
            ...
            to.delete();
            statusDone("<b>"
                    + _t("Plugin from {0} has invalid name or version", url).replace("http://", "") + "</b>");
            return;
        }
        ...
        if (destDir.exists()) {
            if (Boolean.parseBoolean(props.getProperty("install-only"))) {
                to.delete();
                statusDone("<b>"
                        + _t("Downloaded plugin is for new installs only, but the plugin is already installed", url)
                        + "</b>");
                return;
            }
            ...
            try {
                ...
                to.delete();
                statusDone(
                        "<b>" + _t("Installed plugin does not contain the required configuration file", url) + "</b>");
                return;
            }
            ...
            if ((pubkey != null && !pubkey.equals(oldPubkey))
                    || (!signer.equals(oldKeyName))
                    || (!appName.equals(oldAppName))) {
                to.delete();
            ...
            if (oldVersion == null || VersionComparator.comp(oldVersion, version) >= 0) {
                to.delete();
                statusDone("<b>"
                        + _t("Downloaded plugin version {0} is not newer than installed plugin", version)
                                .replace("Downloaded plugin version", "")
                        + "</b>");
                return;
            }
            ...
            if (minVersion != null && VersionComparator.comp(minVersion, oldVersion) > 0) {
                to.delete();
                statusDone("<b>" + _t("Plugin update requires installed plugin version {0} or higher", minVersion)
                        + "</b>");
                return;
            }
            ...
            if (maxVersion != null && VersionComparator.comp(maxVersion, oldVersion) < 0) {
                to.delete();
                statusDone("<b>" + _t("Plugin update requires installed plugin version {0} or lower", maxVersion)
                        + "</b>");
                return;
            }
            ...
            if (Boolean.parseBoolean(props.getProperty("router-restart-required"))) {
                ...
                try {
                    if (!FileUtil.copy(
                            to,
                            (new SecureFile(new SecureFile(appDir.getCanonicalPath() + "/" + appName + "/" + ZIP)
                                    .getCanonicalPath())),
                            true,
                            true)) {
                        to.delete();
                        statusDone(
                                "<b>" + _t("Cannot copy plugin to directory {0}", destDir.getAbsolutePath()) + "</b>");
                        return;
                    }
                } catch (Throwable t) {
                    to.delete();
                ...
                to.delete();
                statusDone("<b>" + _t("Plugin will be installed on next restart.") + ' ' + appName + ' ' + version
                        + "</b>");
                return;
            }
            ...
            if (Boolean.parseBoolean(props.getProperty("update-only"))) {
                to.delete();
                statusDone("<b>" + _t("Plugin is for upgrades only, but the plugin is not installed") + ". " + appName
                        + ' ' + version + "</b>");
                return;
            }
            if (!destDir.mkdir()) {
        ...
        if (Boolean.parseBoolean(props.getProperty("dont-start-at-install"))) {
            ...
            try {
                if (PluginStarter.startPlugin(_context, appName)) {
                    String linkName =
                            PluginStarter.stripHTML(props, "consoleLinkName_" + Messages.getLanguage(_context));
                    if (linkName == null) linkName = PluginStarter.stripHTML(props, "consoleLinkName");
                    ...
                    else link = appName + ' ' + version;
                    statusDone("<b>" + _t("Plugin {0} installed and started", link) + "</b>");
                } else
                    statusDone(
                            "<b>" + _t("Plugin {0} installed but failed to start, check logs", appName + ' ' + version)
                                    + "</b>");
            } catch (Throwable e) {
                statusDone("<b>" + _t("Plugin {0} installed but failed to start", appName + ' ' + version) + ": " + e
                        + "</b>");
