...
class SidebarRenderer {
    ...
    private String getToggle() {
        return "<input type=checkbox id=" + toggleId + " class=\"toggleSection script\" checked hidden>";
    }
    ...
    public void renderSummaryHTML(Writer out) throws IOException {
        ...
        for (String section : sections) {
            ...
            else if ("I2PInternals".equals(section) && (isUnified || !isSitemap)) buf.append(renderI2PInternalsHTML());
            else if ("Advanced".equals(section) && (isUnified || !isSitemap)) buf.append(renderAdvancedHTML());
            else if ("RouterInfo".equals(section) || "General".equals(section))
                buf.append(renderRouterInfoHTML()); // Backwards-compatibility
            else if ("ShortRouterInfo".equals(section) || "ShortGeneral".equals(section))
                buf.append(renderShortRouterInfoHTML()); // Backwards-compatibility
            else if ("AdvancedRouterInfo".equals(section)) buf.append(renderAdvancedRouterInfoHTML());
            else if ("MemoryBar".equals(section)) buf.append(renderMemoryBarHTML());
            else if ("CPUBar".equals(section)) buf.append(renderCPUBarHTML());
            ...
            else if ("RestartStatus".equals(section)) buf.append(restartStatus); // prerendered above
            ...
            else if ("BandwidthGraph".equals(section)) buf.append(renderBandwidthGraphHTML());
            ...
            else if ("NewsHeadings".equals(section) && (isUnified || isHome)) buf.append(renderNewsHeadingsHTML());
            else if ("Clock".equals(section)) buf.append(renderClockHTML());
    ...
    public String renderI2PServicesHTML() {
        ...
        if (pm.isRegistered(PortMapper.SVC_SUSIDNS)) {
            ...
            rbuf.append("<a href=\"/dns?book=router&amp;filter=latest\" target=_top title=\"")
                    ...
                    .append(tx)
                    .append("\" hidden><span><img src=/themes/console/images/addressbook.svg> <span id=newHosts"
                            + " class=\"badge volatile\" title=\"")
    ...
    public String renderClockHTML() {
        ...
        buf.append("<div id=clock class=volatile>")
                .append(dfmt.format(Date.from(Instant.now())))
                .append("</div>");
        return buf.toString();
    }
    ...
    public String renderI2PInternalsHTML() {
        ...
        rbuf.append("<a class=volatile href=/")
    ...
    public String renderRouterInfoHTML() {
        ...
        buf.append("<h3><a href=/info target=_top title=\"")
                .append(_t(
                        "Your Local Identity [{0}] is your unique I2P router identity, similar to an IP address but"
                                + " tailored to I2P. ",
                        _helper.getIdent()))
    ...
    public String renderAdvancedRouterInfoHTML() {
        ...
        if (!sections.contains("MemoryBar")) {
            buf.append("<tr title=\"")
                    .append(_t("How much RAM I2P is using / total RAM available to I2P (excludes RAM allocated to"
                            + " the JVM)"))
                    .append("\">")
                    .append("<td><b>")
                    .append(_t("Memory"))
    ...
    public String renderNetworkReachabilityHTML() {
        ...
        if (floodfillEnabled()) {
            buf.append(" floodfill");
        }
        if (shuttingDown) {
            buf.append(" shuttingDown");
        } else if (restarting) {
            buf.append(" restarting");
        }
        ...
        if (buildStatus.startsWith("[starting]")) {
            ...
            buildStatusText =
                    buildStatusText.substring(buildStatusText.indexOf(']') + 1).trim();
        ...
        buf.append(" <span id=sb_tunnelBuildStatus class=\"volatile ")
                .append(buildStatusClass)
                .append("\" title=\"")
                .append(buildStatusTooltip)
                .append("\">")
                .append("</span>");
    ...
    public String renderBandwidthGraphHTML() {
        ...
        String g =
                "style=\"background-image:url(\'/viewstat.jsp?stat=bw.combined&amp;periodCount=20&amp;width=250&amp;height=50&amp;hideLegend=true&amp;hideGrid=true&amp;hideTitle=true"
                        + t + "\')\"";
        StringBuilder buf = new StringBuilder(512); // NOPMD - AvoidUnnecessaryStringBuilderCreation
    ...
    public String renderTunnelsHTML() {
        ...
        int totalTunnels = _helper.getInboundTunnels()
                + _helper.getOutboundTunnels()
                + _helper.getInboundClientTunnels()
                + _helper.getOutboundClientTunnels()
                + partTunnels;
        ...
        if (_helper.getTunnelBuildSuccess() > 0) {
            buf.append(" / ").append(_t("Tunnel build success (higher of 1m or 10m average)"));
        }
        buf.append("\">").append(totalTunnels);
        if (_helper.getTunnelBuildSuccess() > 0) {
            buf.append(" / ").append(_helper.getTunnelBuildSuccess()).append("%");
        }
        buf.append("</span></a>").append(getToggle()).append("</h3>\n<hr class=b>\n<table id=sb_tunnels>\n");
        ...
        buf.append("<tr title=\"")
                ...
                .append(_t("inbound / outbound"))
                .append(")")
                .append("\"><td><a href=/tunnels#exploratory><b>")
                .append(_t("Utility"))
                .append("</b></a></td><td class=digits><span class=volatile>")
        ...
        if ((maxTunnels == null || Integer.parseInt(maxTunnels) > 0)
                && !_context.router().isHidden()
                && ri != null
                && !ri.getBandwidthTier().equals("K")
                && !_helper.getShareRatio().toString().equals("0")) {
            buf.append("<tr title=\"")
                    .append(_t("The ratio of tunnel hops we provide to tunnel hops we use - a value greater than"
                            + " 1.00 indicates a positive contribution to the network"))
                    .append("\"><td><b>")
    ...
    public String renderCongestionHTML() {
        ...
        buf.append("</a>").append(getToggle()).append("</h3>\n<hr class=b>\n<table id=sb_queue>\n<tr title=\"");
    ...
    public String renderTunnelStatusHTML() {
        ...
        String buildStatus = getStatus.indexOf('[') >= 0 && getStatus.indexOf(']') > getStatus.indexOf('[')
                ? getStatus.substring(getStatus.indexOf(']') + 1)
                : getStatus;
        ...
        if (getStatus.startsWith("[starting]"))
            ...
            buf.append(" rejecting\" title=\"")
                    .append(_t("Router or network performance is impeding the building of transit tunnels"));
        else if (getStatus.startsWith("[rejecting/max]"))
            buf.append(" rejecting\" title=\"")
                    .append(_t("Router is currently hosting the configured maximum number of transit tunnels"));
        else if (getStatus.startsWith("[rejecting/bandwidth]"))
            buf.append(" rejecting\" title=\"")
                    .append(_t("Router is currently using the configured maximum share bandwidth"));
            ...
            buf.append(" active\" title=\"")
                    .append(_t("Accepting transit tunnel requests; hosting {0} tunnels", partTunnels));
            ...
            buf.append(" rejecting disabled\" title=\"")
                    .append(_t("Router is configured to reject all transit tunnel requests"));
        ...
        if (consoleNonce != null) {
            ...
            if (!entries.isEmpty()) {
                ...
                for (NewsEntry entry : entries) {
                    if (i >= min && entry.updated > 0 && entry.updated < oldest) {
                        break;
                    }
                    if (i == 0) {
