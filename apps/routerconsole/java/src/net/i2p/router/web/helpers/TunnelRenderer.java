...
class TunnelRenderer {
    ...
    public void renderStatusHTML(Writer out) throws IOException {
        ...
        if (sorted.size() > 1) DataHelper.sort(sorted, new TPComparator());
        for (TunnelPool in : sorted) {
            Hash client = in.getSettings().getDestination();
            boolean isLocal = _context.clientManager().isLocal(client);
            if ((!isLocal && !isAdvanced) || (getTunnelName(in).startsWith("Ping") && getTunnelName(in).contains("[")) || getTunnelName(in).equals("I2Ping")) {
                continue;
            }
            TunnelPool outPool = tm.getOutboundPool(client);
            if (in.getSettings().getAliasOf() != null || (outPool != null && outPool.getSettings().getAliasOf() != null)) {
            ...
            if (!in.getSettings().shouldTest() || (outPool != null && !outPool.getSettings().shouldTest())) {
                continue;
            }
            ...
            if (isLocal) {
                ...
                if (_context.clientManager().shouldPublishLeaseSet(client)) {
                    ...
                    if (getTunnelName(in).equals(_t("I2PSnark"))) {
                        out.write("snark ");
                    } else if (getTunnelName(in).toLowerCase(Locale.ROOT).equals("messenger") || getTunnelName(in).toLowerCase(Locale.ROOT).equals("i2pchat")) {
                        out.write("i2pchat ");
                    }
                } else {
                    out.write("client ");
                ...
                if (isAdvanced) {
                    ...
                    out.write(" <a href=\"/tunnelmanager\" title=\"" + _t("Configure tunnels") + "\">[" + _t("configure") + "]</a>");
                }
                out.write(" <a class=\"lsview\" href=\"/netdb?l=3#ls_" + client.toBase32().substring(0, 4) + "\">" + "<span class=\"b32\" title=\"" + _t("View LeaseSet") + "\">" + client.toBase32().substring(0, 4) + "</span></a>");
                out.write("</h3>\n");
    ...
    public void renderParticipating(Writer out, boolean bySpeed) throws IOException {
        ...
        if (hasTransit) {
            ...
            if (bySpeed) {
                sb.append(_t("Fastest Active Transit Tunnels"));
            } else {
                sb.append(_t("Most Recent Active Transit Tunnels"));
            }
            sb.append("&nbsp; &nbsp; <a id=refreshPage class=refreshpage style=float:right href=/transit>").append(_t("Refresh")).append("</a></h3>\n");
            int bwShare = getShareBandwidth();
            if (bwShare > 12) {
                sb.append("<table id=allTransit class=\"tunneldisplay tunnels_participating\">\n<thead><tr data-sort-method=thead><th>").append(_t("Role")).append("</th><th");
                if (!bySpeed) {
                    sb.append(" data-sort-default");
                }
                sb.append(" data-sort-method=number>").append(_t("Expiry")).append("</th><th title=\"").append(_t("Data transferred")).append("\" data-sort-method=number>").append(_t("Data")).append("</th><th");
                if (bySpeed) {
                    sb.append(" data-sort-default");
                }
                ...
                if (isAdvanced) {
                    ...
                    sb.append("<th data-sort-method=number>").append(_t("Receive on")).append("</th>");
                }
                sb.append("<th data-sort-method=number>").append(_t("From")).append("</th>");
                if (isAdvanced) {
                    sb.append("<th>").append(_t("Send on")).append("</th>");
                }
                sb.append("<th data-sort-method=number>").append(_t("To")).append("</th></tr>\n</thead>\n<tbody id=transitPeers>\n");
                long processed = 0;
                RateStat rs = _context.statManager().getRate("tunnel.participatingMessageCount");
                if (rs != null) {
                    processed = (long) rs.getRate(RateConstants.TEN_MINUTES).getLifetimeTotalValue();
                }
                int inactive = 0;
                displayed = 0;
                if (bySpeed) {
                ...
                for (int i = 0; i < participating.size(); i++) {
                    ...
                    if (to == null) {
                        sb.append("<td class=\"cells obep\" title=\"").append(_t("Outbound Endpoint")).append("\">").append(_t("Outbound Endpoint")).append("</td>");
                    } else if (from == null) {
                        sb.append("<td class=\"cells ibgw\" title=\"").append(_t("Inbound Gateway")).append("\">").append(_t("Inbound Gateway")).append("</td>");
                    } else {
                        sb.append("<td class=\"cells ptcp\" title=\"").append(_t("Participant")).append("\">").append(_t("Participant")).append("</td>");
                    }
                    ...
                    sb.append("<td class=\"cells datatransfer\" data-sort=").append(count).append("><span class=right>").append(sizeInKB >= 1024 ? String.format("%.2f", sizeInMB) : String.format("%.0f", sizeInKB)).append("</span><span class=left>&#8239; ").append(sizeInKB >= 1024 ? "MB" : "KB").append("</span></td>");
                    ...
                    float kbps = bps / 1024;
                    sb.append("<td class=\"cells bps\" data-sort=").append(bps).append("><span class=right>").append(TWO_DECIMALS.get().format(kbps)).append("&#8239; </span><span class=left>KB/s</span></td>");
                    ...
                    if (isAdvanced) {
                        ...
                        // sb.append("</td>");
                        if (recv != 0) {
                            sb.append("<td title=\"").append(_t("Tunnel identity")).append("\"><span class=tunnel_id>").append(recv).append("</span></td>");
                        } else {
                            sb.append("<td><span hidden>&ndash; </span></td>");
                        }
                    }
                    if (from != null) {
                        sb.append("<td><div class=tunnel_peer>").append(netDbLink(from)).append("</div></td>");
                    } else {
                        sb.append("<td><span hidden>&ndash; </span></td>");
                    }
                    ...
                    if (isAdvanced) {
                        if (send != 0) {
                            sb.append("<td title=\"").append(_t("Tunnel identity")).append("\"><span class=tunnel_id>").append(send).append("</span></td>");
                        } else {
                            sb.append("<td><span hidden>&ndash; </span></td>");
                        }
                    }
                    if (to != null) {
                        sb.append("<td><div class=tunnel_peer>").append(netDbLink(to)).append("</div></td>");
                    } else {
                        sb.append("<td><span hidden>&ndash; </span></td>");
                    }
                    sb.append("</tr>\n");
                ...
                if (displayed >= 2) {
                    ...
                    if (inactive > 0) {
                        sb.append("&nbsp; &bullet; &nbsp; <b>").append(_t("Inactive")).append(":</b>&nbsp; ").append(inactive).append("&nbsp; &bullet; &nbsp; <b>").append(_t("Total")).append(":</b>&nbsp; ").append((inactive + displayed));
                    }
                } else if (inactive > 0) {
                    sb.append("<b>").append(_t("Inactive")).append(":</b>&nbsp; ").append(inactive);
                }
                sb.append("</td></tr>\n<tr class=bwUsage><td colspan=8>").append("<b>").append(_t("Lifetime bandwidth usage")).append(":</b>&nbsp; ").append(DataHelper.formatSize2(processed * 1024, true).replace("i", "")).append("B").append("</td></tr></tfoot>\n</table>\n</div>\n");
            } else { // bwShare < 12K/s
                sb.append("<div class=\"statusnotes noparticipate\"><b>").append(_t("Not enough shared bandwidth to build transit tunnels.")).append("</b> <a href=\"config\">[").append(_t("Configure")).append("]</a>\n</div>\n");
            }
        } else if (_context.router().isHidden()) {
            sb.append("<p class=infohelp>").append(_t("Router is currently operating in Hidden Mode which prevents transit tunnels from being built.")).append("</p>");
        } else {
    ...
    public void renderTransitSummary(Writer out) throws IOException {
        ...
        if (!participating.isEmpty() && participating.size() > 1) {
            ...
            tbuf.append("<div class=tablewrap>\n<h3 class=tabletitle>").append(_t("Transit Tunnels by Peer (Top {0})", DISPLAY_LIMIT * 10)).append("</h3>\n<table id=transitSummary class=\"tunneldisplay tunnels_participating\">\n<thead><tr><th id=country data-sort-direction=ascending>").append(_t("Country")).append("</th><th id=router data-sort-direction=ascending>").append(_t("Router")).append("</th><th id=version>").append(_t("Version")).append("</th><th id=tier data-sort=LMNOPX>").append(_t("Tier")).append("</th><th id=address>").append(_t("Address")).append("</th>");
            if (enableReverseLookups()) {
                tbuf.append("<th id=domain>").append(_t("Domain")).append("</th>");
            }
            tbuf.append("<th class=tcount data-sort-method=number data-sort-default>").append(_t("Tunnels")).append("</th><th id=data data-sort-method=number>").append(_t("Data")).append("</th><th id=edit data-sort-method=none>").append(_t("Edit")).append("</th></tr></thead>\n<tbody id=transitPeers>\n");
            ...
            for (Hash h : sorted) {
                ...
                boolean isBanned = _context.banlist().isBanlisted(h) || _context.banlist().isBanlistedHostile(h);

                sb.append("<tr class=lazy><td>").append(peerFlag(h)).append("</td><td><span class=routerHash><a href=\"netdb?r=").append(h.toBase64()).append("\">").append(truncHash).append("</a></span></td><td data-sort=").append(version != null ? DataHelper.stripHTML(version) : "0").append(">");

                if (version != null) {
                    sb.append("<span class=version title=\"").append(_t("Show all routers with this version in the NetDb")).append("\"><a href=\"/netdb?v=").append(DataHelper.stripHTML(version)).append("\">").append(DataHelper.stripHTML(version)).append("</a></span>");
                } else if (isBanned) {
                    sb.append("<span class=banlisted title=\"").append(_t("Router is banlisted")).append("\">???</span>");
                } else {
                    sb.append("<span>???</span>");
                }
                sb.append("</td><td>");
                ...
                if (enableReverseLookups()) {
                    sb.append("<td>");
                    if (rlResult != null && rlResult.canonicalHostName != null && !rlResult.canonicalHostName.isEmpty() && !rlResult.ip.equals(rlResult.canonicalHostName)) {
                        String display = (rlResult.whois != null) ? rlResult.whois : rlResult.domain;
                        if (display == null) display = _t("unknown");
                        sb.append("<span class=rlookup title=\"").append(rlResult.canonicalHostName).append("\">").append(display).append("</span>");
                    } else {
                ...
                if (bw > 0) {
                    sb.append("<span class=data>").append(TWO_DECIMALS.get().format(bw).replace(".00", "")).append("KB</span>");
                } else {
                ...
                if (isBanned) {
                    sb.append("<span hidden>ban</span><a class=banlisted href=\"/profiles?f=3\" title=\"").append(_t("Router is banlisted")).append("\">Banned</a> ");
                    bannedCount++;
                }
                sb.append("</td>");
                ...
                if (info != null && info.getHash() != null) {
                    sb.append("<a class=configpeer href=\"/configpeer?peer=").append(info.getHash()).append("\" title=\"Configure peer\">").append(_t("Edit")).append("</a>");
                }
                sb.append("</td></tr>\n");
            }
    ...
    public void renderPeers(Writer out) throws IOException {
        ...
        if (!peerList.isEmpty() && (tunnelCount > 0 || partCount > 0)) {
            ...
            headerSb.append("<div class=tablewrap>\n<h3 class=tabletitle id=peercount>").append(_t("All Tunnels by Peer")).append("&nbsp; &nbsp; <a id=refreshPage class=refreshpage style=float:right href=/tunnelpeercount>").append(_t("Refresh")).append("</a></h3>\n<table id=tunnelPeerCount><thead class=lazy>\n<tr><th id=country data-sort-direction=ascending>").append(_t("Country")).append("</th><th id=router>").append(_t("Router")).append("</th><th id=version>").append(_t("Version")).append("</th><th id=tier data-sort=LMNOPX>").append(_t("Tier")).append("</th><th id=address title=\"").append(_t("Primary IP address")).append("\">").append(_t("Address")).append("</th>");
            if (doReverseLookups) {
                headerSb.append("<th id=domain>").append(_t("Domain")).append("</th>");
            }
            headerSb.append("<th class=tcount colspan=2 title=\"Client and Exploratory Tunnels\" data-sort-method=number data-sort-column-key=localCount>").append(_t("Local")).append("</th><th class=tcount colspan=2 data-sort-method=number data-sort-column-key=transitCount>").append(_t("Transit")).append("</th><th id=edit data-sort-method=none>").append(_t("Edit")).append("</th></tr>\n</thead>\n<tbody id=allPeers>\n");
            ...
            for (int start = 0; start < validPeerList.size(); start += chunkSize) {
                ...
                for (int i = start; i < end; i++) {
                    ...
                    chunkSb.append("<tr class=lazy><td>").append(peerFlag(h)).append("</td><td><span class=routerHash><a href=\"netdb?r=").append(h.toBase64()).append("\">").append(truncHash).append("</a></span></td><td data-sort=").append(version != null ? DataHelper.stripHTML(version) : "0").append(">");
                    if (version != null) {
                        chunkSb.append("<span class=version title=\"").append(_t("Show all routers with this version in the NetDb")).append("\"><a href=\"/netdb?v=").append(DataHelper.stripHTML(version)).append("\">").append(DataHelper.stripHTML(version)).append("</a></span>");
                    }
                    chunkSb.append("</td><td>").append(_context.commSystem().renderPeerCaps(h, false)).append("</td><td><span class=ipaddress>");
                    ...
                    if (doReverseLookups) {
                        ...
                        if (rlResult != null && rlResult.canonicalHostName != null && !rlResult.canonicalHostName.isEmpty() && !rlResult.ip.equals(rlResult.canonicalHostName)) {
                            String display = (rlResult.whois != null) ? rlResult.whois : rlResult.domain;
                            if (display == null) display = _t("unknown");
                            chunkSb.append("<span class=rlookup title=\"").append(rlResult.canonicalHostName).append("\">").append(display).append("</span>");
                        } else {
                            chunkSb.append("&ndash; ");
                        }
                        chunkSb.append("</td>");
                    ...
                    if (localTunnelCount > 0) {
                        chunkSb.append(String.format("<td class=tcount data-sort-column-key=localCount data-sort=%d>%d</td><td class=bar data-sort-column-key=localCount>", localTunnelCount, localTunnelCount));
                        chunkSb.append(String.format("<span class=percentBarOuter><span class=percentBarInner style=\"width:%s%%\"><span class=percentBarText>%d%%</span></span></span>", TWO_DECIMALS.get().format(localTunnelCount * 100.0 / tunnelCount).replace(".00", ""), localTunnelCount * 100 / tunnelCount));
                    } else {
                    ...
                    if (!peerList.isEmpty()) {
                        if (transitTunnelCount > 0) {
                            chunkSb.append(String.format("<td class=tcount data-sort-column-key=transitCount data-sort=%d>%d</td><td class=bar data-sort-column-key=transitCount>", transitTunnelCount, transitTunnelCount)).append(String.format("<span class=percentBarOuter><span class=percentBarInner style=\"width:%s%%\"><span class=percentBarText>%d%%</span></span></span>", TWO_DECIMALS.get().format(transitTunnelCount * 100.0 / partCount).replace(".00", ""), transitTunnelCount * 100 / partCount)).append("</td>");
                        } else {
                            chunkSb.append("<td class=tcount colspan=2 data-sort=0></td>");
                        }
                    } else {
                    ...
                    chunkSb.append(String.format("<td><a class=configpeer href=\"/configpeer?peer=%s\" title=\"%s\">%s</a></td></tr>\n", info.getHash(), _t("Configure peer"), _t("Edit")));
                }
                out.write(chunkSb.toString());
                out.flush();
            }
            StringBuilder footerSb = new StringBuilder();
            footerSb.append("</tbody>\n<tfoot class=lazy><tr class=tablefooter data-sort-method=none><td colspan=4><b>").append(validPeerList.size()).append(" ").append(_t("unique peers")).append("</b></td><td></td>");
            if (doReverseLookups) {
                footerSb.append("<td></td>");
            }
            footerSb.append("<td colspan=2><b>").append(tunnelCount).append(" ").append(_t("local")).append("</b></td>");
            if (partCount > 0) {
                footerSb.append("<td colspan=2><b>").append(partCount).append(" ").append(_t("transit")).append("</b></td>");
            } else {
                footerSb.append("<td></td>");
            }
    ...
    private ReverseLookupResult getReverseLookupInfo(Hash h, RouterInfo info, long uptime) {
        ...
        if (ip != null && enableReverseLookups() && uptime > 30 * 1000) {
            ...
            if (rl != null && rl.contains(" ")) {
                String whoisChain = rl.replace("Administered by ", "") .replace("Asia Pacific Network Information Centre (APNIC)", "APNIC")
                ...
                String whois = PARENS_CONTENT.matcher(whoisChain).replaceAll("").trim();
                result.whois = whois;
                result.domain = null;
            } else if (rl != null) {
    ...
    private void renderPool(Writer out, TunnelPool in, TunnelPool outPool) throws IOException {
        ...
        if (tunnels.size() != 0) {
            buf.append("<table class=\"tunneldisplay tunnels_client\">\n<tr><th title=\"").append(_t("Inbound or outbound?")).append("\">").append(_t("In/Out")).append("</th><th class=status title=\"").append(_t("Tunnel test status")).append("\">").append(_t("Status")).append("</th><th>").append(_t("Expiry")).append("</th><th class=latency title=\"").append(_t("Latency")).append("\">").append(_t("Latency")).append("</th><th title=\"").append(_t("Data transferred")).append("\">").append(_t("Data")).append("</th><th>").append(_t("Gateway")).append("</th>");
            if (maxLength > 3) {
                buf.append("<th colspan=\"").append(maxLength - 2).append("\">").append(_t("Participants")).append("</th>");
            } else if (maxLength == 3) {
                buf.append("<th>").append(_t("Participant")).append("</th>");
            }
            if (maxLength > 1) {
                buf.append("<th>").append(_t("Endpoint")).append("</th>");
            }
            buf.append("</tr>\n");
        }
        ...
        for (int i = 0; i < tunnels.size(); i++) {
            ...
            String rowClass = isFailed ? " class=failed" : isFailing ? " class=failing" : isGood ? " class=good" : isTesting ? " class=testing" : " class=untested";
            if (isInbound) {
                buf.append("<tr").append(rowClass).append("><td class=direction data-sort=in><span class=inbound title=\"").append(tib).append("\"><img src=/themes/console/images/inbound.svg alt=\"").append(tib).append("\"></span></td>");
            } else {
                buf.append("<tr").append(rowClass).append("><td class=direction data-sort=out><span class=outbound title=\"").append(tob).append("\"><img src=/themes/console/images/outbound.svg alt=\"").append(tob).append("\"></span></td>");
            }
            buf.append("<td class=status>");
            switch (testStatus) {
                case GOOD: buf.append("<span title=\"").append(_t("Test successful")).append("\"></span>");
                    break;
                case TESTING: buf.append("<span title=\"").append(_t("Test in progress")).append("\"></span>");
                    break;
                case FAILING: int fails = info.getConsecutiveFailures();
                    buf.append("<span title=\"").append(_t("Test failing (failures: {0})", fails)).append("\"></span>");
                    break;
                case FAILED: buf.append("<span title=\"").append(_t("Test failed (3 consecutive failures)")).append("\"></span>");
                    break;
                default: buf.append("<span title=\"").append(_t("Not yet tested")).append("\"></span>");
                    break;
            }
            buf.append("</td>");
            ...
            if (count > 0) {
                buf.append("<span class=right>").append(sizeInKB >= 1024 ? String.format("%.2f", sizeInMB) : String.format("%.0f", sizeInKB)).append("</span><span class=left>&#8239; ").append(sizeInKB >= 1024 ? "MB" : "KB").append("</span>");
            }
            buf.append("</td>");
            int length = info.getLength();
            boolean isAdvanced = _context.getBooleanProperty(HelperBase.PROP_ADVANCED);
            for (int j = 0; j < length; j++) {
                ...
                if (_context.routerHash().equals(peer)) {
                    ...
                    if (isAdvanced) {
                        buf.append("<span class=tunnel_id title=\"").append(_t("Tunnel identity")).append("\">").append((id == null ? "" : "" + id)).append("</span>");
                    }
                    buf.append("</td>");
                } else {
        ...
        if (live > 0) {
            ...
            buf.append("<tfoot id=statusnotes>").append("<tr class=bwUsage><td colspan=").append(colCount).append(" class=center><b>").append(_t("Lifetime bandwidth usage")).append(":&nbsp; &nbsp; ").append(DataHelper.formatSize2(processedIn * 1024, true).replace("i", "")).append("B ").append(_t("in")).append(", ").append(DataHelper.formatSize2(processedOut * 1024, true).replace("i", "")).append("B ").append(_t("out")).append("</b></td></tr></tfoot>\n");
        }
    ...
    public void renderLifetimeBandwidth(Writer out, TunnelPool in, TunnelPool outPool) throws IOException {
        ...
        StringBuilder buf = new StringBuilder(32 * 1024);
        if (tunnels.size() != 0) {
            buf.append("<table id=tunnelbandwidth><tr><th>").append(_t("Tunnel Name")).append("</th><th>").append(_t("Data In")).append("</th><th>").append(_t("Data Out")).append("</th></tr>\n");
        }
        long lifetimeIn = 0;
        long lifetimeOut = 0;
        for (int i = 0; i < tunnels.size(); i++) {
            ...
            buf.append("<tr><td>").append(tunnelName).append("</td><td>").append(DataHelper.formatSize2(lifetimeIn, true)).append("</td><td>").append(DataHelper.formatSize2(lifetimeOut, true)).append("</td></tr>\n");
        }
        buf.append("</table>\n");
        out.append(buf);
        out.flush();
    ...
    private String renderExpiryBar(long timeLeft) {
        ...
        return "<span class=\"percentBarOuter" + expiry + "\">" + "<span class=percentBarInner style=width:" + percent + "%>" + "<span class=percentBarText>" + timeStr + "</span></span></span>";
    }
    ...
    private void renderPoolSummary(Writer out, TunnelPool in, TunnelPool outPool, Hash client) throws IOException {
        ...
        out.write("<th class=inCount title=\"" + _t("Inbound") + ": " + _t("Active / Configured") + "\">" + inCount + sep + inWanted + "</th>");
        out.write("<th class=outCount title=\"" + _t("Outbound") + ": " + _t("Active / Configured") + "\">" + outCount + sep + outWanted + "</th><th id=sep></th>");
        out.write("<th class=\"inBuild" + (buildIn ? " building" : "") + "\" title=\"" + _t("Inbound") + ": " + _t("Building") + "&hellip; \">" + inBuilding + "</th>");
        out.write("<th class=\"outBuild" + (buildOut ? " building" : "") + "\"  title=\"" + _t("Outbound") + ": " + _t("Building") + "&hellip; \">" + outBuilding + "</th>");
        out.write("</tr></thead>\n</table>\n");
    }
    ...
    private long getNextExpiry(TunnelPool pool) {
        ...
        for (TunnelInfo ti : pool.listTunnels()) {
            ...
            if (ti.getTunnelFailed() || ti.getTestStatus() == net.i2p.router.TunnelTestStatus.FAILED || ti.getConsecutiveFailures() > 1) {
                continue;
            }
