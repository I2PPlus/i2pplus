...
public class ConfigUIHelper extends HelperBase {
    ...
    public String getSettings() {
        ...
        for (String theme : themes) {
            buf.append("<label for=\"").append(theme).append("\"><div class=themechoice style=display:inline-block; text-align:center>").append("<input type=radio class=optbox name=theme ");
            if (theme.equals(current)) {
                buf.append(CHECKED);
            }
            buf.append("value=\"").append(theme).append("\" id=\"").append(theme).append("\"><img height=48 width=48 alt=\"\" src=\"/themes/console/").append(theme).append("/images/thumbnail.png\"><br><div class=themelabel>").append(_t(theme)).append("</div></div></label>\n");
        }
        buf.append("</div><div id=themeoptions><label><input type=checkbox class=\"optbox slider\" name=\"universalTheming\" ");
        if (universalTheming) {
            buf.append(CHECKED);
        }
        buf.append("value=1>").append(_t("Set theme universally across all apps")).append("</label><br>\n<label><input type=checkbox class=\"optbox slider\" name=\"useSoraFont\" ");
        if (useSoraFont) {
            buf.append(CHECKED);
        }
        buf.append("value=1>").append(_t("Use alternative display font in console and webapps")).append("</label><br>\n");
        return buf.toString();
    }
    ...
    public String getForceMobileConsole() {
        ...
        buf.append("value=1>").append(_t("Force the mobile console to be used")).append("</label><br>\n<label title=\"").append(_t("Enabling the Universal Theming option is recommended when embedding these applications")).append("\"><input type=checkbox class=\"optbox slider\" name=embedApps ");
        if (embedApps) {
            buf.append(CHECKED);
        }
        buf.append("value=1>").append(_t("Embed I2PSnark and I2PMail in the console")).append("</label></div>\n");
        return buf.toString();
    }
    ...
    public String getLangSettings() {
        ...
        for (int i = 0; i < langs.length; i++) {
            ...
            buf.append("<label for=\"").append(lang).append("\"><div class=langselect>").append("<input type=radio class=optbox name=lang ");
            if (lang.equals(current)) buf.append(CHECKED);
            buf.append("value=\"").append(lang).append("\" id=\"").append(lang).append("\">").append("<span class=langflag><img width=48 height=36 alt=\"\" src=\"/flags.jsp?c=").append(langs[i][1]).append("\"></span>").append("<div class=ui_lang>");
    ...
    public String getPasswordForm() {
        ...
        if (!config.toString().contains("routerconsole.auth.i2prouter")) buf.append("<style>#consolepass .delete{display:none!important)</style>\n");
        buf.append("<table id=consolepass>\n");
        if (userpw.isEmpty()) {
            buf.append("<tr><td class=infohelp colspan=3>").append(_t("Add a user and password to enable.")).append("&nbsp; ").append(_t("Note: If you forget your password, removing the configuration entry in your router.config file will clear it (restart required).")).append("</td></tr>\n");
        } else {
            buf.append("<tr><td class=infohelp colspan=3>").append(_t("Router console password is enabled.")).append("</td></tr>\n").append("<tr><th title=\"").append(_t("Mark for deletion")).append("\">").append(_t("Remove")).append("</th><th>").append(_t("Username")).append("</th><th>&nbsp; </th></tr>\n");
            for (String name : userpw.keySet()) {
                buf.append("<tr><td><input type=checkbox class=optbox id=\"").append(name).append("\" name=\"delete_").append(name).append("\"></td>").append("<td colspan=2><label for=\"").append(name).append("\">").append(name).append("</label></td></tr>\n");
            }
        }
        buf.append("<tr><td id=pw_adduser colspan=3>").append("<b>").append(_t("Username")).append(":</b> ").append("<input type=text name=name title=\"").append(_t("Please supply a username")).append("\"><b>").append(_t("Password")).append(":</b> ").append("<input type=password size=40 name=nofilter_pw title=\"").append(_t("Please supply a password")).append("\">").append("</td></tr>\n</table>\n");
        return buf.toString();
    }
}
