...
public class ConfigClientsHelper extends HelperBase {
     ...
     */
    private static final Pattern VALID_EMAIL_ADDRESS_REGEX = Pattern.compile("[A-Z0-9._%+-]+@[A-Z0-9.-]+\\.[A-Z2]{2,6}", Pattern.CASE_INSENSITIVE);
    ...
    public boolean isPluginInstallEnabled() {
        return PluginStarter.pluginsEnabled(_context) && (_context.getBooleanPropertyDefaultTrue(PROP_ENABLE_PLUGIN_INSTALL) || isAdvanced());
    }
    ...
    public String getPort() {
        return _context.getProperty(ClientManagerFacadeImpl.PROP_CLIENT_PORT, Integer.toString(ClientManagerFacadeImpl.DEFAULT_PORT));
    }
    ...
    public String getForm1() {
        ...
        buf.append("<table id=clientconfig>\n" + "<tr><th class=right>").append(_t("Client")).append("</th><th class=center>").append(_t("Run at Startup?").replace("?", "")).append("</th><th class=center>").append(_t("Control")).append("</th><th class=left>").append(_t("Class and arguments")).append("</th></tr>\n");
        ...
        if (allowEdit && "new".equals(_edit)) renderForm(buf, Integer.toString(clients.size()), "", false, false, false, false, "", true, false, false, false, false, false);
        buf.append("</table>\n");
        return buf.toString();
    }
    ...
    public String getForm2() {
        StringBuilder buf = new StringBuilder(1024);
        buf.append("<table id=webappconfig>\n" + "<tr><th class=right>").append(_t("WebApp")).append("</th><th class=center>").append(_t("Run at Startup?").replace("?", "")).append("</th><th class=center>").append(_t("Control")).append("</th><th class=left>").append(_t("Description")).append("</th></tr>\n");
        ...
        for (String name : keys) {
            if (name.startsWith(RouterConsoleRunner.PREFIX) && name.endsWith(RouterConsoleRunner.ENABLED)) {
                ...
                if (app.equals("routerconsole")) desc = _t("I2P+ Router Console");
                else if (app.equals("i2psnark")) desc = _t("Torrents");
                else if (app.equals("i2ptunnel")) desc = _t("Tunnel Manager");
                else if (app.equals("imagegen")) desc = _t("Identification Image Generator");
                else if (app.equals("susidns")) desc = _t("Addressbook");
                else if (app.equals("susimail")) desc = _t("Email");
                else desc = DataHelper.escapeHTML(app) + ".war";
                boolean isConsole = RouterConsoleRunner.ROUTERCONSOLE.equals(app);
                renderForm(buf, app, app, isConsole || (isRunning && !"addressbook".equals(app)), "true".equals(val), isConsole, isConsole, desc, false, false, false, isRunning, false, !isRunning);
            }
        }
        buf.append("</table>\n");
        return buf.toString();
    ...
    public String getForm3() {
        StringBuilder buf = new StringBuilder(1024);
        buf.append("<table id=pluginconfig>\n").append("<tr>").append("<th class=right>").append(_t("Plugin")).append("</th>").append("<th class=center>").append(_t("Run at Startup?").replace("?", "")).append("</th>").append("<th class=center>").append(_t("Control")).append("</th>").append("<th class=left>").append(_t("Description")).append("</th>").append("</tr>\n");
        ...
        for (String name : keys) {
            if (name.startsWith(PluginStarter.PREFIX) && name.endsWith(PluginStarter.ENABLED)) {
                ...
                desc.append("\n<table border=0>\n").append("<tr>").append("<td class=right><b>").append(_t("Version")).append("</b></td>").append("<td class=left>").append(stripHTML(appProps, "version"));
                ...
                if (updateURL != null) {
                    desc.append("<tr><td class=right><b>").append(_t("Update link")).append("</b></td>").append("<td class=left><a href=\"").append(updateURL).append("\">").append(updateURL).append("</a></td></tr>\n");
                }
                s = stripHTML(appProps, "websiteURL");
                if (s != null) {
                    desc.append("<tr><td class=right><b>").append(_t("Website")).append("</b></td>").append("<td class=left><a href=\"").append(s).append("\" target=_blank>").append(s).append("</a></td></tr>\n");
                }
                s = stripHTML(appProps, "license");
                if (s != null) {
                    desc.append("<tr><td class=right><b>").append(_t("License")).append("</b></td>").append("<td class=left>").append(s).append("</td></tr>\n");
                }
                desc.append("\n</table>\n");
    ...
    private void renderForm(StringBuilder buf, String index, String name, boolean urlify, boolean enabled, boolean ro, boolean preventDisable, String escapedDesc, boolean edit, boolean showEditButton, boolean showUpdateButton, boolean showStopButton, boolean showDeleteButton, boolean showStartButton) {
        ...
        buf.append("</td><td class=center><input type=checkbox class=\"optbox slider\" id=\"client_").append(index).append("\" name=\"").append(index).append(".enabled\"");
        if (enabled) {
            buf.append(CHECKED);
        }
        ...
        if (showStartButton && (!ro) && !edit) {
            buf.append("<button type=submit title=\"").append(_t("Start")).append("\" class=\"control accept\" name=action value=\"Start ").append(index).append("\">").append(_t("Start")).append("<span class=hide> ").append(index).append("</span></button>");
        }
        if (showStopButton && (!edit)) buf.append("<button type=submit title=\"").append(_t("Stop")).append("\" class=\"control stop\" name=action value=\"Stop ").append(index).append("\">").append(_t("Stop")).append("<span class=hide> ").append(index).append("</span></button>");
        if (isClientChangeEnabled() && showEditButton && (!edit) && !ro) buf.append("<button type=submit title=\"").append(_t("Edit")).append("\" class=\"control add\" name=\"edit\" value=\"Edit ").append(index).append("\">").append(_t("Edit")).append("<span class=hide> ").append(index).append("</span></button>");
        if (showUpdateButton && (!edit) && !ro) {
            buf.append("<button type=submit title=\"").append(_t("Check for updates")).append("\" class=\"control check\" name=action value=\"Check ").append(index).append("\">").append(_t("Check for updates")).append("<span class=hide> ").append(index).append("</span></button>");
            buf.append("<button type=submit title=\"").append(_t("Update")).append("\" class=\"control download\" name=action value=\"Update ").append(index).append("\">").append(_t("Update")).append("<span class=hide> ").append(index).append("</span></button>");
        }
        if (showDeleteButton && (!edit) && !ro) {
            buf.append("<button type=submit title=\"").append(_t("Delete")).append("\" class=\"control delete\" name=action value=\"Delete ").append(index).append("\" client=\"").append(_t(escapedName)).append("\">").append(_t("Delete")).append("<span class=hide> ").append(index).append("</span></button>");
        }
        buf.append("</td>\n<td class=left>");
        if (edit && !ro) {
