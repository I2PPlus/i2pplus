...
public class SidebarHelper extends HelperBase {
    ...
    private static final ThreadLocal<DecimalFormat> ZERO_TWO_DECIMALS =
            ThreadLocal.withInitial(() -> new DecimalFormat("##0.00"));
    ...
    static final String DEFAULT_FULL_NEWUSER = "RouterInfo" + S + "CPUBar" + S + "MemoryBar" + S + "UpdateStatus" + S
            + "Bandwidth" + S + "BandwidthGraph" + S + "NetworkReachability" + S + "FirewallAndReseedStatus" + S
            + "I2PServices" + S + "I2PInternals" + S + "HelpAndFAQ" + S + "Peers" + S + "Tunnels" + S + "TunnelStatus"
            + S + "NewsHeadings" + S + "RestartStatus" + S + "Destinations" + S + "";

    static final String DEFAULT_FULL = "RouterInfo" + S + "CPUBar" + S + "MemoryBar" + S + "UpdateStatus" + S
            + "Bandwidth" + S + "BandwidthGraph" + S + "NetworkReachability" + S + "FirewallAndReseedStatus" + S
            + "I2PServices" + S + "I2PInternals" + S + "Peers" + S + "Tunnels" + S + "TunnelStatus" + S + "NewsHeadings"
            + S + "RestartStatus" + S + "Destinations" + S + "";

    static final String DEFAULT_FULL_ADVANCED = "AdvancedRouterInfo" + S + "CPUBar" + S + "MemoryBar" + S
            + "UpdateStatus" + S + "Bandwidth" + S + "BandwidthGraph" + S + "NetworkReachability" + S
            + "FirewallAndReseedStatus" + S + "I2PServices" + S + "I2PInternals" + S + "Advanced" + S + "Peers" + S
            + "Tunnels" + S + "Congestion" + S + "NewsHeadings" + S + "RestartStatus" + S + "Destinations" + S + "";

    static final String DEFAULT_MINIMAL = "ShortRouterInfo" + S + "CPUBar" + S + "MemoryBar" + S + "UpdateStatus" + S
            + "Bandwidth" + S + "BandwidthGraph" + S + "NetworkReachability" + S + "FirewallAndReseedStatus" + S
            + "Peers" + S + "Tunnels" + S + "NewsHeadings" + S + "RestartStatus" + S + "Destinations" + S + "";

    /** @since 0.9.32 */
    static final String DEFAULT_MINIMAL_ADVANCED = "AdvancedRouterInfo" + S + "CPUBar" + S + "MemoryBar" + S
            + "UpdateStatus" + S + "Bandwidth" + S + "BandwidthGraph" + S + "NetworkReachability" + S
            + "FirewallAndReseedStatus" + S + "Peers" + S + "Tunnels" + S + "Congestion" + S + "NewsHeadings" + S
            + "RestartStatus" + S + "Destinations" + S + "";
    ...
    private NetworkStateMessage reachability() {
        ...
        /*
                if (_context.router().getUptime() > 60*1000 && !_context.clientManager().isAlive() && !_context.router().gracefulShutdownInProgress() &&
        ...
        */
        ...
        if (Math.abs(skew) > 30 * 1000) {
            return new NetworkStateMessage(
                    NetworkState.CLOCKSKEW, _t("Clock Skew of {0}", DataHelper.formatDuration2(Math.abs(skew))));
        }
        ...
        switch (status) {
            ...
            case DISCONNECTED:
                return new NetworkStateMessage(
                        NetworkState.ERROR,
                        _t("Disconnected")); // .sb_warning shows -> _t("Check Network Connection"));

            case HOSED:
                return new NetworkStateMessage(NetworkState.ERROR, _t("UDP Port In Use"));
            // TODO: display advice in .sb_warning -> _t("Set i2np.udp.internalPort=xxxx in advanced config and
            // restart"));
            ...
            default:
                ...
                if (ra.isEmpty() && _context.router().getUptime() > 5 * 60 * 1000) {
                    if (getActivePeers() <= 0) {
                        ...
                        return new NetworkStateMessage(
                                NetworkState.ERROR, _t("UDP Disabled &amp; Inbound TCP host/port not set"));
                    } else {
    ...
    public String getMemory() {
        ...
        long used = (long) _context.statManager()
    ...
    public int getTunnelBuildSuccess() {
        ...
        Rate explSuccess10 =
                _context.statManager().getRate("tunnel.buildExploratorySuccess").getRate(RateConstants.TEN_MINUTES);
        Rate explReject10 =
                _context.statManager().getRate("tunnel.buildExploratoryReject").getRate(RateConstants.TEN_MINUTES);
        Rate explExpire10 =
                _context.statManager().getRate("tunnel.buildExploratoryExpire").getRate(RateConstants.TEN_MINUTES);
        Rate clientSuccess10 =
                _context.statManager().getRate("tunnel.buildClientSuccess").getRate(RateConstants.TEN_MINUTES);
        Rate clientReject10 =
                _context.statManager().getRate("tunnel.buildClientReject").getRate(RateConstants.TEN_MINUTES);
        Rate clientExpire10 =
                _context.statManager().getRate("tunnel.buildClientExpire").getRate(RateConstants.TEN_MINUTES);

        // Get 1-minute rates
        Rate explSuccess1 =
                _context.statManager().getRate("tunnel.buildExploratorySuccess").getRate(RateConstants.ONE_MINUTE);
        Rate explReject1 =
                _context.statManager().getRate("tunnel.buildExploratoryReject").getRate(RateConstants.ONE_MINUTE);
        Rate explExpire1 =
                _context.statManager().getRate("tunnel.buildExploratoryExpire").getRate(RateConstants.ONE_MINUTE);
        Rate clientSuccess1 =
                _context.statManager().getRate("tunnel.buildClientSuccess").getRate(RateConstants.ONE_MINUTE);
        Rate clientReject1 =
                _context.statManager().getRate("tunnel.buildClientReject").getRate(RateConstants.ONE_MINUTE);
        Rate clientExpire1 =
                _context.statManager().getRate("tunnel.buildClientExpire").getRate(RateConstants.ONE_MINUTE);

        // Calculate 10-minute percentage
        int success10 = (int) explSuccess10.getLastEventCount() + (int) clientSuccess10.getLastEventCount();
        int reject10 = (int) explReject10.getLastEventCount() + (int) clientReject10.getLastEventCount();
        int expire10 = (int) explExpire10.getLastEventCount() + (int) clientExpire10.getLastEventCount();
        int percentage10;
        if (success10 < 1) {
            success10 = 1;
        }
        ...
        int success1 = (int) explSuccess1.getLastEventCount() + (int) clientSuccess1.getLastEventCount();
        int reject1 = (int) explReject1.getLastEventCount() + (int) clientReject1.getLastEventCount();
        int expire1 = (int) explExpire1.getLastEventCount() + (int) clientExpire1.getLastEventCount();
        int percentage1;
        if (success1 < 1) {
            success1 = 1;
        }
    ...
    public int getUnreachablePeers() {
        ...
        return _context.peerManager()
                .getPeersByCapability(FloodfillNetworkDatabaseFacade.CAPABILITY_UNREACHABLE)
                .size();
    }
    ...
    public String getSecondKBps() {
        if (_context == null) {
            return "0 / 0";
        } else if (_context.bandwidthLimiter().getReceiveBps() < 1024 * 1024
                || _context.bandwidthLimiter().getSendBps() < 1024 * 1024) {
            return formatPair(
                    Math.round(_context.bandwidthLimiter().getReceiveBps() * 10.0 / 10.0),
                    Math.round(_context.bandwidthLimiter().getSendBps() * 10.0 / 10.0));
        }
        return formatPair(
                _context.bandwidthLimiter().getReceiveBps(),
    ...
    public String getDestinations() {
        ...
        List<Destination> clients =
                new ArrayList<Destination>(_context.clientManager().listClients());
        ...
        if (!clients.isEmpty()) {
            ...
            buf.append("><a href=\"/tunnelmanager\" target=_top title=\"")
                    ...
                    .append(" <span id=tunnelCount class=\"badge volatile\" title=\"")
                    .append(_t("How many local service tunnels we're running"))
                    .append("\">")
                    .append(clientCount)
                    .append("</span>")
                    .append("</a>")
                    .append("<input type=checkbox id=toggle_sb_localtunnels class=\"toggleSection script\" checked"
                            + " hidden></h3>\n"
                            + "<hr class=\"b\">\n")
            ...
            for (Destination client : clients) {
                ...
                Boolean isI2PChat = name.equals(_t("Messenger"))
                        || name.toLowerCase(Locale.ROOT).equals(_t("i2pchat"));
                ...
                if (isSnark) {
                    ...
                    if (!isAdvanced()) {
                        buf.append(" (")
                                .append(_t("service may be available to peers"))
                                .append(")");
                    }
                } else {
                    if (isPing) {
                    ...
                    if (!isAdvanced()) {
                        buf.append(" (")
                                .append(_t("service is only available locally"))
                                .append(")");
                    }
                }
                buf.append("\" width=16 height=16>");
                buf.append("</td><td><b><a href=\"/tunnels#")
                        .append(h.toBase64().substring(0, 4));
                ...
                if (name.length() <= 32) {
                    ...
                    buf.append(DataHelper.escapeHTML(ServletUtil.truncate(name, 29)))
                            .append("&hellip; ");
                }
                buf.append("</a></b></td>\n");
    ...
    private class AlphaComparator implements Comparator<Destination> {
        ...
        public int compare(Destination lhs, Destination rhs) {
            ...
            boolean lI2PChat = lname.equalsIgnoreCase("Messenger")
                    || lname.equalsIgnoreCase(_t("Messenger"))
                    || lname.equalsIgnoreCase("I2PChat")
                    || lname.equalsIgnoreCase(_t("I2PChat"));
            boolean rI2PChat = rname.equalsIgnoreCase("Messenger")
                    || rname.equalsIgnoreCase(_t("Messenger"))
                    || rname.equalsIgnoreCase("I2PChat")
                    || rname.equalsIgnoreCase(_t("I2PChat"));
            boolean lPing = lname.startsWith("Ping")
                    || lname.startsWith(_t("Ping"))
                    || lname.equalsIgnoreCase("I2Ping")
                    || lname.equalsIgnoreCase(_t("I2Ping"));
            boolean rPing = rname.startsWith("Ping")
                    || rname.startsWith(_t("Ping"))
                    || rname.equalsIgnoreCase("I2Ping")
                    || rname.equalsIgnoreCase(_t("I2Ping"));
    ...
    public String getMaxParticipatingTunnels() {
        int defaultMax = SystemVersion.isSlow()
                ? 2 * 1000
                : SystemVersion.getMaxMemory() < 512 * 1024 * 1024
                        ? 5 * 1000
    ...
    public String getShareRatio() {
        ...
        if (sr < 1) {
            fmt = ZERO_TWO_DECIMALS.get();
        } else if (sr < 10) {
            fmt = ZERO_ONE_DECIMAL;
        }
    ...
    public String getJobLag() {
        ...
        if (!isAdvanced() || maxLag < (double) 30) {
            ...
            if (lagRate.getAverageValue() < 1 && (double) maxLag < 1) {
                return DataHelper.formatDuration2((double) lagRate.getAverageValue()) + THINSP + (double) maxLag
                        + "&nbsp; µs";
    ...
    public String getUpdateStatus() {
        ...
        if (dver != null
            ...
            if (!_context.router().gracefulShutdownInProgress() && !NewsHelper.isUpdateInProgress()) {
                ...
                buf.append("\"><b");
                if (status.contains(_t("Unable"))) {
                    buf.append(" class=fail");
                }
                buf.append(">");
                if (source != null && source.contains("skank")) {
                    buf.append("I2P+ ");
                }
                buf.append(_t("Update downloaded"))
                        .append("<br>")
                        .append("[")
                        .append(_t("{0}", DataHelper.escapeHTML(dver)))
                        .append("]")
                        .append("</b></h4>");
            } else {
                buf.append("<h4 id=restartRequired class=\"sb_info sb_update volatile inactive\" hidden></h4>");
            }
        } else if (dver != null
                && _context.router().gracefulShutdownInProgress()
                && !NewsHelper.isUpdateInProgress()
            ...
            buf.append("<h4 id=shutdownInProgress class=\"sb_info sb_update volatile\"><b>")
                    .append(_t("Updating after restart"))
                    .append("&hellip; </b></h4>");
        }
        ...
        */
        ...
        if ((avail || unsignedAvail || devSU3Avail)
            ...
            /*
                            if (avail) {
                                buf.append("<span id=updateAvailable class=volatile>").append(_t("Release update available")).append("<br><i>") .append(_t("Version")).append(": ").append(getUpdateVersion()) .append("</i></span><br><button type=submit id=sb_downloadReleaseUpdate class=download name=updateAction value=signed>")
            ...
            */
            if (devSU3Avail) {
                buf.append("<span id=updateAvailable class=volatile>")
                        ...
                        .append("</i></span><br><button type=submit id=sb_downloadSignedDevUpdate class=download"
                                + " name=updateAction value=DevSU3>")
                        .append(_t("Download I2P Update"))
                        .append("</button><br>\n");
            }
            ...
            if (unsignedAvail) {
                ...
                buf.append("<br><i>")
                        .append(getUnsignedUpdateVersion())
                        .append("</i></span><br><button type=submit id=sb_downloadUnsignedDevUpdate class=download"
                                + " name=updateAction value=Unsigned>");
                if (source != null && source.contains("skank")) {
    ...
    public String getFirewallAndReseedStatus() {
        ...
        if (showFirewallWarning()) {
            buf.append("<h4 id=sb_warning class=volatile><span><a href=\"/help#configurationhelp\" target=_top"
                            + " title=\"")
        ...
        if (DeadlockDetector.isDeadlocked()) {
            buf.append("<div class=sb_notice><b>")
                    ...
                    .append("</a> - ")
                    .append(_t("After reporting, please restart your router"))
                    .append("</b></div>");
        }
        ...
        if (status.length() > 0) {
            ...
            buf.append("<div class=\"sb_notice volatile\" id=sb_notice><i>")
                    .append(status)
                    .append("</i></div>");
        } else {
        ...
        if (checker != null && !checker.inProgress()) {
            ...
            if (allowReseed()) {
                ...
                buf.append("<input type=hidden name=reseedNonce value=")
                        .append(nonce)
                        .append(">\n");
                buf.append("<button type=submit title=\"")
                        .append(_t("Attempt to download router reference files (if automatic reseed has failed)"));
                buf.append("\" id=sb_manualReseed class=reload value=Reseed>")
                        .append(_t("Reseed"))
                        .append("</button></form></p>\n");
            }
        }
        if (buf.length() <= 0) {
            return "";
    ...
    public List<String> getSummaryBarSections(String page) {
        ...
        if ("home".equals(page) && !unifiedSidebar()) {
            config = _context.getProperty(
                    PROP_SUMMARYBAR + page, isAdvanced() ? DEFAULT_MINIMAL_ADVANCED : DEFAULT_MINIMAL);
        } else {
            config = _context.getProperty(PROP_SUMMARYBAR + page);
            if (config == null) {
                if (version.equals(firstVersion)) {
                    config = _context.getProperty(
                            PROP_SUMMARYBAR + "default", isAdvanced() ? DEFAULT_FULL_ADVANCED : DEFAULT_FULL_NEWUSER);
                } else {
                    config = _context.getProperty(
                            PROP_SUMMARYBAR + "default", isAdvanced() ? DEFAULT_FULL_ADVANCED : DEFAULT_FULL);
                }
            }
        }
        if (config.length() <= 0) {
