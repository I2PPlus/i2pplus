...
public class ConfigTunnelsHelper extends HelperBase {
    ...
    public String getForm() {
        ...
        for (Destination dest : clients) {
            ...
            if (in != null) sorted.add(in);
        }
        if (sorted.size() > 1) DataHelper.sort(sorted, new TPComparator());
        for (TunnelPoolSettings in : sorted) {
            buf.append("<input type=hidden name=\"pool.").append(cur).append("\" value=\"").append(in.getDestination().toBase64()).append("\">\n");
            cur++;
        }
        ...
        for (TunnelPoolSettings in : sorted) {
            ...
            if (in.getAliasOf() != null || out == null || out.getAliasOf() != null) {
                cur++;
                continue;
            }
    ...
    private void renderForm(StringBuilder buf, int index, String prefix, String name, TunnelPoolSettings in, TunnelPoolSettings out) {
        ...
        buf.append("<thead><tr id=").append(prefix).append(">").append("<th colspan=3 class=\"th_title left\">").append("<span class=tunnelName>").append(name).append("</span> ").append("<span class=tIn hidden>").append(in.getTotalQuantity()).append(" / ").append(in.getLength()).append("</span> ").append("<span class=tOut hidden>").append(out.getTotalQuantity()).append(" / ").append(out.getLength()).append("</span> ").append("</th>").append("<th class=\"th_title right\">").append("<a href=/tunnels#").append(prefix).append("><span class=b32>").append(prefix).append("</span></a></th>").append("</tr></thead>\n");
        if (in.getLength() <= 0 || in.getLength() + in.getLengthVariance() <= 0 || out.getLength() <= 0 || out.getLength() + out.getLengthVariance() <= 0) {
            buf.append("<tbody><tr class=tunnelwarn><th colspan=4>").append(_t("ANONYMITY WARNING - Settings include 0-hop tunnels.")).append("</th></tr>\n");
            if (TransportUtil.getIPv6Config(_context, "SSU") == TransportUtil.IPv6Config.IPV6_ONLY) {
                buf.append("<tr class=tunnelwarn><th colspan=4>").append(_t("WARNING - 0-hop tunnels not recommended for IPv6-only routers.")).append("</th></tr>\n");
            }
            if ((in.getLength() <= 0 || in.getLength() + in.getLengthVariance() <= 0) && _context.router().isHidden()) {
                buf.append("<tr class=tunnelwarn><th colspan=4>").append(_t("WARNING - Inbound 0-hop tunnels not recommended for hidden routers.")).append("</th></tr>\n");
            }
        } else if (in.getLength() <= 1 || in.getLength() + in.getLengthVariance() <= 1 || out.getLength() <= 1 || out.getLength() + out.getLengthVariance() <= 1) {
            buf.append("<tr class=tunnelwarn><th colspan=4>").append(_t("ANONYMITY WARNING - Settings include 1-hop tunnels.")).append("</th></tr>\n");
        }
        if (in.getLength() + Math.abs(in.getLengthVariance()) >= WARN_LENGTH || out.getLength() + Math.abs(out.getLengthVariance()) >= WARN_LENGTH) buf.append("<tr class=tunnelwarn><th colspan=4>").append(_t("PERFORMANCE WARNING - Settings include very long tunnels.")).append("</th></tr>\n");
        if (in.getTotalQuantity() >= WARN_QUANTITY || out.getTotalQuantity() >= WARN_QUANTITY) buf.append("<tr class=tunnelwarn><th colspan=4>").append(_t("PERFORMANCE WARNING - Settings include high tunnel quantities.")).append("</th></tr>\n");

        buf.append("<tr class=heading><th></th><th class=inbound>").append(_t("Inbound")).append("</th><th class=outbound>").append(_t("Outbound")).append("</th><th class=spacer></th>\n</tr>\n");
        ...
        if (!advanced && !"exploratory".equals(prefix)) {
            buf.append(" disabled");
        }
        buf.append(">\n");
        ...
        if (!props.isEmpty()) {
            buf.append("<tr class=options><td><b>").append(_t("Inbound options")).append(":</b></td>\n").append("<td colspan=2><input name=\"").append(index).append(".inboundOptions\" type=text size=32 disabled ").append("value=\"");
        ...
        if (!props.isEmpty()) {
            buf.append("<tr class=options><td><b>").append(_t("Outbound options")).append(":</b></td>\n").append("<td colspan=2><input name=\"").append(index).append(".outboundOptions\" type=text size=32 disabled ").append("value=\"");
            for (String prop : props.stringPropertyNames()) {
                String val = props.getProperty(prop);
                buf.append(prop).append('=').append(val).append(' ');
            }
    ...
    private void renderOptions(StringBuilder buf, int min, int max, int now, String prefix, String name) {
        for (int i = min; i <= max; i++) {
            ...
            buf.append(">").append(ngettext(DUMMY2 + name, DUMMY2 + name + 's', i)).append("</option>\n");
        }
    }
