...
public class ShellService implements ClientApp {
    ...
    public ShellService(I2PAppContext context, ClientAppManager listener, String[] args) {
        ...
        if (!exe.exists()) {
            if (_log.shouldError()) _log.error("ShellService: Command does not exist: " + _commandPath);
            throw new RuntimeException("Command does not exist: " + _commandPath);
        }
        if (!exe.canExecute()) {
            if (_log.shouldWarn()) _log.warn("ShellService: Command is not executable: " + _commandPath + " marking it executable");
            exe.setExecutable(true);
        }

        _pb = new ProcessBuilder(procArgs);

        if (_log.shouldDebug()) _log.debug("ShellService: ProcessBuilder: " + _pb.command().toString() + " is built");
        ...
        if (!pluginDir.exists()) {
            ...
            if (!pluginDir.exists()) {
                ...
                if (!pluginDir.exists()) {
                    ...
                    if (_log.shouldDebug()) _log.debug("ShellService: Plugin name revised to match directory: " + this.getName());
                }
            } else {
        ...
        if (_log.shouldDebug()) _log.debug("ShellService: Logs: " + _errorLog.getAbsolutePath() + ", " + _outputLog.getAbsolutePath());

        _pb.directory(pluginDir);
        if (_log.shouldDebug()) _log.debug("ShellService: ProcessBuilder: " + _pb.directory() + " is set");
    ...
    private ArrayList<String> trimArgs(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i].startsWith(NAME_OPTION)) {
                ...
                if (_log.shouldDebug()) _log.debug("Adding arg: " + args[i]);
                newargs.add(args[i]);
            }
        }
        if (getName() == null) throw new IllegalArgumentException("ShellService: ShellService passed with args=" + Arrays.toString(args) + " must have a name");
        if (getDisplayName() == null) displayName = name;
        return newargs;
    }
    ...
    public synchronized void startup() throws Throwable {
        ...
        if (getName().equals("unnamedClient")) {
            if (_log.shouldWarn()) _log.warn("ShellService: ShellService has no name, not starting");
            return;
        }
        ...
        if (start) {
            _p = _pb.start();
            if (!_p.isAlive() && _log.shouldError()) _log.error("ShellService: Error getting Process of application from recently instantiated shellservice " + _pb.command() + " " + _p.exitValue());
            if (_log.shouldDebug()) _log.debug("ShellService: Started " + getName() + "process");
        }
        if (_p.isAlive()) changeState(ClientAppState.RUNNING, "ShellService: " + getName() + " started");
        Boolean reg = _cmgr.register(this);
        if (reg) {
            if (_log.shouldDebug()) _log.debug("ShellService: " + getName() + " registered with the router");
        } else {
            if (_log.shouldWarn()) _log.warn("ShellService: " + getName() + " failed to register with the router");
    ...
    public boolean isProcessRunning() {
        ...
        if (_log.shouldDebug()) _log.debug("ShellService: Checking process status " + getName() + _p.isAlive());
        return _p.isAlive();
    }
    ...
    public synchronized void shutdown(String[] args) throws Throwable {
        if (getName().equals("unnamedClient")) {
            if (_log.shouldWarn()) _log.warn("ShellService: ShellService has no name, not shutting down");
            return;
        }
        changeState(ClientAppState.STOPPING, "ShellService: " + getName() + " stopping");
        if (_p != null) {
            if (_log.shouldDebug()) _log.debug("ShellService: Stopping " + getName() + "process started with ShellService " + getName() + "...");
            _p.destroy();
        }
    ...
    public ClientAppState getState() {
        if (!isProcessRunning()) {
            if (_log.shouldDebug()) _log.debug("ShellService: Process is not running " + getName());
            changeState(ClientAppState.STOPPED, "ShellService: " + getName() + " stopped");
            _cmgr.unregister(this);
        }
        return _state;
