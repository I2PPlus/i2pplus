...
public class PeerHelper extends HelperBase {
    ...
    private Comparator<NTCPConnection> getNTCPComparator(int sortFlags) {
        Comparator<NTCPConnection> rv = AlphaComparator.instance();
    ...
    private void render(UDPTransport ut, Writer out, String urlBase, int sortFlags, boolean debugmode) throws IOException {
        ...
        peers.addAll(ut.getPeers());
        StringBuilder buf = new StringBuilder(4*1024);
        renderSSUHeader(buf, urlBase, sortFlags, debugmode, ut.countActivePeers(), ut.getMaxConnections(), !peers.isEmpty());
        out.append(buf);
        buf.setLength(0);
        long now = _context.clock().now();
        SSUTotals totals = new SSUTotals();
    ...
    private void renderSSURow(StringBuilder buf, PeerState peer, long now, boolean debugmode, SSUTotals totals) {
        ...
        if (debugmode) {
            ...
            if (_context.banlist().isBanlisted(h, "SSU")) {
                if (!appended) {buf.append("<br>");}
                buf.append(" <i>")
                   .append(_t("Banned"))
                   .append("</i>");
        ...
        buf.append("<td class=edit><a class=configpeer href=\"/configpeer?peer=")
           ...
           .append(_t("Edit"))
           .append("</a></td></tr>\n");
        totals.bpsIn += recvBps;
        totals.bpsOut += sendBps;
        totals.uptimeMsTotal += uptime;
        totals.skewTotal += skew;
        totals.cwinTotal += sendWindow;
        if (rtt > 0) {
            totals.rttTotal += rtt;
            totals.numRTTPeers++;
        }
        totals.rtoTotal += rto;
        totals.sendTotal += sent;
        totals.recvTotal += recv;
        totals.resentTotal += resent;
        totals.dupRecvTotal += dupRecv;
        totals.numPeers++;
    }
    ...
    private void renderSSUFooter(StringBuilder buf, boolean debugmode, UDPTransport ut, SSUTotals totals) {
        ...
        String bwout = formatKBps(totals.bpsOut).replace(".00", "");
        buf.append("<tfoot><tr class=tablefooter><td class=peer colspan=");
        ...
        buf.append("><b>")
           ...
           .append(DataHelper.formatDuration2(totals.uptimeMsTotal/totals.numPeers))
           .append("</b></td><td class=skew><b>")
           .append(DataHelper.formatDuration2(totals.skewTotal/totals.numPeers))
           .append("</b></td>\n<td class=tx><b>")
           .append(totals.sendTotal)
           .append("</b></td><td class=rx><b>")
           .append(totals.recvTotal)
           .append("</b></td>\n<td class=duptx><b>")
           .append(totals.resentTotal)
           .append("</b></td><td class=duprx><b>")
           .append(totals.dupRecvTotal)
           .append("</b></td>");
        if (debugmode) {
            buf.append("<td class=cwnd><b>")
               .append(totals.cwinTotal/((long) totals.numPeers * 1024))
               .append("K</b></td><td class=sst>&nbsp;</td><td class=rtt><b>");
            if (totals.numRTTPeers > 0) {
                buf.append(DataHelper.formatDuration2(totals.rttTotal/totals.numRTTPeers));
            } else {buf.append("n/a");}
            buf.append("</b></td><td class=rto><b>")
               .append(DataHelper.formatDuration2(totals.rtoTotal/totals.numPeers))
