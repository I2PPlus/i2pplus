...
public class PeerHelper extends HelperBase {
    ...
    private static final String titles[] = {_x("Summary"), "NTCP", "SSU"
        // "SSU (Advanced)"
    };

    private static final String links[] = {"", "?transport=ntcp", "?transport=ssu", "?transport=ssudebug"};
    ...
    private void renderStatusHTML(Writer out, String urlBase, int sortFlags) throws IOException {
        ...
        if (_transport != null) {
            ...
            buf.append("<p class=infohelp>").append(_t("Your transport connection limits are automatically set based on your configured bandwidth.")).append(" ").append(_t("To override these limits, add the settings {0} and {1}", "<code>i2np.ntcp.maxConnections=nnn</code>", "<code>i2np.udp.maxConnections=nnn</code>")).append(" ");
            if (isAdvanced()) {
    ...
    private void renderSummary(Writer out) throws IOException {
        ...
        boolean showIPv4 = connected.contains(AddressType.IPV4) && (ntcpConfig != TransportUtil.IPv6Config.IPV6_ONLY || ssuConfig != TransportUtil.IPv6Config.IPV6_ONLY);
        boolean showIPv6 = connected.contains(AddressType.IPV6) && (ntcpConfig != TransportUtil.IPv6Config.IPV6_DISABLED || ssuConfig != TransportUtil.IPv6Config.IPV6_DISABLED);

        StringBuilder buf = new StringBuilder(6 * 1024);
        buf.append("<h3 id=transports>").append(_t("Peer Connections")).append("</h3>\n").append("<table id=transportSummary>\n<thead><tr>").append("<th>").append(_t("Transport")).append("</th>");

        if (showIPv4) {
            buf.append("<th class=\"ipv4 in\">").append(_t("IPv4")).append("&nbsp; <span>").append(_t("Inbound")).append("</span></th>").append("<th class=\"ipv4 out\">").append(_t("IPv4")).append("&nbsp; <span>").append(_t("Outbound")).append("</span></th>");
        }
        if (showIPv6) {
            buf.append("<th class=\"ipv6 in\">").append(_t("IPv6")).append("&nbsp; <span>").append(_t("Inbound")).append("</span></th>").append("<th class=\"ipv6 out\">").append(_t("IPv6")).append("&nbsp; <span>").append(_t("Outbound")).append("</span></th>");
        }

        buf.append("<th title=\"").append(_t("Active connections / Max permitted")).append("\">").append(_t("Connections / Limit")).append("</th>").append("</tr></thead>\n<tbody>\n");
        ...
        for (Map.Entry<String, Transport> e : transports.entrySet()) {
            ...
            for (int idx = 0; idx < 8; idx += 4) {
                ...
                buf.append("<td><span class=\"percentBarOuter\">").append("<span class=\"percentBarInner\" style=\"width:").append(percent).append("%\">").append("<span class=\"percentBarText\">").append(total).append(" / ").append(limit).append("</span>").append("</span></span></td></tr>\n");
            }
        }
    ...
    private void renderNavBar(Writer out) throws IOException {
        ...
        for (int i = 0; i < titles.length; i++) {
            if (i == tab) {
                ...
                buf.append("<span class=tab>").append("<a href=\"peers").append(links[i]).append("\">").append(_t(titles[i])).append("</a>");
            }
            buf.append("</span>\n");
        }
        buf.append("</div>");
    ...
    private void render(NTCPTransport nt, Writer out, String urlBase, int sortFlags) throws IOException {
        boolean IPv6Enabled = _context.getBooleanProperty("i2np.ntcp.ipv6") || _context.getBooleanProperty("i2np.udp.ipv6");
        ...
        buf.append("<div id=ntcp>\n<h3 id=ntcpcon title=\"").append(_t("Current / maximum permitted")).append("\">").append(_t("NTCP connections")).append(":&nbsp; ").append(nt.countActivePeers()).append(" / ").append(nt.getMaxConnections()).append("<span id=topCount hidden></span></h3>\n<div class=widescroll>\n<table id=ntcpconnections class=cells>\n");
        if (peers.size() != 0) {
            buf.append("<thead><tr><th class=peer>").append(_t("Peer")).append("</th><th class=caps title=\"").append(_t("Peer capabilities")).append("\">").append(_t("Caps")).append("</th><th class=direction title=\"").append(_t("Direction/Introduction")).append("\">").append(_t("Dir")).append("</th>");
            if (IPv6Enabled) {
                buf.append("<th class=ipv6>").append(_t("IPv6")).append("</th>");
            }
            buf.append("<th class=idle title=\"") .append(_t("Peer inactivity")) .append("\">")
                    .append(_t("Idle"))
        ...
        for (NTCPConnection con : peers) {
            ...
            buf.append("<tr class=lazy><td class=peer data-sort-direction=ascending>").append(_context.commSystem().renderPeerHTML(h, false)).append("</td><td class=caps>").append(_context.commSystem().renderPeerCaps(h, false)).append("</td><td class=direction data-sort=").append(isInbound ? "in" : "out").append(">");
            if (isInbound) {
            ...
            buf.append("<td class=idle><span class=right data-sort=").append(con.getTimeSinceReceive(now)).append(">").append(DataHelper.formatDuration2(con.getTimeSinceReceive(now))).append("</span>").append(THINSP).append("<span class=left>").append(DataHelper.formatDuration2(con.getTimeSinceSend(now))).append("</span></td>");
            ...
            if (con.getRecvRate() >= 0.01 || con.getSendRate() >= 0.01) {
                ...
                if ((peers.size() >= 300 && con.getTimeSinceReceive(now) <= 60 * 1000) || (peers.size() >= 100 && con.getTimeSinceReceive(now) <= 3 * 60 * 1000) || con.getTimeSinceReceive(now) <= 10 * 60 * 1000) {
                ...
                if ((peers.size() >= 300 && con.getTimeSinceSend(now) <= 60 * 1000) || (peers.size() >= 100 && con.getTimeSinceSend(now) <= 3 * 60 * 1000) || con.getTimeSinceSend(now) <= 10 * 60 * 1000) {
            ...
            buf.append("</td><td class=uptime data-sort=").append(con.getUptime()).append("><span>").append(DataHelper.formatDuration2(con.getUptime())).append("</span></td><td class=skew data-sort=").append(con.getClockSkew()).append(">");
            if (con.getClockSkew() > 0) {
                buf.append("<span>").append(DataHelper.formatDuration2(con.getClockSkew())).append("</span>");
            }
            totalSend += con.getMessagesSent();
            totalRecv += con.getMessagesReceived();
            long outQueue = con.getOutboundQueueSize();
            buf.append("</td><td class=tx><span>").append(con.getMessagesSent()).append("</span></td><td class=rx><span>").append(con.getMessagesReceived()).append("</span></td><td class=queue>");
            if (outQueue > 0) {
            ...
            buf.append("<td class=edit><a class=configpeer href=\"/configpeer?peer=").append(h.toBase64()).append("\" title=\"").append(_t("Configure peer")).append("\" alt=\"[").append(_t("Configure peer")).append("]\">").append(_t("Edit")).append("</a></td></tr>\n");
            out.append(buf);
            buf.setLength(0);
        }
        buf.append("</tbody>");
        ...
        if (!peers.isEmpty()) {
            ...
            buf.append("<tfoot><tr class=tablefooter><td class=peer colspan=").append(IPv6Enabled ? "5" : "4").append("><b>").append(ngettext("{0} peer", "{0} peers", nt.countActivePeers())).append("</b></td><td class=inout nowrap><span class=right><b>").append(rx).append("</b></span>").append(THINSP).append("<span class=left><b>").append(tx).append("</b></span></td><td class=uptime><span><b>").append(DataHelper.formatDuration2(totalUptime / peers.size())).append("</b></span></td><td class=skew><span><b>").append(DataHelper.formatDuration2(offsetTotal * 1000 / peers.size())).append("</b></span></td><td class=tx><span><b>").append(totalSend).append("</b></span></td><td class=rx><span><b>").append(totalRecv).append("</b></span></td><td colspan=2></td></tr>\n");
        }
    ...
    private Comparator<NTCPConnection> getNTCPComparator(int sortFlags) {
        ...
        switch (Math.abs(sortFlags)) {
            default: rv = AlphaComparator.instance();
        }
        if (sortFlags < 0) {
    ...
    private void render(UDPTransport ut, Writer out, String urlBase, int sortFlags, boolean debugmode) throws IOException {
        ...
        StringBuilder buf = new StringBuilder(4 * 1024);
        buf.append("<div id=udp>\n<h3 id=udpcon title=\"").append(_t("Current / maximum permitted")).append("\">").append(_t("UDP connections")).append(":&nbsp; ").append(ut.countActivePeers()).append(" / ").append(ut.getMaxConnections());
        if (!debugmode) {
        ...
        if (peers.size() != 0) {
            ...
            buf.append("</th><th class=caps>").append(_t("Caps")).append("</th><th class=direction title=\"").append(_t("Direction/Introduction")).append("\">").append(_t("Dir"));
            if (debugmode) {
                buf.append("</th><th class=ipv6>").append(_t("IPv6"));
            }
            buf.append("</th><th class=idle title=\"").append(_t("Peer inactivity")).append("\" data-sort-method=number>").append(_t("Idle"));
            if (debugmode) {
                buf.append("<br>");
            ...
            buf.append("</th><th class=inout title=\"").append(_t("Average inbound/outbound rate (KBps)")).append("\" data-sort-method=number>").append(_t("In / Out"));
            if (debugmode) {
                buf.append("<br>");
            ...
            buf.append("</th><th class=uptime title=\"").append(_t("Duration of connection to peer")).append("\" data-sort-method=number><div class=peersort>").append(_t("Up"));
            if (debugmode) {
                buf.append("<br>");
                appendSortLinks(buf, urlBase, sortFlags, _t("Sort by connection uptime"), FLAG_UPTIME);
            }
            buf.append("</div></th><th class=skew title=\"").append(_t("Peer's clockskew relative to our clock")).append("\" data-sort-method=number><div class=peersort>").append(_t("Skew"));
            if (debugmode) {
                buf.append("<br>");
                appendSortLinks(buf, urlBase, sortFlags, _t("Sort by clock skew"), FLAG_SKEW);
            }
            buf.append("</div></th><th class=tx title=\"").append(_t("Messages sent")).append("\"><div class=peersort>").append(_t("TX"));
            if (debugmode) {
                buf.append("<br>");
                appendSortLinks(buf, urlBase, sortFlags, _t("Sort by packets sent"), FLAG_SEND);
            }
            buf.append("</div></th><th class=rx title=\"").append(_t("Messages received")).append("\"><div class=peersort>").append(_t("RX"));
            if (debugmode) {
                buf.append("<br>");
                appendSortLinks(buf, urlBase, sortFlags, _t("Sort by packets received"), FLAG_RECV);
            }
            buf.append("</div></th><th class=duptx title=\"").append(_t("Retransmitted packets")).append("\"><div class=peersort>").append(_t("Dup TX"));
            if (debugmode) {
                buf.append("<br>");
                appendSortLinks(buf, urlBase, sortFlags, _t("Sort by packets retransmitted"), FLAG_RESEND);
            }
            buf.append("</div></th><th class=duprx title=\"").append(_t("Received duplicate packets")).append("\"><div class=peersort>").append(_t("Dup RX"));
            if (debugmode) {
                buf.append("<br>");
            ...
            if (debugmode) {
                ...
                buf.append("</th><th class=sst title=\"").append(_t("Slow start threshold")).append("\"><div class=peersort>SST<br>");
                appendSortLinks(buf, urlBase, sortFlags, _t("Sort by slow start threshold"), FLAG_SSTHRESH);
                buf.append("</div></th>\n<th class=rtt title=\"").append(_t("Round trip time")).append("\"><div class=peersort>RTT<br>");
                appendSortLinks(buf, urlBase, sortFlags, _t("Sort by round trip time"), FLAG_RTT);
                buf.append("</div></th><th class=rto title=\"").append(_t("Retransmission timeout")).append("\"><div class=peersort>RTO<br>");
                appendSortLinks(buf, urlBase, sortFlags, _t("Sort by retransmission timeout"), FLAG_RTO);
                buf.append("</div></th>\n<th class=mtu title=\"").append(_t("Maximum transmission unit")).append("\">MTU<br>");
        ...
        for (PeerState peer : peers) {
            if (peers.size() >= 300 && now - peer.getLastReceiveTime() > 60 * 1000) {
                continue;
            } // don't include old peers
            if (peers.size() >= 100 && now - peer.getLastReceiveTime() > 3 * 60 * 1000) {
                continue;
            } else if (now - peer.getLastReceiveTime() > 10 * 60 * 1000) {
                continue;
            }
            ...
            buf.append("</td><td class=caps>").append(_context.commSystem().renderPeerCaps(peer.getRemotePeer(), false)).append("</td><td class=direction nowrap data-sort=").append(isInbound ? "in" : "out").append(introToUs ? "IntroToUs" : introToThem ? "IntroToThem" : "").append(">");
            if (isInbound) {
                buf.append("<span class=inbound title=\"").append(_t("Inbound"));
            } else {
                buf.append("<span class=outbound title=\"").append(_t("Outbound"));
            }
            buf.append("\"></span>");
            if (introToThem) {
                buf.append("&nbsp; &nbsp; <span class=\"inbound small\" title=\"").append(_t("We offered to introduce them")).append("\">");
            }
            if (introToUs) {
                buf.append("&nbsp; &nbsp; <span class=\"outbound small\" title=\"").append(_t("They offered to introduce us")).append("\">");
            }
            if (introToUs || introToThem) {
                buf.append("</span>");
            }
            if (debugmode) {
            ...
            buf.append("<td class=idle data-sort=").append(idleIn).append("><span class=right>").append(DataHelper.formatDuration2(idleIn)).append("</span>").append(THINSP).append("<span class=left>").append(DataHelper.formatDuration2(idleOut)).append("</span></td><td class=inout nowrap>");
            if (recvBps > 0 || sendBps > 0) {
                buf.append("<span class=right>").append(rx).append("</span>").append(THINSP).append("<span class=left>").append(tx).append("</span>");
            }
            ...
            buf.append("</td><td class=uptime data-sort=").append(uptime).append("\">").append(DataHelper.formatDuration2(uptime)).append("</td><td class=skew data-sort=").append(skew).append("\">").append(DataHelper.formatDuration2(skew)).append("</td><td class=tx><span class=right>").append(sent).append("</span></td><td class=rx><span class=right>").append(recv).append("</span></td><td class=duptx>");
            if (resent > 0) {
            ...
            int rto = peer.getRTO();
            if (debugmode) {
                buf.append("<td class=cwnd><span class=right>").append(sendWindow / 1024).append("K</span>").append(THINSP).append("<span class=right>").append(peer.getConcurrentSends()).append("</span>").append(THINSP).append("<span class=right>").append(peer.getConcurrentSendWindow()).append("</span>").append(THINSP).append("<span class=left>").append(peer.getConsecutiveSendRejections()).append("</span>");
                if (peer.isBacklogged()) {
                    buf.append("<br><span class=peerBacklogged>").append(_t("backlogged")).append("</span>");
                }

                buf.append("</td><td class=sst>").append(peer.getSlowStartThreshold() / 1024).append("K</td><td class=rtt>");
                if (rtt > 0) {
                    buf.append(DataHelper.formatDuration2(rtt));
                } else {
                    buf.append("n/a");
                }
                buf.append("</td><td class=rto>").append(DataHelper.formatDuration2(rto)).append("</td><td class=mtu><span class=right>").append(peer.getMTU()).append("</span>").append(THINSP).append("<span class=left>").append(peer.getReceiveMTU()).append("</span></td>");
            }
            buf.append("<td class=edit><a class=configpeer href=\"/configpeer?peer=").append(h.toBase64()).append("\" title=\"").append(_t("Configure peer")).append("\" alt=\"[").append(_t("Configure peer")).append("]\">").append(_t("Edit")).append("</a></td></tr>\n");
            out.append(buf);
            buf.setLength(0);
        ...
        if (numPeers > 0) {
            ...
            buf.append("><b>").append(ngettext("{0} peer", "{0} peers", ut.countActivePeers())).append("</b></td><td class=inout nowrap><span class=right><b>").append(bwin).append("</b></span>").append(THINSP).append("<span class=left><b>").append(bwout).append("</b></span></td><td class=uptime><b>").append(DataHelper.formatDuration2(x));
            x = offsetTotal / numPeers;
            buf.append("</b></td><td class=skew><b>").append(DataHelper.formatDuration2(x)).append("</b></td>\n<td class=tx><b>").append(sendTotal).append("</b></td><td class=rx><b>").append(recvTotal).append("</b></td>\n<td class=duptx><b>").append(resentTotal).append("</b></td><td class=duprx><b>").append(dupRecvTotal).append("</b></td>");
            if (debugmode) {
                buf.append("<td class=cwnd><b>").append(cwinTotal / (numPeers * 1024L)).append("K</b></td><td class=sst>&nbsp; </td><td class=rtt><b>");
                if (numRTTPeers > 0) {
                    buf.append(DataHelper.formatDuration2(rttTotal / numRTTPeers));
                } else {
                    buf.append("n/a");
                }
                buf.append("</b></td><td class=rto><b>").append(DataHelper.formatDuration2(rtoTotal / numPeers)).append("</b></td><td class=mtu><b>").append(ut.getMTU(false)).append("</b></td>");
            }
    ...
    private final String getTransportsLegend() {
        StringBuilder buf = new StringBuilder(1024);
        buf.append("<h3 class=tabletitle>") .append(_t("Definitions")) .append("</h3><table id=peerdefs>\n<tr><td>\n<ul><li><b id=def.peer>")
                .append(_t("Peer"))
                .append(":</b> ")
                ...
                .append("</li>\n");
        if (isAdvanced()) {
            buf.append("<li><b id=def.cwnd>CWND (") .append(_t("Congestion window")) .append("):</b></br>&nbsp; &nbsp; &bullet; ")
        ...
        buf.append("<li><b id=def.send>").append(_t("TX")).append(":</b> ").append(_t("Messages sent to peer")).append("</li>\n<li><b id=def.recv>").append(_t("RX")).append(":</b> ").append(_t("Messages received from peer")).append("</li><li><b id=def.resent>").append(_t("Dup TX")).append(":</b> ").append(_t("Packets retransmitted to peer")).append("</li>\n<li><b id=def.dupRecv>").append(_t("Dup RX")).append(":</b> ").append(_t("Duplicate packets received from peer")).append("</li>\n</ul></td></tr></table>");
        return buf.toString();
    }
