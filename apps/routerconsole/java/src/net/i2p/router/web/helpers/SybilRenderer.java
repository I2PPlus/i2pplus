...
public class SybilRenderer {
    ...
    private void renderRouterInfoHTML(Writer out, String nonce, int mode, long date) throws IOException {
        ...
        buf.append( "<li id=floodfillSummary><a href=\"netdb?f=3&amp;m=1\">" + _t("Floodfill Summary")
                        + "</a></li>\n"
                        + "<li id=sameFamily><a href=\"netdb?f=3&amp;m=2\">"
                        + _t("Same Family")
                        + "</a></li>\n"
                        + "<li><a href=\"netdb?f=3&amp;m=3\">"
                        + _t("IP close to us")
                        + "</a></li>\n"
                        +
                        /**
                         ...
                         **/
                        "<li><a href=\"netdb?f=3&amp;m=7\">"
                        + _t("Pair distance")
                        + "</a></li>\n"
                        + "<li><a href=\"netdb?f=3&amp;m=8\">"
                        + _t("Close to us")
                        + "</a></li>\n"
                        + "<li><a href=\"netdb?f=3&amp;m=9\">"
                        + _t("Close to us tomorrow")
                        + "</a></li>\n"
                        + "<li><a href=\"netdb?f=3&amp;m=10\">"
                        + _t("DHT neighbors")
                        + "</a></li>\n"
                        + "<li><a href=\"netdb?f=3&amp;m=11\">"
                        + _t("Close to our destinations")
                        + "</a></li>\n"
                        + "</ul>\n</div>\n");
        writeBuf(out, buf);
        ...
        if (mode == 0) {
            ...
            try {
                ...
                out.write("<b>Failed to load analysis for " + DataHelper.formatTime(date).replace("-", " ") + "</b>: " + DataHelper.escapeHTML(ioe.toString()));
                return;
            }
            if (points.isEmpty()) {
    ...
    private void renderOverview(Writer out, StringBuilder buf, String nonce, Analysis analysis) throws IOException {
        ...
        out.write(_t("This is an experimental network database tool for debugging and analysis. " + "Do not panic even if you see warnings indicated. " + "Possible \"threats\" are summarized, however these are unlikely to be real threats. " + "If you see anything you would like to discuss with the devs, contact us on IRC #i2p-dev."));
        out.write("</b></p>\n");
        out.write("<div class=infohelp id=sybilanalysis>\n");
        if (dates.isEmpty()) {
            out.write("<b><i>");
            ...
            buf.append("</select>\n<input type=submit name=action class=go value=").append(_t("Review")).append(">\n</form>\n</div>\n");
        }
        writeBuf(out, buf);
    }
    ...
    private static void renderRunForm(Writer out, StringBuilder buf, String nonce) throws IOException {
        buf.append("<form class=sybilScan action=/netdb method=POST>\n<input type=hidden name=f value=3>\n" + "<input type=hidden name=m value=13>\n<input type=hidden name=nonce value=\"").append(nonce).append("\">\n<table>\n<tr><td><b>").append(_x("Run analysis on Floodfills only")).append(":</b></td><td><input type=submit name=action class=go value=\"").append(_x("Start Scan")).append("\"></td></tr>\n</table>\n</form>\n<hr>\n");
        buf.append("<form class=sybilScan action=/netdb method=POST>\n" + "<input type=hidden name=f value=3>\n" + "<input type=hidden name=m value=\"16\">\n" + "<input type=hidden name=nonce value=\"").append(nonce).append("\">\n<table><tr><td><b>").append(_x("Run analysis on all routers in NetDb")).append(":</b></td><td><input type=submit name=action class=go value=\"").append(_x("Start Scan")).append("\"></td></tr>\n</table>\n</form>\n");
        writeBuf(out, buf);
    }
    ...
    private void renderBackgroundForm(Writer out, StringBuilder buf, String nonce) throws IOException {
        ...
        buf.append("<form action=\"netdb\" method=POST>\n" + "<input type=hidden name=f value=3>\n" + "<input type=hidden name=m value=\"15\">\n" + "<input type=hidden name=nonce value=\"").append(nonce).append("\">\n<p class=sybilinfo><b>").append(_t("The sybil detection routine can be configured to scan the Network Database on a regular interval, " + "with the option to block peers meeting the configured threshold. " + "Blocking peers prevents your router from building tunnels with them for the duration configured, or until the router is restarted.")).append("</b></p>\n<table id=sybilTask>\n<tr><th>").append(_t("Configure Background Analysis")).append("</th></tr>\n<tr><td>\n<div class=optionlist>\n<span class=nowrap><b>").append(_t("Run task every")).append(":</b>\n" + "<select name=\"runFrequency\">\n");
        for (int i = 0; i < HOURS.length; i++) {
            buf.append("<option value=\"");
            buf.append(HOURS[i]);
            buf.append('"');
            long time = HOURS[i] * 60L * 60 * 1000L;
            if (time == freq) buf.append(HelperBase.SELECTED);
            buf.append('>');
            if (HOURS[i] > 0) buf.append(DataHelper.formatDuration2(time));
        ...
        buf.append("</select>\n").append("</span><br>\n<span class=nowrap><b>").append(_t("Delete scans older than")).append(":</b>\n<select name=\"deleteAge\">\n");
        ...
        for (int i = 0; i < DAYS.length; i++) {
            ...
            if (time == age) buf.append(HelperBase.SELECTED);
            buf.append('>');
            if (DAYS[i] > 0) buf.append(DataHelper.formatDuration2(time).replace("-", " "));
            else buf.append(_t("Never"));
            buf.append("</option>\n");
        }
        buf.append("</select>\n</span><br>\n<span class=nowrap><b>").append(_t("Automatic blocking")).append(":</b><label><input type=checkbox class=\"optbox slider\" value=1 name=\"block\" ");
        if (auto) buf.append(HelperBase.CHECKED);
        buf.append(">").append(_t("Add detected sybils to banlist")).append("</label>").append("</span><br>\n<span class=nowrap><b>").append(_t("Block all detected sybils")).append(":</b><label><input type=checkbox class=\"optbox slider\" value=1 name=\"nonff\" ");
        if (nonff) buf.append(HelperBase.CHECKED);
        buf.append(">").append(_t("Include non-floodfill routers")).append("</label>").append("</span><br>\n<span class=nowrap><b>").append(_t("Minimum threshold for block")).append(":</b><input type=text name=\"threshold\" pattern=\"^[0-9]{1,4}\\.[0-9]{0,1}$\" value=\"").append(thresh).append("\">").append(_t("threat points")).append("</span><br>\n<span class=nowrap><b>").append(_t("Enforce block for")).append(":</b><input type=text name=\"days\" pattern=\"^[0-9]{1,4}$\" value=\"").append(days).append("\">").append(_t("days")).append("</span><br>\n</td></tr>\n").append("<tr><td class=optionsave><input type=submit name=action class=accept value=").append(_t("Save")).append(">\n</div>\n</td></tr>\n</table>\n</form>\n");
        writeBuf(out, buf);
    }
    ...
    private void renderDestSummary(Writer out, StringBuilder buf, Analysis analysis, double avgMinDist, List<RouterInfo> ris, Map<Hash, Points> points) throws IOException {
        ...
        for (Iterator<Hash> iter = destinations.iterator(); iter.hasNext(); ) {
            ...
            if (!_context.clientManager().isLocal(client) || !_context.clientManager().shouldPublishLeaseSet(client) || _context.netDb().lookupLeaseSetLocally(client) == null) {
                iter.remove();
            }
        }
        ...
        for (Hash client : destinations) {
            ...
            buf.append("<h3 class=sybils>").append(_t("Closest floodfills to the Routing Key for")).append(" ").append(name).append(" (").append(_t("where we store our LS")).append(")</h3>");
            ...
            buf.append("<h3 class=sybils>").append(_t("Closest floodfills to Tomorrow's Routing Key for")).append(" ").append(name).append(" (").append(_t("where we will store our LS")).append(")</h3>");
            buf.append("<p class=sybil_info><a href=\"/netdb?caps=f&amp;sybil=").append(ls.getHash().toBase64()).append("\">See all</a></p>\n");
    ...
    private void renderThreatsHTML(Writer out, StringBuilder buf, long date, Map<Hash, Points> points) throws IOException {
        ...
        try {
            ...
            if (threshold < Analysis.MIN_BLOCK_POINTS) threshold = Analysis.MIN_BLOCK_POINTS;
        } catch (NumberFormatException nfe) {
        }
        ...
        if (!points.isEmpty()) {
            ...
            buf.append("<h3 id=threats class=sybils>").append(_t("Routers with Most Threat Points")).append("<span style=float:right>").append(DataHelper.formatTime(date).replace("-", " ")).append("</span></h3>\n");
            for (Hash h : warns) {
                Points pp = points.get(h);
                double p = pp.getPoints();
                if (p < minDisplay) break; // sorted
                buf.append("<span class=sybil_wrap>\n");
                if (p >= 100) buf.append("<p class=\"threatpoints hot\"><b>");
                else buf.append("<p class=threatpoints><b>");
                buf.append(_t("Threat Points")).append(": " + TWO_DECIMALS.get().format(p).replace(".00", "") + "</b></p>\n<ul>\n");
                List<String> reasons = pp.getReasons();
                if (reasons.size() > 1) Collections.sort(reasons, rcomp);
                ...
                if (ri != null) {
                    ...
                    buf.append("<table class=sybil_routerinfo id=hash_").append(hash, 0, 6).append(">\n<tr>" + "<th><b>" + _t("Router") + ":</b> <code>").append(hash).append("</code></th>" + "<th colspan=2><b style=float:right>" + _t("Router info not available") + "</b></th></tr>\n</table>\n");
                }
                buf.append("</span>");
            }
        }
    ...
    private void renderPairDistance(Writer out, StringBuilder buf, List<Pair> pairs, double avg) throws IOException {
        buf.append("<h3 class=sybils>").append(_t("Average Floodfill Distance is")).append(" ").append(TWO_DECIMALS.get().format(avg)).append("</h3>\n" + "<h3 id=pairs class=sybils>").append(_t("Closest Floodfill Pairs by Hash")).append("</h3>\n");

        for (Pair p : pairs) {
            double distance = biLog2(p.dist);
            double point = MIN_CLOSE - distance;
            // limit display
            if (point < 2) break; // sorted;
            buf.append("<p class=hashdist><b>").append(_t("Hash Distance")).append(": ").append(TWO_DECIMALS.get().format(distance)).append(": </b>" + "</p>\n");
     ...
     *
     */
    private void renderIPGroupsUs(Writer out, StringBuilder buf, List<RouterInfo> ri32, List<RouterInfo> ri24, List<RouterInfo> ri16, List<RouterInfo> ri64, List<RouterInfo> ri48) throws IOException {
    ...
    private void renderIPGroups32(Writer out, StringBuilder buf, Map<Integer, List<RouterInfo>> map) throws IOException {
        ...
        for (Integer ii : foo) {
            ...
            buf.append("<p class=sybil_info><b>").append(count).append(" ").append(_t("routers with IP")).append(" <a href=\"/netdb?ip=").append(sip).append("&amp;sybil\">").append(sip).append("</a>:</b></p>\n");
            for (RouterInfo info : ris) {
                found = true;
                renderRouterInfo(buf, info, null, false, false);
            }
        }
        if (!found) buf.append("<p class=notfound>").append(_t("None")).append("</p>\n");
        writeBuf(out, buf);
    }
    ...
    private void renderIPGroups64(Writer out, StringBuilder buf, Map<Long, List<RouterInfo>> map) throws IOException {
        ...
        for (Long ii : foo) {
            ...
            int i6 = (int) (i & 0xffff);
            String sip = Integer.toString(i0, 16) + ":" + Integer.toString(i2, 16) + ':' + Integer.toString(i4, 16) + ':' + Integer.toString(i6, 16) + "::";
    ...
    private void renderIPGroups48(Writer out, StringBuilder buf, Map<Long, List<RouterInfo>> map) throws IOException {
        ...
        for (Long ii : foo) {
            ...
            int i4 = (int) (i & 0xffff);
            String sip = Integer.toString(i0, 16) + ":" + Integer.toString(i2, 16) + ':' + Integer.toString(i4, 16) + "::";
            buf.append("<p class=sybil_info><b>").append(count).append(" routers with IP <a href=\"/netdb?ip=").append(sip).append("&amp;sybil\">").append(sip).append("</a></b></p>");
            for (RouterInfo info : ris) {
                found = true;
                renderRouterInfo(buf, info, null, false, false);
    ...
    private void renderIPGroupsFamily(Writer out, StringBuilder buf, Map<String, List<RouterInfo>> map) throws IOException {
        buf.append("<h3 id=samefamily class=sybils>").append(_t("Routers in the same Family")).append("</h3><div class=\"sybil_container\">\n");
        ...
        for (String s : foo) {
            ...
            if (count > 1) {
                buf.append("<p class=family><b>").append(count).append(' ').append(_t("routers in family")).append(":</b> &nbsp; <wbr><a href=\"/netdb?fam=").append(ss).append("&amp;sybil\">").append(ss).append("</a></p>\n");
                found = true;
            }
            // for (RouterInfo info : ris) {
            // limit display
    ...
    private void renderRouterInfoHTML(Writer out, StringBuilder buf, Hash us, double avgMinDist, List<RouterInfo> ris) throws IOException {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (dist < MIN_CLOSE) break;
            if (dist < avgMinDist) {
                if (i == 0) {
                    // buf.append("<p><b>Not to worry, but above router is closer than average minimum distance " + TWO_DECIMALS.get().format(avgMinDist) + "</b></p>\n");
                } else if (i == 1) {
                    buf.append("<p class=sybil_info><b>" + _t("Not to worry, but above routers are closer than average minimum distance") + " " + TWO_DECIMALS.get().format(avgMinDist) + "</b></p>\n");
                } else if (i == 2) {
                    buf.append("<p class=sybil_info><b>" + _t("Possible Sybil Warning - above routers are closer than average minimum distance") + " " + TWO_DECIMALS.get().format(avgMinDist) + "</b></p>\n");
                } else {
                    buf.append("<p class=sybil_info><b>" + _t("Major Sybil Warning - above router is closer than average minimum distance") + " " + TWO_DECIMALS.get().format(avgMinDist) + "</b></p>\n");
                }
            }
        ...
        buf.append("<p id=sybil_totals><b>" + _t("Totals for") + " " + count + " " + _t("floodfills") + ": &nbsp; </b><span class=netdb_name>" + _t("MIN") + ":</span > " + TWO_DECIMALS.get().format(min) + "&nbsp; <span class=netdb_name>" + _t("AVG") + ":</span> " + TWO_DECIMALS.get().format(avg) + "&nbsp; <span class=netdb_name>" + _t("MEDIAN") + ":</span> " + TWO_DECIMALS.get().format(median) + "&nbsp; <span class=netdb_name>" + _t("MAX") + ":</span> " + TWO_DECIMALS.get().format(max) + "</p>\n");
        writeBuf(out, buf);
    }
    ...
    private double renderRouterInfo(StringBuilder buf, RouterInfo info, Hash us, boolean isUs, boolean full) {
        ...
        double distance = 0;
        if (isUs) {
            buf.append("<tr id=sybil_ourinfo><th id=our-info><b>" + _t("Our info") + ":</b> <code>").append(hash).append("</code>");
        } else {
        ...
        String caps = DataHelper.stripHTML(info.getCapabilities()) .replace("XO", "X")
                        .replace("PO", "P")
                        .replace("Kf", "fK")
                        .replace("Lf", "fL")
                        .replace("Mf", "fM")
        ...
        if (hasD) {
            ...
            else caps = caps.replace("D", "").replace("class=tier", "class=\"tier isD\"").replace("\"><span class", "RD\"><span class");
        } else if (hasE) {
            if (isU) caps = caps.replace("E", "").replace("class=tier", "class=\"tier isE\"").replace("\"><span class", "UE\"><span class");
            else caps = caps.replace("E", "").replace("class=tier", "class=\"tier isE\"").replace("\"><span class", "RE\"><span class");
        } else if (hasG) {
            if (isU) caps = caps.replace("G", "").replace("class=tier", "class=\"tier isG\"").replace("\"><span class", "UG\"><span class");
            else caps = caps.replace("G", "").replace("class=tier", "class=\"tier isG\"").replace("\"><span class", "RG\"><span class");
        }
        caps = caps.replace("\"><span", tooltip);
        buf.append(caps);
        buf.append("<a href=\"/netdb?v=").append(DataHelper.stripHTML(info.getVersion())).append("\">").append("<span class=version title=\"").append(_t("Show all routers with this version in the NetDb")).append("\">").append(DataHelper.stripHTML(info.getVersion())).append("</span></a>");
        if (!isUs) {
            buf.append("<span class=netdb_header>");
            String family = info.getOption("family");
            if (family != null) {
                buf.append("<a class=familysearch href=\"/netdb?fam=").append(family).append("\" title=\"").append(_t("Show all routers for this family in NetDb")).append("\">").append(_t("Family")).append("</a>");
            }
            buf.append("<a class=viewprofile href=\"/viewprofile?peer=").append(hash).append("\" title=\"").append(_t("View profile")).append("\">").append(_t("Profile")).append("</a>").append("<a class=configpeer href=\"/configpeer?peer=").append(hash).append("\" title=\"").append(_t("Configure peer")).append("\">").append(_t("Edit")).append("</a>");
            String country = _context.commSystem().getCountry(info.getIdentity().getHash());
            if (country != null) {
                buf.append("<a href=\"/netdb?c=").append(country).append("\">").append("<img height=12 width=16 loading=lazy alt=\"").append(country.toUpperCase(Locale.US)).append('\"').append(" title=\"").append(getTranslatedCountry(country)).append('\"').append(" src=\"/flags.jsp?c=").append(country).append("\"> ").append("</a>");
            } else {
                buf.append("<img height=12 width=16 loading=lazy alt=\"??\"").append(" title=\"unknown\"").append(" src=\"/flags.jsp?c=a0\"></a>");
            }
        }
        buf.append("</span>");
        if (_context.portMapper().isRegistered("imagegen")) buf.append("<img class=identicon src=\"/imagegen/id?s=32&amp;c=").append(hash.replace("=", "%3d")).append("\" height=28 width=28 loading=lazy>");
        buf.append("</th></tr>\n");
        ...
        if (us != null) {
            ...
            buf.append("<p><b>").append(_t("Hash Distance")).append(":</b> ").append(TWO_DECIMALS.get().format(distance)).append("</p>\n");
        }
    ...
    public static void renderSybilHTML(Writer out, RouterContext ctx, List<Hash> sybils, String victim) throws IOException {
        ...
        for (Hash h : sybils) {
            ...
            if (prev != null) {
                ...
                writeDistance(out, TWO_DECIMALS.get(), dist);
            }
            prev = h;
            out.write("</tr>\n");
        }
        ...
        for (int i = start; i <= days; i++) {
            out.write("<h3 class=tabletitle>" + _x("Distance for") + ' ' + utcTWO_DECIMALS.format(Date.from(Instant.ofEpochMilli(now))) + "</h3><table class=sybil_distance><tr><th>Hash<th>Distance<th>Distance from previous</tr>\n");
