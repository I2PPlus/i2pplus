...
<tr class=config><th>i2psnark.clientId={random|i2psnark|Vuze|BiglyBT|Transmission|KTorrent|Deluge|qBittorrent|libtorrent|Tixati} <span class=plus>I2P+</span></th></tr>
<tr><td><%=intl._t("This setting controls client identity spoofing: the tracker peer ID and User-Agent, the BitTorrent handshake peer ID, the extension handshake client name, and web seed fetches all present as the selected well-known client instead of I2PSnark. In multi-destination mode each destination pool is assigned its own random client by default; in single-destination mode I2PSnark identifies as itself unless a client name or \"random\" is set. Set to \"random\" to force random selection in all modes, \"i2psnark\" to keep the classic identity even in multi-destination mode, or to a client name to always impersonate that client. Version strings are pinned to current upstream releases and are consistent across the peer ID, User-Agent and extension handshake. To change, add to I2PSnark's configuration file <code>i2psnark.config</code>. [Default is random per pool in multi-destination mode; applies to torrents started afterwards]")%></td></tr>

<tr class=config><th>i2psnark.clientIds={name1,name2,...} <span class=plus>I2P+</span></th></tr>
<tr><td><%=intl._t("This setting optionally restricts the clients chosen by random selection for <code>i2psnark.clientId</code> to a comma-separated subset of known client names; unknown names are ignored and an empty value selects from all supported clients. To change, add to I2PSnark's configuration file <code>i2psnark.config</code>. [Default is unset, applies to torrents started afterwards]")%></td></tr>
