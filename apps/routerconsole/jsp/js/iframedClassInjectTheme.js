...
// refresh embedded logs or orchid every 30 seconds

function doRefresh() {
    if (((u.indexOf(".log") >= 0 || u.indexOf("orchid") >= 0)) && (r != null)) {
        w = document.getElementById(a + '_frame').contentWindow;
        var end = document.createElement("span");
        end.setAttribute("id", "endOfPage");
        end.setAttribute("data-iframe-height", "");
        frames[a + "_frame"].document.body.appendChild(end);
//        setTimeout("document.getElementById(a + '_frame').contentWindow.location.reload(true);", 30000);
        if (u.indexOf(".log") >=0) {
