#!/usr/bin/env python3
"""
Create a RAW session that forwards received raw messages to a local UDP port.

Usage:  ./samForward.py [port] [name]
          port : local UDP port to forward to (default: 26000)
          name : session nickname (default: rawSamForward)

Example: ./samForward.py 26000 myRawForward
"""
...
def main():
    forward_port = int(sys.argv[1]) if len(sys.argv) >= 2 else 26000
    name = sys.argv[2] if len(sys.argv) >= 3 else "rawSamForward"
    ...
    sess.sendall(
        "SESSION CREATE STYLE=RAW ID={} DESTINATION=TRANSIENT"
