                    ...
                    if (this.verbose) {
                        if (sendPorts) {
                            SAMv3Handler.notifyStreamIncomingConnection(
                                    clientServerSock, i2ps.getPeerDestination(), i2ps.getPort(), i2ps.getLocalPort());
                        } else {
            ...
            try {
                while (!Thread.interrupted() && (in.read(buf) >= 0 || buf.position() != 0)) {
                    ...
                    if (nick != null) {
                        ...
                        if (now - lastTouch > 120_000L) {
                            SAMv3Handler.sSessionsHash.get(nick);
                            lastTouch = now;
                        }
                    }
                }
