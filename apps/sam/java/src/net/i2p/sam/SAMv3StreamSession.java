...
class SAMv3StreamSession extends SAMStreamSession implements Session {
    ...
    public SAMv3StreamSession(String login) throws IOException, DataFormatException, SAMException {
        super(getDB().get(login).getDest(), "__v3__", getDB().get(login).getProps(), getDB().get(login).getHandler());
        this.nick = login;
        _acceptQueue = null;
    }
     ...
     * @since 0.9.25
     */
    public SAMv3StreamSession(String login, Properties props, SAMv3Handler handler, I2PSocketManager mgr, int listenPort) throws IOException, DataFormatException, SAMException {
    ...
    public void startForwardingIncoming(Properties props, boolean sendPorts) throws SAMException, InterruptedIOException {
        ...
        if (host == null) {
            ...
            if (_log.shouldDebug()) _log.debug("No host specified. Taken from the client socket : " + host + ':' + port);
        }
