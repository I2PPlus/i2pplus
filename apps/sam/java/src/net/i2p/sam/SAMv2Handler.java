...
class SAMv2Handler extends SAMv1Handler implements SAMRawReceiver, SAMDatagramReceiver, SAMStreamReceiver {
    ...
    protected boolean execStreamMessage(String opcode, Properties props) {
        ...
        if (opcode.equals("SEND")) {
            ...
            if (_log.shouldDebug()) _log.debug("Unrecognized RAW message opcode: \"" + opcode + "\"");
            return false;
        }
    }
    ...
    private boolean execStreamReceive(Properties props) {
        if (props.isEmpty()) {
            if (_log.shouldDebug()) _log.debug("No parameters specified in STREAM RECEIVE message");
            return false;
        }
...

        {
            ...
            if (strid == null) {
                if (_log.shouldDebug()) _log.debug("ID not specified in STREAM RECEIVE message");
                return false;
            }
            ...
            try {
                ...
                if (_log.shouldDebug()) _log.debug("Invalid STREAM RECEIVE ID specified: " + strid);
                return false;
            }
        }
        ...
        long limit = 0;
        {
            ...
            if (strsize == null) {
                if (_log.shouldDebug()) _log.debug("Limit not specified in STREAM RECEIVE message");
                return false;
            }
            ...
            if (strsize.equals("NONE")) {
                ...
                try {
                    ...
                    if (_log.shouldDebug()) _log.debug("Invalid STREAM RECEIVE size specified: " + strsize);
                    return false;
                }

                if (limit < 0) {
                    if (_log.shouldDebug()) _log.debug("Specified limit (" + limit + ") is out of protocol limits");
                    return false;
                }
            }
        }
