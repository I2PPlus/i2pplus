...
class SAMv3Handler extends SAMv1Handler {
    ...
    public SAMv3Handler(SocketChannel s, int verMajor, int verMinor, Properties i2cpProps, SAMBridge parent) throws SAMException, IOException {
        ...
        if (_log.shouldDebug()) _log.debug("SAMv3Handler instantiated");
    }
    ...
    public void handle() {
        ...
        if (_log.shouldDebug()) _log.debug("SAMv3Handler started");
        ...
        try {
            ...
            while (true) {
                if (shouldStop()) {
                    if (_log.shouldDebug()) _log.debug("Stop request received for SAMv3Handler");
                    break;
                }
                String line;
                if (sendPorts) {
                    ...
                    try {
                        ...
                        if (buf.length() <= 0) {
                            if (_lastPing > 0) {
                                ...
                                if (_log.shouldDebug()) _log.debug("Sending PING " + now);
                                _lastPing = now;
                ...
                if (_log.shouldInfo()) _log.info("New message received: [" + line + ']');
                props = SAMUtils.parseParams(line);
                domain = (String) props.remove(SAMUtils.COMMAND);
                if (domain == null) {
                    if (_log.shouldDebug()) _log.debug("Ignoring newline in SAMv3 command");
                    continue;
                }
                gotFirstLine = true;
                ...
                if (opcode == null) {
                    ...
                    if (writeString(domain + " STATUS RESULT=I2P_ERROR", "missing subcommand, enter HELP for help")) continue;
                    else break;
                }
            ...
            if (_log.shouldDebug()) _log.debug("Caught IOException in handler" + "\n* Error: " + e.getMessage());
            ...
            if (_log.shouldDebug()) _log.debug("Stopping SAMv3Handler");
            ...
            if (streamForwardingSocket) {
                if (this.getStreamSession() != null) {
                    try {
                        ...
                        if (_log.shouldWarn()) _log.warn("Error while stopping forwarding connections" + "\n* Error: " + e.getMessage());
                    } catch (InterruptedIOException e) {
                        if (_log.shouldWarn()) _log.warn("Interrupted while stopping forwarding connections" + "\n* Error: " + e.getMessage());
                    }
                }
            }
            die();
        ...
        try {
            if (opcode.equals("CREATE")) {
                ...
                if (dest.equals("TRANSIENT")) {
                    if (_log.shouldDebug()) _log.debug("TRANSIENT destination requested");
                    ...
                    if (_log.shouldDebug()) _log.debug("Custom destination specified [" + dest + "]");
                ...
                try {
                    ...
                    if (_log.shouldDebug()) _log.debug("SESSION ID parameter already in use");
                ...
                if (style.equals("RAW")) {
                    ...
                    if (_log.shouldDebug()) _log.debug("Unsupported SESSION STYLE: \"" + style + "\"");
                ...
                if (rv && ignoredCommand != null) rv = writeString(ignoredCommand + " STATUS RESULT=I2P_ERROR", "invalid state");
                return rv;
            ...
            if (detector != null) {
                ...
                String ignoredCommand = detector.ignoredCommand;
                if (ignoredCommand != null) writeString(ignoredCommand + " STATUS RESULT=I2P_ERROR", "invalid state");
            }
    ...
    private class SocketCloseDetector extends I2PAppThread {
        ...
        public void run() {
            ...
            try {
                            ...
                            while (true) {
                                ...
                                if (c == '\n') {
                                    ...
                                    try {
                                        ...
                                        if (domain == null) continue; // empty line
                                    ...
                                    if (_log.shouldWarn()) _log.warn("Ignoring SAM command during tunnel build: " + line);
                                }
                                buf.append((char) c);
                            }
                        } catch (SocketTimeoutException ste) {
                ...
                if (_log.shouldWarn()) _log.warn("Detector exited after tunnel build");
            } catch (IOException ioe) {
    ...
    protected boolean execStreamMessage(String opcode, Properties props) {
        ...
        if (nick == null) {
            if (_log.shouldDebug()) _log.debug("SESSION ID parameter not specified");
            try {
        ...
        if (rec == null) {
            if (_log.shouldDebug()) _log.debug("STREAM SESSION ID does not exist");
            try {
        ...
        if (streamSession == null) {
            if (_log.shouldDebug()) _log.debug("specified ID is not a stream session");
            try {
        ...
        if (streamSession.isDestroyed()) {
            if (_log.shouldDebug()) _log.debug("Session manager is destroyed");
            try {
        ...
        if (opcode.equals("CONNECT")) {
            ...
            if (_log.shouldDebug()) _log.debug("Unrecognized STREAM message opcode: \"" + opcode + "\"");
            try {
    ...
    protected boolean execStreamConnect(Properties props) {
        ...
        try {
            ...
            if (dest == null) {
                ...
                if (_log.shouldDebug()) _log.debug("Destination not specified in STREAM CONNECT message");
                return false;
            }
    ...
    private boolean execStreamForwardIncoming(Properties props) {
        ...
        try {
            try {
                ...
                if (_log.shouldDebug()) _log.debug("Forwarding STREAM connections failed" + "\n* Error: " + e.getMessage());
    ...
    private boolean execStreamAccept(Properties props) throws I2PSessionException, RouterRestartException {
        ...
        try {
            try {
                ...
                if (_log.shouldDebug()) _log.debug("STREAM ACCEPT failed" + "\n* Error: " + e.getMessage());
                notifyStreamResult(verbose, "TIMEOUT", e.getMessage());
            } catch (I2PSessionException e) {
                // As of 0.9.61, this is thrown for a destroyed session.
                // Kill the SAM session.
                if (_log.shouldDebug()) _log.debug("STREAM ACCEPT failed", e);
                notifyStreamResult(verbose, "I2P_ERROR", e.getMessage());
    ...
    private void execPongMessage(String s) {
        ...
        if (_lastPing > 0) {
            ...
            if (expected.equals(s)) {
                _lastPing = 0;
                if (_log.shouldInfo()) _log.warn("Received expected pong: " + s);
            } else {
                if (_log.shouldInfo()) _log.warn("Received unexpected pong: " + s);
            }
        } else {
