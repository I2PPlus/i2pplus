...
public class ID3v2CommentFrameData extends AbstractID3v2FrameData {
    ...
    public ID3v2CommentFrameData(
        ...
        if (description != null
                && comment != null
                && description.getTextEncoding() != comment.getTextEncoding()) {
            throw new IllegalArgumentException(
                    "description and comment must have same text encoding");
        }
    ...
    public ID3v2CommentFrameData(boolean unsynchronisation, byte[] bytes)
    ...
    protected void unpackFrameData(byte[] bytes) throws InvalidDataException {
        ...
        comment =
                new EncodedText(
                        bytes[0], BufferTools.copyBuffer(bytes, marker, bytes.length - marker));
    }
    ...
    protected byte[] packFrameData() {
        ...
        if (description != null) {
            ...
            BufferTools.copyIntoByteBuffer(
                    descriptionBytes, 0, descriptionBytes.length, bytes, marker);
            marker += descriptionBytes.length;
        } else {
            byte[] terminatorBytes = comment != null ? comment.getTerminator() : new byte[] {0};
            BufferTools.copyIntoByteBuffer(
                    terminatorBytes, 0, terminatorBytes.length, bytes, marker);
