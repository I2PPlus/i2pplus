...
class TrackerInfo {
    ...
    private TrackerInfo(Map<String, BEValue> m, byte[] my_id, byte[] infohash, MetaInfo metainfo, I2PSnarkUtil util) throws IOException {
        ...
        if (reason != null) {
            ...
            if (bePeers == null) {peers = Collections.emptySet();}
            else {
                Set<Peer> p;
                try {p = getPeers(bePeers.getBytes(), my_id, infohash, metainfo, util);} // One big string (the official compact format)
    ...
    private static Set<Peer> getPeers(Set<Hash> hashes, byte[] my_id, byte[] infohash, MetaInfo metainfo, I2PSnarkUtil util) {
        ...
        for (Hash h : hashes) {
            ...
            try {peerID = new PeerID(hash, util);}
            catch (InvalidBEncodingException ibe) {continue;} // won't happen
            peers.add(new Peer(peerID, my_id, infohash, metainfo));
        }
        return peers;
    }

    public Set<Peer> getPeers() {return peers;}
