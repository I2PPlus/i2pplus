...
public class MagnetURI {
    ...
    public MagnetURI(I2PSnarkUtil util, String url) throws IllegalArgumentException {
        ...
        byte[] ih = null;
        if (ihash.length() == 32) {ih = Base32.decode(ihash);}
            ...
            } catch (NumberFormatException nfe) {ih = null;}
        }
     ...
     */
    public byte[] getInfoHash() {return _ih;}
     ...
     */
    public String getTrackerURL() {return _trackers != null ? _trackers.get(0) : null;}

    /**
     *  @return all valid tracker urls or null if none
     *  @since 0.9.67 TODO to be hooked in via SnarkManager.addMagnet() and new Snark()
     */
    public List<String> getTrackerURLs() {return _trackers;}
    ...
    private static List<String> getTrackerParam(String uri) {
        ...
        for (String t : trackers) {
            try {
                ...
                if (protocol == null || host == null) {continue;}
    ...
    private static String decode(String s) {
        ...
        if (utf8) {
            try {return new String(buf.toString().getBytes("ISO-8859-1"), "UTF-8");}
