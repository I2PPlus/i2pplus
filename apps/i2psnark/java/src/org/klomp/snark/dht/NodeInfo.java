 ...
 *
 * <p>This class encapsulates all essential information about a DHT node including its Node ID
 * (NID), destination hash, port, and optionally the full I2P destination. It serves as the primary
 * data structure stored in DHTNodes for routing table management and node discovery operations.
 *
 * <p>Data structure components:<br>
 * - NID: 20-byte node identifier used for Kademlia distance calculations<br>
 * - Hash: 32-byte destination hash for I2P routing<br>
 * - Port: 2-byte network port for DHT communication<br>
 * - Destination: Optional full I2P destination (may be added later)
 *
 * <p>The compact representation (54 bytes) is immutable and contains NID, hash, and port. This
 * compact format is used for network transmission and efficient storage. The full destination may
 * be added later when available, enabling direct communication with the node.
 *
 * <p>Design considerations:<br>
 * - DHT exchanges often use only hashes initially, with destinations added later<br>
...
class NodeInfo extends SimpleDataStructure {
    ...
    public NodeInfo(String s) throws DataFormatException {
        ...
        if (nid == null || nid.length != NID.HASH_LENGTH) throw new DataFormatException("Bad NID");
        nID = new NID(nid);
    ...
    /**
     * Verify the NID matches the Hash. See generateNID() for requirements.
     *
