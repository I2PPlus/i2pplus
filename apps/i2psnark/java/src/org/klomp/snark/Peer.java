...
public class Peer implements Comparable<Peer>, BandwidthListener {
  protected final Log _log = I2PAppContext.getGlobalContext().logManager().getLog(getClass());
     ...
     * @since 0.9.30
     */
    public boolean isIncoming() {return _isIncoming;}
    ...
    public String toString() {
      if (peerID != null) {return peerID.toString() + ' ' + _id;}
      else {return "[unknown id] " + _id;}
    }
     ...
     */
    @Override
    public int hashCode() {return peerID.hashCode() ^ (7777 * (int)_id);}
     ...
     *  @since 0.8.4
     */
    public MagnetState getMagnetState() {return magnetState;}

    /** @return could be null @since 0.8.4 */
    public Map<String, BEValue> getHandshakeMap() {return handshakeMap;}
     ...
     *  @since 0.9.47
     */
    public int getMaxPipeline() {return _maxPipeline;}
    ...
    public void setMetaInfo(MetaInfo meta) {
        metainfo = meta;
        PeerState s = state;
        if (s != null) {s.setMetaInfo(meta);}
    }

    public boolean isConnected() {return state != null;}

    /**
     * Disconnects this peer if it was connected.
     * PeerListener.disconnected() will be called when the connection is completely terminated.
    ...
    void disconnect() {
        ...
        if (s != null) {
            ...
            // This is blocking in streaming, so do this after closing the socket so it won't really block
    ...
    public void uploaded(int size) {
        ...
        PeerState s = state;
        if (s != null) {s.getBandwidthListener().uploaded(size);}
    }
     ...
     */
    public long getUploadRate() {return PeerCoordinator.getRate(uploaded_old);}
    ...
    public boolean shouldRequest(int size) {
        PeerState s = state;
        if (s != null) {return s.getBandwidthListener().shouldRequest(this, size);}
        return false;
    }
    ...
    public long getUpBWLimit() {
        PeerState s = state;
        if (s != null) {return s.getBandwidthListener().getUpBWLimit();}
    ...
    public boolean overUpBWLimit() {
        PeerState s = state;
        if (s != null) {return s.getBandwidthListener().overUpBWLimit();}
        return false;
    }
    ...
    public long getDownBWLimit() {
        PeerState s = state;
        if (s != null) {return s.getBandwidthListener().getDownBWLimit();}
        return Integer.MAX_VALUE;
    }
    ...
    public boolean overDownBWLimit() {
        PeerState s = state;
        if (s != null) {return s.getBandwidthListener().overDownBWLimit();}
        return false;
    }
    ...
    public long getInactiveTime() {
        ...
        if (s != null) {
            ...
            } else {return -1;} //"state, no out";
        } else {return -1;} //"no state";
    }
    ...
    public void retransmitRequests() {
        PeerState s = state;
        if (s != null) {s.retransmitRequests();}
    }
    ...
    public boolean isCompleted() {
        PeerState s = state;
        if (s == null || s.bitfield == null) {return false;}
        return s.bitfield.complete();
    }

    /** @since 0.9.31 */
    int getTotalCommentsSent() {return _totalCommentsSent;}

    /** @since 0.9.31 */
    void setTotalCommentsSent(int count) {_totalCommentsSent = count;}

    /**
     * @return false
     * @since 0.9.49
     */
    public boolean isWebPeer() {return false;}

    /**
     * when did handshake complete?
     * @since 0.9.63
     */
    public long getWhenConnected() {return connected;}

    /**
     * when did we last send pex peers?
     * @since 0.9.63
     */
    public long getPexLastSent() {return pexLastSent;}

    /**
     * when did we last send pex peers?
     * @since 0.9.63
     */
    public void setPexLastSent(long now) {pexLastSent = now;}
}
