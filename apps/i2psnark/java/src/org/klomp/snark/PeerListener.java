...
interface PeerListener {
     ...
     * @param choke true when the peer got a choke message, false when the peer got an unchoke
     *     message.
     */
     ...
     *
     * @param peer the Peer that got the message.
     * @param interest true when the peer got a interested message, false when the peer got an
     *     uninterested message.
     */
    void gotInterest(Peer peer, boolean interest);

    /**
     * Called when a have piece message is received. If the method returns true and the peer has not
     * yet received a interested message or we indicated earlier to be not interested then an
     * interested message will be send.
     *
     * @param peer the Peer that got the message.
     * @param piece the piece number that the per just got.
     * @return true when it is a piece that we want, false if the piece is already known.
     */
    boolean gotHave(Peer peer, int piece);

    /**
     * Called when a bitmap message is received. If this method returns true a interested message
     * will be send back to the peer.
     *
     * @param peer the Peer that got the message.
    ...
    /**
     * Called when a piece is received from the peer. The piece must be requested by Peer.request()
     * first. If this method returns false that means the Peer provided a corrupted piece and the
    ...
    /**
     * Called when the peer wants (part of) a piece from us. Only called when the peer is not choked
     * by us (<code>peer.choke(false)</code> was called).
     *
     ...
     * @return a byte array containing the piece or null when the piece is not available (which is a
     *     protocol error).
     */
