 ...
 * method when it detects an incomming BT protocol connection. The PeerAcceptor will then create a
 * new peer if the PeerCoordinator wants more peers.
 */
class PeerAcceptor {
        ...
        if (in instanceof BufferedInputStream) {
            ...
            try {
                ...
                if (_log.shouldInfo())
                    _log.info(
                            "Infohash read from "
                                    + socket.getPeerDestination().calculateHash().toBase64()
                                    + ": "
                                    + Base64.encode(peerInfoHash));
            } catch (IOException ioe) {
                if (_log.shouldInfo())
                    _log.info(
                            "Unable to read the infohash from "
                                    + socket.getPeerDestination().calculateHash().toBase64());
                throw ioe;
            }
        ...
        if (coordinator != null) {
            ...
            if (DataHelper.eq(coordinator.getInfoHash(), peerInfoHash)) {
                ...
                throw new IOException(
                        "Peer wants another torrent ("
                                + Base64.encode(peerInfoHash)
                                + ") but we only support ["
                                + Base64.encode(coordinator.getInfoHash())
                                + "]");
            }
        } else {
    ...
    /**
     * Read ahead to the infohash, throwing an exception if there isn't enough data. Also check the
     * first 20 bytes for the correct protocol here and throw IOE if bad, so we don't hang waiting
     * for 48 bytes if it's not a bittorrent client. The 20 bytes are checked again in
     * Peer.handshake().
     */
