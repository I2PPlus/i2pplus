...
public final class RyuDouble {
            ...
            if (i < POW5_INV_SPLIT.length) {
                ...
                for (int k = 0; k < 4; k++) {
                    if (k == 0) {
                        ...
                        POW5_INV_SPLIT[i][k] =
                                inv.shiftRight((3 - k) * POW5_INV_QUARTER_BITCOUNT)
                                        .and(invMask)
                                        .intValueExact();
                    }
                }
    ...
    public static String doubleToString(final double value, final int decimals) {
        ...
        // representations.
        //
        // We do some extra work here in order to follow Float/Double.toString semantics. In
        // particular,
        // that requires printing in scientific format if and only if the exponent is between -3 and
        // 7,
        ...
        if (dmIsTrailingZeros || dvIsTrailingZeros) {
            ...
            output =
                    dv
                            + ((dv == dm && !(dmIsTrailingZeros && even)) || (lastRemovedDigit >= 5)
                                    ? 1
                                    : 0);
        } else {
            while (dp / 10 > dm / 10) {
        ...
        index = min(index, abs(index - (olength - (vplength + e10)) + decimals));
    ...
    /**
     * Compute the high digits of m * 5^p / 10^q = m * 5^(p - q) / 2^q = m * 5^i / 2^j, with q
     * chosen such that m * 5^i / 2^j has sufficiently many decimal digits to represent the original
