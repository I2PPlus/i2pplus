 ...
 * {@code SVGGraphics2D}, you can retrieve an SVG element (see {@link #getSVGElement()}) or an SVG
 * document (see {@link #getSVGDocument()}) containing your content. <b>Usage</b><br>
 *
 * <p>Using the {@code SVGGraphics2D} class is straightforward. First, create an instance specifying
 * the height and width of the SVG element that will be created. Then, use standard Java2D API calls
 * to draw content into the element. Finally, retrieve the SVG element that has been accumulated.
 * For example:
 *
 ...
 *
 * <p>For the content generation step, you can make use of third party libraries, such as <a
 * href="http://www.jfree.org/jfreechart/">JFreeChart</a> and <a
 * href="http://www.object-refinery.com/orsoncharts/">Orson Charts</a>, that render output using
 * standard Java2D API calls. <b>Rendering Hints</b><br>
 *
 * <p>The {@code SVGGraphics2D} supports a couple of custom rendering hints - for details, refer to
 * the {@link SVGHints} class documentation. Also see the examples in this blog post: <a
 * href="http://www.object-refinery.com/blog/blog-20140509.html">Orson Charts 3D / Enhanced SVG
 * Export</a>. <b>Other Notes</b><br>
 * Some additional notes:
 *
 * <ul>
 *   <li>Images are supported, but for methods with an {@code ImageObserver} parameter note that the
 *       observer is ignored completely. In any case, using images that are not fully loaded already
 *       would not be a good idea in the context of generating SVG data/files;
 *   <li>the {@link #getFontMetrics(java.awt.Font)} and {@link #getFontRenderContext()} methods
 *       return values that come from an internal {@code BufferedImage}, this is a short-cut and we
 *       don't know if there are any negative consequences (if you know of any, please let us know
 *       and we'll add the info here or find a way to fix it);
 *   <li>there are settings to control the number of decimal places used to write the coordinates
 *       for geometrical elements (default 2dp) and transform matrices (default 6dp). These defaults
 *       may change in a future release.
 *   <li>when an HTML page contains multiple SVG elements, the items within the DEFS element for
 *       each SVG element must have IDs that are unique across <em>all</em> SVG elements in the
 *       page. We auto-populate the {@code defsKeyPrefix} attribute to help ensure that unique IDs
 *       are generated.
 * </ul>
 *
 * <p>For some demos showing how to use this class, look at the JFree-Demos project at GitHub:
...
public final class SVGGraphics2D extends Graphics2D {
    ...
    /**
     * The shape rendering property to set for the SVG element. Permitted values are "auto",
     * "crispEdges", "geometricPrecision" and "optimizeSpeed".
     */
    private String shapeRendering = "crispEdges";

    /**
     * The text rendering property for the SVG element. Permitted values are "auto",
     * "optimizeSpeed", "optimizeLegibility" and "geometricPrecision".
     */
    ...
    /** The number of decimal places to use when writing the matrix values for transformations. */
    ...
    /**
     * A prefix for the keys used in the DEFS element. This can be used to ensure that the keys are
     * unique when creating more than one SVG element for a single HTML page.
     */
    private String defsKeyPrefix = "";

    /**
     * A map of all the gradients used, and the corresponding id. When generating the SVG file, all
     * the gradient paints used must be defined in the defs element.
     */
    private Map<GradientPaintKey, String> gradientPaints = new HashMap<>();

    /**
     * A map of all the linear gradients used, and the corresponding id. When generating the SVG
     * file, all the linear gradient paints used must be defined in the defs element.
     */
    private Map<LinearGradientPaintKey, String> linearGradientPaints = new HashMap<>();

    /**
     * A map of all the radial gradients used, and the corresponding id. When generating the SVG
     * file, all the radial gradient paints used must be defined in the defs element.
     */
    ...
    /**
     * The filename prefix for images that are referenced rather than embedded but don't have an
     * {@code href} supplied via the {@link SVGHints#KEY_IMAGE_HREF} hint.
     */
    private String filePrefix = "image-";

    /**
     * The filename suffix for images that are referenced rather than embedded but don't have an
     * {@code href} supplied via the {@link SVGHints#KEY_IMAGE_HREF} hint.
     */
    private String fileSuffix = ".png";

    /**
     * A list of images that are referenced but not embedded in the SVG. After the SVG is generated,
     * the caller can make use of this list to write PNG files if they don't already exist.
     */
    ...
    /**
     * The font render context. The fractional metrics flag solves the glyph positioning issue
     * identified by Christoph Nahr:
    ...
    /**
     * An instance that is lazily instantiated in drawLine and then subsequently reused to avoid
    ...
    /**
     * An instance that is lazily instantiated in draw/fillRoundRect and then subsequently reused to
     * avoid creating a lot of garbage.
     */
    private RoundRectangle2D roundRect;

    /**
     * An instance that is lazily instantiated in draw/fillOval and then subsequently reused to
     * avoid creating a lot of garbage.
     */
    private Ellipse2D oval;

    /**
     * An instance that is lazily instantiated in draw/fillArc and then subsequently reused to avoid
     * creating a lot of garbage.
     */
    private Arc2D arc;

    /**
     * If the current paint is an instance of {@link GradientPaint}, this field will contain the
     * reference id that is used in the DEFS element for that linear gradient.
     */
    private String gradientPaintRef = null;

    /**
     * The device configuration (this is lazily instantiated in the getDeviceConfiguration()
     * method).
     */
     ...
     * StringBuilder instance. This constructor is used by the {@link #create()} method, but won't
     * normally be called directly by user code.
     *
     * @param width the width of the SVG element.
     * @param height the height of the SVG element.
     ...
     * @param width the width of the SVG element.
     * @param height the height of the SVG element.
     * @param units the units for the width and height above ({@code null} permitted).
     * @param sb the string builder ({@code null} not permitted).
     * @since 3.2
     */
    public SVGGraphics2D(int width, int height, SVGUnits units, StringBuilder sb) {
        ...
        this.hints =
                new RenderingHints(
                        SVGHints.KEY_IMAGE_HANDLING, SVGHints.VALUE_IMAGE_HANDLING_EMBED);
    ...
    /**
     * Returns the width for the SVG element, specified in the constructor. This value will be
     * written to the SVG element returned by the {@link #getSVGElement()} method.
     *
    ...
    /**
     * Returns the height for the SVG element, specified in the constructor. This value will be
     * written to the SVG element returned by the {@link #getSVGElement()} method.
     *
     ...
     * constructor. The default value is {@code null}).
     *
     ...
     * Permitted values are "auto", "crispEdges", "geometricPrecision", "inherit" and
     * "optimizeSpeed".
     *
     ...
     * Permitted values are "auto", "optimizeSpeed", "optimizeLegibility" and "geometricPrecision".
     *
     * @param value the new value.
     * @since 2.0
     */
    ...
    /**
     * Returns the flag that controls whether or not this object will observe the {@code
     * KEY_STROKE_CONTROL} rendering hint. The default value is {@code true}.
     *
     * @return A boolean.
    ...
    /**
     * Sets the flag that controls whether or not this object will observe the {@code
     * KEY_STROKE_CONTROL} rendering hint. When enabled (the default), a hint to normalise strokes
     * will write a {@code stroke-style} attribute with the value {@code crispEdges}.
     *
    ...
    /**
     * Returns the prefix used for all keys in the DEFS element. The default value is {@code "_"+
    ...
    /**
     * Sets the prefix that will be used for all keys in the DEFS element. If required, this must be
     * set immediately after construction (before any content generation methods have been called).
     *
     ...
     *
     * <p>Note that there is a separate attribute to control the number of decimal places for
     * geometrical elements in the output (see {@link #getGeometryDP()}).
     *
     ...
     * output. Values in the range 1 to 10 will be used to configure a formatter to that number of
     * decimal places, for all other values we revert to the normal {@code String} conversion of
     * {@code double} primitives (approximately 16 decimals places).
     *
     * <p>Note that there is a separate attribute to control the number of decimal places for
     * geometrical elements in the output (see {@link #setGeometryDP(int)}).
     *
    ...
    /**
     * Returns the number of decimal places used to write the coordinates of geometrical shapes. The
     * default value is 2.
     *
     * <p>Note that there is a separate attribute to control the number of decimal places for
     * transform matrices in the output (see {@link #getTransformDP()}).
     *
     * @return The number of decimal places.
     */
     ...
     * SVG output. Values in the range 1 to 10 will be used to configure a formatter to that number
     * of decimal places, for all other values we revert to the normal String conversion of double
     * primitives (approximately 16 decimals places).
     *
     * <p>Note that there is a separate attribute to control the number of decimal places for
     * transform matrices in the output (see {@link #setTransformDP(int)}).
     *
     * @param dp the number of decimal places (normally 1 to 10).
     */
     ...
     * (the default value is {@code 0.1}). In the Java specification for {@code BasicStroke} it
     * states "If width is set to 0.0f, the stroke is rendered as the thinnest possible line for the
     * target device and the antialias hint setting." We don't have a means to implement that
     * accurately since we must specify a fixed width.
     *
     * @return The width.
     * @since 1.9
     */
     ...
     * accumulated the drawing operations). Not sure yet when or why this would be useful when
     * creating SVG output. Note that the {@code fontMapper} object ({@link #getFontMapper()}) is
    ...
    /**
     * Returns the paint used to draw or fill shapes (or text). The default value is {@link
     * Color#BLACK}.
     *
    ...
    /**
     * Sets the paint used to draw or fill shapes (or text). If {@code paint} is an instance of
     * {@code Color}, this method will also update the current color attribute (see {@link
     * #getColor()}). If you pass {@code null} to this method, it does nothing (in accordance with
     * the JDK specification).
     *
    ...
    /**
     * Returns the foreground color. This method exists for backwards compatibility in AWT, you
     * should use the {@link #getPaint()} method.
     *
    ...
    /**
     * Sets the foreground color. This method exists for backwards compatibility in AWT, you should
     * use the {@link #setPaint(java.awt.Paint)} method.
     *
    ...
    /**
     * Returns the background color. The default value is {@link Color#BLACK}. This is used by the
    ...
    /**
     * Sets the background color. This is used by the {@link #clearRect(int, int, int, int)} method.
     * The reference implementation allows {@code null} for the background color so we allow that
     * too (but for that case, the clearRect method will do nothing).
     *
    ...
    /**
     * Returns the current value for the specified hint. See the {@link SVGHints} class for
     * information about the hints that can be used with {@code SVGGraphics2D}.
     *
     * @param hintKey the hint key ({@code null} permitted, but the result will be {@code null}
     *     also).
    ...
    /**
     * Sets the value for a hint. See the {@link SVGHints} class for information about the hints
     * that can be used with this implementation.
     *
            ...
            if (groupId != null) {
                if (this.elementIDs.contains(groupId)) {
                    throw new IllegalArgumentException(
                            "The group id (" + groupId + ") is not unique.");
                } else {
    ...
    /**
     * Returns a copy of the rendering hints. Modifying the returned copy will have no impact on the
     * state of this {@code Graphics2D} instance.
     *
    ...
    /**
     * A utility method that appends an optional element id if one is specified via the rendering
     * hints.
     *
     * @param sb the string builder ({@code null} not permitted).
     */
        ...
        if (elementID != null) {
            ...
            if (this.elementIDs.contains(elementID)) {
                throw new IllegalStateException(
                        "The element id " + elementID + " is already used.");
            } else {
    ...
    /**
     * Draws the specified shape with the current {@code paint} and {@code stroke}. There is direct
     * handling for {@code Line2D}, {@code Rectangle2D}, {@code Ellipse2D} and {@code Path2D}. All
    ...
    /**
     * Fills the specified shape with the current {@code paint}. There is direct handling for {@code
     * Rectangle2D}, {@code Ellipse2D} and {@code Path2D}. All other shapes are mapped to a {@code
     * GeneralPath} and then filled.
     *
     * @param s the shape ({@code null} not permitted).
    ...
    /**
     * Returns the current alpha (transparency) in the range 0.0 to 1.0. If the current composite is
     * an {@link AlphaComposite} we read the alpha value from there, otherwise this method returns
     * 1.0.
     *
    ...
    /**
     * Returns an SVG color string based on the current paint. To handle {@code GradientPaint} we
     * rely on the {@code setPaint()} method having set the {@code gradientPaintRef} attribute.
     *
    ...
    private String strokeStyle() {
        ...
        if (!(this.getPaint() instanceof Color
                && ((Color) this.getPaint()).getRGB() == Color.BLACK.getRGB())) {
    ...
    /**
     * Returns the alpha value of the current {@code paint}, or {@code 1.0f} if it is not an
     * instance of {@code Color}.
     *
    ...
    /**
     * Returns the font mapper (an object that optionally maps font family names to alternates). The
     * default mapper will convert Java logical font names to the equivalent SVG generic font name,
     * and leave all other font names unchanged.
     *
    ...
    /**
     * Sets the font size units. In general, if this method is used it should be called immediately
     * after the {@code SVGGraphics2D} instance is created and before any content is generated.
     *
    ...
    /**
     * Draws a string at {@code (x, y)}. The start of the text at the baseline level will be aligned
     * with the {@code (x, y)} point. <br>
     * <br>
     * Note that you can make use of the {@link SVGHints#KEY_TEXT_RENDERING} hint when drawing
     * strings (this is completely optional though).
     *
    ...
    /**
     * Draws a string of attributed characters at {@code (x, y)}. The call is delegated to {@link
     ...
     *
     * @param iterator an iterator over the characters ({@code null} not permitted).
     * @param x the x-coordinate.
     * @param y the y-coordinate.
     */
    @Override
    ...
    /**
     * Applies the translation {@code (tx, ty)}. This call is delegated to {@link #translate(double,
     * double)}.
     *
    ...
    /**
     * Applies a shear transformation. This is equivalent to the following call to the {@code
     * transform} method: <br>
     * <br>
     *
     * <ul>
     ...
     * Sets the transform.
     *
     * @param t the new transform ({@code null} permitted, resets to the identity transform).
     * @see #getTransform()
     */
    @Override
     ...
     * interior, if {@code onStroke} is {@code false}, otherwise the stroked outline of the shape).
     *
    ...
    /**
     * Returns the user clipping region. The initial default value is {@code null}.
     *
    ...
    /**
     * Registers the clip so that we can later write out all the clip definitions in the DEFS
     * element.
     *
     ...
     *
     * <p>According to the Oracle API specification, this method will accept a {@code null}
     * argument, but there is an open bug report (since 2004) that suggests this is wrong:
     *
     * <p><a href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=6206189">
    ...
    /**
     * Draws a line from {@code (x1, y1)} to {@code (x2, y2)} using the current {@code paint} and
     * {@code stroke}.
     *
    ...
    /**
     * Clears the specified rectangle by filling it with the current background color. If the
     * background color is {@code null}, this method will do nothing.
     *
     * @param x the x-coordinate.
     * @param y the y-coordinate.
     * @param width the width.
    ...
    /**
     * Draws an oval framed by the rectangle {@code (x, y, width, height)} using the current {@code
     * paint} and {@code stroke}.
     *
     * @param x the x-coordinate.
     * @param y the y-coordinate.
     * @param width the width.
    ...
    /**
     * Draws an arc contained within the rectangle {@code (x, y, width, height)}, starting at {@code
     * startAngle} and continuing through {@code arcAngle} degrees using the current {@code paint}
     * and {@code stroke}.
     *
     * @param x the x-coordinate.
     * @param y the y-coordinate.
    ...
    /**
     * Fills an arc contained within the rectangle {@code (x, y, width, height)}, starting at {@code
     * startAngle} and continuing through {@code arcAngle} degrees, using the current {@code paint}.
     *
     * @param x the x-coordinate.
     * @param y the y-coordinate.
    ...
    /**
     * Draws an image at the location {@code (x, y)}. Note that the {@code observer} is ignored.
     *
    ...
    /**
     * Draws the image into the rectangle defined by {@code (x, y, w, h)}. Note that the {@code
     * observer} is ignored (it is not useful in this context).
     *
    ...
    /**
     * Draws an image to the rectangle {@code (x, y, w, h)} (scaling it if required), first filling
     * the background with the specified color. Note that the {@code observer} is ignored.
     *
     * @param img the image.
     * @param x the x-coordinate.
     * @param y the y-coordinate.
    ...
    public boolean drawImage(
            Image img, int x, int y, int w, int h, Color bgcolor, ImageObserver observer) {
        Paint saved = getPaint();
        setPaint(bgcolor);
        fillRect(x, y, w, h);
        setPaint(saved);
    ...
    /**
     * Draws part of an image (defined by the source rectangle {@code (sx1, sy1, sx2, sy2)}) into
     * the destination rectangle {@code (dx1, dy1, dx2, dy2)}. Note that the {@code observer} is
     * ignored.
     *
     * @param img the image.
    ...
    /**
     * Draws part of an image (defined by the source rectangle {@code (sx1, sy1, sx2, sy2)}) into
     * the destination rectangle {@code (dx1, dy1, dx2, dy2)}. The destination rectangle is first
     * cleared by filling it with the specified {@code bgcolor}. Note that the {@code observer} is
     * ignored.
     *
     * @param img the image.
     * @param dx1 the x-coordinate for the top left of the destination.
    ...
    /**
     * Draws the rendered image. If {@code img} is {@code null} this method does nothing.
     *
    ...
    /**
     * Draws an image with the specified transform. Note that the {@code observer} is ignored.
     *
     * @param img the image.
    ...
    /**
     * This method does nothing. The operation assumes that the output is in bitmap form, which is
     * not the case for SVG, so we silently ignore this method call.
     *
     * @param x the x-coordinate.
     * @param y the y-coordinate.
    ...
    /**
     * Returns the SVG element that has been generated by calls to this {@code Graphics2D}
     * implementation, giving it the specified {@code id}. If {@code id} is {@code null}, the
     * element will have no {@code id} attribute.
     *
     ...
     * Returns the SVG element that has been generated by calls to this {@code Graphics2D}
     * implementation, giving it the specified {@code id}. If {@code id} is {@code null}, the
     * element will have no {@code id} attribute. This method also allows for a {@code viewBox} to
     * be defined, along with the settings that handle scaling.
     *
     * @param id the element id ({@code null} permitted).
     * @param includeDimensions include the width and height attributes?
     * @param viewBox the view box specification (if {@code null} then no {@code viewBox} attribute
     *     will be defined).
     * @param preserveAspectRatio the value of the {@code preserveAspectRatio} attribute (if {@code
     *     null} then not attribute will be defined).
    ...
    /**
     * Returns an SVG document (this contains the content returned by the {@link #getSVGElement()}
     ...
     * embedded. If the image files don't already exist, you can use this list as the basis for
    ...
    /**
     * Returns an element to represent a linear gradient. All the linear gradients that are used get
     * written to the DEFS element in the SVG.
     *
    ...
    private String getLinearGradientElement(String id, GradientPaint paint) {
        ...
        b.append("x1=\"")
                .append(geomDP(p1.getX()))
                .append("\" ")
                .append("y1=\"")
                .append(geomDP(p1.getY()))
                .append("\" ")
                .append("x2=\"")
                .append(geomDP(p2.getX()))
                .append("\" ")
                .append("y2=\"")
                .append(geomDP(p2.getY()))
                .append("\" ")
    ...
    /**
     * Returns an element to represent a radial gradient. All the radial gradients that are used get
     * written to the DEFS element in the SVG.
     *
     * @param id the reference id.
    ...
    private String getRadialGradientElement(String id, RadialGradientPaint rgp) {
        StringBuilder b =
                new StringBuilder("<radialGradient id=\"")
                        .append(id)
                        .append("\" gradientUnits=\"userSpaceOnUse\" ");
        ...
        b.append("cx=\"")
                .append(geomDP(center.getX()))
                .append("\" ")
                .append("cy=\"")
                .append(geomDP(center.getY()))
                .append("\" ")
                .append("r=\"")
                .append(geomDP(radius))
                .append("\" ")
                .append("fx=\"")
                .append(geomDP(focus.getX()))
                .append("\" ")
                .append("fy=\"")
                .append(geomDP(focus.getY()))
                .append("\">");
    ...
    /**
     * Returns a clip path reference for the current user clip. This is written out on all SVG
     * elements that draw or fill shapes or text.
     *
    ...
    /**
     * Sets the attributes of the reusable {@link Rectangle2D} object that is used by the {@link
     * SVGGraphics2D#drawRect(int, int, int, int)} and {@link SVGGraphics2D#fillRect(int, int, int,
     * int)} methods.
     *
     * @param x the x-coordinate.
     * @param y the y-coordinate.
    ...
    /**
     * Sets the attributes of the reusable {@link RoundRectangle2D} object that is used by the
     * {@link #drawRoundRect(int, int, int, int, int, int)} and {@link #fillRoundRect(int, int, int,
     * int, int, int)} methods.
     *
     * @param x the x-coordinate.
     * @param y the y-coordinate.
    ...
    /**
     * Sets the attributes of the reusable {@link Arc2D} object that is used by {@link #drawArc(int,
     * int, int, int, int, int)} and {@link #fillArc(int, int, int, int, int, int)} methods.
     *
     * @param x the x-coordinate.
     * @param y the y-coordinate.
     * @param width the width.
     ...
     * #drawOval(int, int, int, int)} and {@link #fillOval(int, int, int, int)} methods.
     *
     * @param x the x-coordinate.
     * @param y the y-coordinate.
     * @param width the width.
