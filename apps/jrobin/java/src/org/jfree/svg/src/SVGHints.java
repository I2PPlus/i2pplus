...
/**
 * Defines the rendering hints that can be used with the {@link SVGGraphics2D} class. The supported
 * hints are:<br>
 *
 * <ul>
 *   <li>{@link #KEY_IMAGE_HANDLING} that controls how images are handled (embedded in the SVG, or
 *       referenced externally);
 *   <li>{@link #KEY_IMAGE_HREF} that allows the caller to specify the image href attribute for the
 *       next image;
 *   <li>{@link #KEY_TEXT_RENDERING} that allows configuration of the preferred value of the SVG
 *       {@code text-rendering} attribute in text elements;
 *   <li>{@link #KEY_ELEMENT_ID} that allows the caller to specify the element ID for the next
 *       element;
 *   <li>{@link #KEY_BEGIN_GROUP} tells the {@code SVGGraphics2D} instance to start a new group
 *       element with an id equal to the hint value (which must be an instance of String). Any other
 *       {@code Graphics2D} implementation will ignore this hint;
 *   <li>{@link #KEY_END_GROUP} tells the {@code SVGGraphics2D} instance to end a group element. The
 *       hint value is ignored. The caller assumes responsibility for balancing the number of {@code
 *       KEY_BEGIN_GROUP} and {@code KEY_END_GROUP} hints. Any other {@code Graphics2D}
...
public final class SVGHints {
    ...
    /**
     * The key for the hint that controls whether images are embedded in the SVG or referenced
     * externally. Valid hint values are {@link #VALUE_IMAGE_HANDLING_EMBED} and {@link
     * #VALUE_IMAGE_HANDLING_REFERENCE}.
     */
    public static final SVGHints.Key KEY_IMAGE_HANDLING = new SVGHints.Key(0);

    /**
     * Hint value for {@code KEY_IMAGE_HANDLING} to specify that images should be embedded in the
     * SVG output using PNG data {@code Base64} encoded.
     */
    public static final Object VALUE_IMAGE_HANDLING_EMBED = "VALUE_IMAGE_HANDLING_EMBED";

    /**
     * Hint value for {@code KEY_IMAGE_HANDLING} to say that images should be referenced externally.
     */
    ...
    /**
     * Hint value for {@code KEY_TEXT_RENDERING} to set the {@code text-rendering} attribute in SVG
    ...
    /**
     * Hint key to supply string to be used as the href for an image that is referenced rather than
     * embedded. The value associated with the key should be a string and will be used for the next
     * image element written to the SVG output (and then the hint will be cleared).
     *
     * @since 1.5
     */
    ...
    /**
     * Hint key that informs the {@code SVGGraphics2D} that the caller would like to begin a new
     * group element. The hint value is the id for the new group. After opening the new group the
     * hint is cleared and it is the caller's responsibility to close the group later using {@link
     * SVGHints#KEY_END_GROUP}. Groups can be nested.
     *
     * @since 1.7
     */
    ...
    /**
     * Hint key that informs the {@code SVGGraphics2D} that the caller would like to close a
     * previously opened group element. The hint value is ignored.
     *
     * @since 1.7
     */
    ...
    /**
     * Hint key that informs the {@code SVGGraphics2D} that the caller would like to add a title
    ...
    /**
     * The key for the hint that controls whether strings are rendered as characters or vector
     * graphics (implemented using {@code TextLayout}). The latter will result in larger output
     * files but avoids problems with fonts not being available for the viewer. Valid hint values
     * are {@link #VALUE_DRAW_STRING_TYPE_STANDARD} and {@link #VALUE_DRAW_STRING_TYPE_VECTOR}.
     *
     * @since 2.0
     */
    ...
    /**
     * Hint value for {@code KEY_DRAW_STRING_TYPE} to specify that strings should be written to the
    ...
    /**
     * Hint value for {@code KEY_DRAW_STRING_TYPE} to say that strings should be written to the
    ...
    /**
     * A list of keys that are treated as synonyms for KEY_BEGIN_GROUP (the list does not include
     * KEY_BEGIN_GROUP itself).
     */
    private static final List<RenderingHints.Key> beginGroupKeys;

    /**
     * A list of keys that are treated as synonyms for KEY_END_GROUP (the list does not include
     * KEY_END_GROUP itself).
     */
    private static final List<RenderingHints.Key> endGroupKeys;

    /**
     * A list of keys that are treated as synonyms for KEY_ELEMENT_TITLE (the list does not include
     * KEY_ELEMENT_TITLE itself).
     */
    ...
    /**
     * Creates and returns a list of keys that are synonymous with {@link #KEY_BEGIN_GROUP}.
     *
    ...
    /**
     * Clears the list of keys that are treated as synonyms for {@link SVGHints#KEY_BEGIN_GROUP}.
     *
     * @since 1.8
     */
    ...
    /**
     * Returns {@code true} if this key is equivalent to {@link #KEY_BEGIN_GROUP}, and {@code false}
     * otherwise. The purpose of this method is to allow certain keys from external packages (such
     * as JFreeChart and Orson Charts) to use their own keys to drive the behaviour of {@code
     * SVGHints.KEY_BEGIN_GROUP}. This has two benefits: (1) it avoids the necessity to make
    ...
    /**
     * Creates and returns a list of keys that are synonymous with {@link #KEY_END_GROUP}.
     *
     * @return A list (never {@code null}).
     * @since 1.8
     */
    ...
    /**
     * Clears the list of keys that are treated as synonyms for {@link SVGHints#KEY_END_GROUP}.
     *
     * @since 1.8
     */
    ...
    /**
     * Returns {@code true} if this key is equivalent to {@link #KEY_END_GROUP}, and {@code false}
     * otherwise. The purpose of this method is to allow certain keys from external packages (such
     * as JFreeChart and Orson Charts) to use their own keys to drive the behaviour of {@code
     * SVGHints.KEY_END_GROUP}. This has two benefits: (1) it avoids the necessity to make JFreeSVG
    ...
    /**
     * Creates and returns a list of keys that are synonymous with {@link #KEY_ELEMENT_TITLE}.
     *
     * @return A list (never {@code null}).
     * @since 1.9
     */
    ...
    /**
     * Clears the list of keys that are treated as synonyms for {@link SVGHints#KEY_ELEMENT_TITLE}.
     *
     * @since 1.9
     */
    ...
    /**
     * Returns {@code true} if this key is equivalent to {@link #KEY_ELEMENT_TITLE}, and {@code
     * false} otherwise. The purpose of this method is to allow certain keys from external packages
     * (such as JFreeChart and Orson Charts) to use their own keys to drive the behaviour of {@code
     * SVGHints.KEY_ELEMENT_TITLE}. This has two benefits: (1) it avoids the necessity to make
    ...
    /**
     * Returns {@code true} if Orson Charts (version 1.3 or later) is on the classpath, and {@code
     * false} otherwise. This method is used to auto-register keys from Orson Charts that should
     * translate to the behaviour of {@link SVGHints#KEY_BEGIN_GROUP} and {@link
     * SVGHints#KEY_END_GROUP}. <br>
     * <br>
     * The Orson Charts library can be found at http://www.object-refinery.com/orsoncharts/
     *
     * @return A boolean.
     * @since 1.8
     */
    ...
    /**
     * Returns {@code true} if JFreeChart (1.0.18 or later) is on the classpath, and {@code false}
     * otherwise. This method is used to auto-register keys from JFreeChart that should translate to
     * the behaviour of {@link SVGHints#KEY_BEGIN_GROUP} and {@link SVGHints#KEY_END_GROUP}.
     *
    ...
    public static class Key extends RenderingHints.Key {
        ...
        /**
         * Returns {@code true} if {@code val} is a value that is compatible with this key, and
