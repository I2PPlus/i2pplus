...
/**
 * Base implementation class for all backend classes. Each Round Robin Database object ({@link
 * org.rrd4j.core.RrdDb} object) is backed with a single RrdBackend object which performs actual I/O
 ...
 *       operating system directly). <b>This is the default backend used in Rrd4j since 1.4.0
 *       release.</b>
 ...
 * </ul>
 *
 * <p>To create your own backend in order to provide some custom type of RRD storage, you should do
 * the following:
 *
 * <ul>
 *   <li>Create your custom RrdBackend class (RrdCustomBackend, for example) by extending RrdBackend
 *       class. You have to implement all abstract methods defined in the base class.
 *   <li>Create your custom RrdBackendFactory class (RrdCustomBackendFactory, for example) by
 *       extending RrdBackendFactory class. You have to implement all abstract methods defined in
 *       the base class. Your custom factory class will actually create custom backend objects when
 *       necessary.
 *   <li>Create instance of your custom RrdBackendFactory and register it as a regular factory
 *       available to Rrd4j framework. See javadoc for {@link org.rrd4j.core.RrdBackendFactory} to
...
public abstract class RrdBackend {
    ...
    private static final int STARTPRIVATEAREACODEPOINT =
            Character.codePointAt(new char[] {STARTPRIVATEAREA}, 0);
    private static final int ENDPRIVATEAREACODEPOINT =
            Character.codePointAt(new char[] {ENDPRIVATEAREA}, 0);
    private static final int PRIVATEAREASIZE =
            ENDPRIVATEAREACODEPOINT - STARTPRIVATEAREACODEPOINT + 1;
     ...
     *
     * @param path String identifying RRD storage. For files on the disk, this argument should
     *     represent file path. Other storage types might interpret this argument differently.
     */
    ...
    /**
     * Sets the number of bytes in the underlying RRD storage. This method is called only once,
     * immediately after a new RRD storage gets created.
     *
    ...
    /**
     * Closes the underlying backend. Call by {@code RrdDb#close()} when it's closed. All subclass
    ...
    /**
     * This method suggests the caching policy to the Rrd4j frontend (high-level) classes. If <code>
     * true</code> is returned, frontend classes will cache frequently used parts of a RRD file in
     * memory to improve performance. If <code>false</code> is returned, high level classes will
     * never cache RRD file sections in memory.
     *
     * @return <code>true</code> if file caching is enabled, <code>false</code> otherwise. By
     *     default, the method returns <code>true</code> but it can be overridden in subclasses.
     */
    ...
    protected final void writeString(long offset, String value) throws IOException {
        ...
        // The real position is encoded in the "short" ds name, using the private use area from
        // Unicode
        ...
        if (value.length() > RrdPrimitive.STRING_LENGTH) {
            ...
            StringBuilder newValue =
                    new StringBuilder(value.substring(0, RrdPrimitive.STRING_LENGTH));
            ...
            while (reminder > 0) {
                ...
                newValue.setCharAt(
                        --i,
                        Character.toChars(
                                (int) (reminder % PRIVATEAREASIZE + STARTPRIVATEAREACODEPOINT))[0]);
    ...
    protected final String readString(long offset) throws IOException {
        ...
        while (++i < RrdPrimitive.STRING_LENGTH) {
            ...
            if (c >= STARTPRIVATEAREA && c <= ENDPRIVATEAREA) {
                realStringOffset +=
                        ((long) c - STARTPRIVATEAREACODEPOINT) * Math.pow(PRIVATEAREASIZE, i);
