 ...
 * RRDTool's <a href="http://oss.oetiker.ch/rrdtool/doc/rrdfetch.en.html" target="man">rrdfetch man
 * page</a>.
 *
 * <p>You cannot create <code>FetchRequest</code> directly (no public constructor is provided). Use
 * {@link org.rrd4j.core.RrdDb#createFetchRequest(ConsolFun, long, long, long) createFetchRequest()}
 * method of your {@link org.rrd4j.core.RrdDb RrdDb} object.
 *
...
public class FetchRequest {
    ...
    FetchRequest(
        ...
        if (fetchStart < 0) {
            throw new IllegalArgumentException(
                    "Invalid start time in fetch request: " + fetchStart);
        }
        if (fetchEnd < 0) {
        ...
        if (fetchStart > fetchEnd) {
            throw new IllegalArgumentException(
                    "Invalid start/end time in fetch request: " + fetchStart + " > " + fetchEnd);
        }
        if (resolution <= 0) {
            throw new IllegalArgumentException(
                    "Invalid resolution in fetch request: " + resolution);
        }
     ...
     * (datasource names). If not set (or set to null), fetched data will contain values of all
     * datasources defined in the corresponding RRD. To fetch data only from selected datasources,
     ...
     * (datasource names). If the filter is not set (or set to null), fetched data will contain
     * values of all datasources defined in the corresponding RRD. To fetch data only from selected
    ...
    /**
     * Sets request filter in order to fetch data only for a single datasource (datasource name). If
     * not set (or set to null), fetched data will contain values of all datasources defined in the
     * corresponding RRD. To fetch data for a single datasource only, specify an array of datasource
     * names as method argument.
     *
    ...
    /**
     * Returns request filter. See {@link #setFilter(String...) setFilter()} for complete
     * explanation.
     *
    ...
    public String dump() {
        return "fetch \""
                + parentDb.getRrdBackend().getPath()
                + "\" "
                + consolFun
                + " --start "
                + fetchStart
    ...
    /**
     * Returns data from the underlying RRD and puts it in a single {@link org.rrd4j.core.FetchData
     * FetchData} object.
     *
