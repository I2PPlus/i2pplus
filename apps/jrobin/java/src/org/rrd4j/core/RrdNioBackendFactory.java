 ...
 *
 * <p>Each RrdNioBackendFactory is optionally backed by a {@link org.rrd4j.core.RrdSyncThreadPool},
 ...
 * cause memory leaks.
 *
 * <p>if sync period is negative, no sync thread will be launched
 */
...
public class RrdNioBackendFactory extends RrdFileBackendFactory {
     ...
     * SYNC_BACKGROUND. By default in-memory cache will be transferred to the disc every 300 seconds
     * (5 minutes). Default value can be changed via {@link #setSyncPeriod(int)} method.
     */
     ...
     * #setSyncPeriod(int)} method call, defaults to {@link #DEFAULT_SYNC_PERIOD}. See {@link
     * #setSyncPeriod(int)} for more information.
     *
    ...
    /**
     * Sets time between consecutive background synchronizations. If negative, it will disabling
     * syncing for all NIO backend factory.
     *
    ...
    /**
     * Returns the number of synchronizing threads. If not changed via {@link #setSyncPoolSize(int)}
     * method call, defaults to {@link #DEFAULT_SYNC_CORE_POOL_SIZE}. See {@link
     * #setSyncPoolSize(int)} for more information.
     *
    ...
    /**
     * Sets the number of synchronizing threads. It must be set before the first use of this
     * factory. It will not have any effect afterward.
     *
    ...
    public RrdNioBackendFactory() {
        this(
                RrdNioBackendFactory.defaultSyncPeriod,
                RrdNioBackendFactory.defaultSyncPeriod > 0 ? DefaultSyncThreadPool.INSTANCE : null);
    }
     ...
     * @param syncPoolSize The number of threads to use to sync the mapped file to disk, if negative
     *     or 0, sync threads are disabled.
     */
    ...
    /**
     * Setter for the field <code>syncThreadPool</code>.
     *
     ...
     * Setter for the field <code>syncThreadPool</code>.
     *
     * @param syncThreadPool the ScheduledExecutorService that will back the RrdSyncThreadPool used
     *     to sync the memory-mapped files.
     * @deprecated Create a custom instance instead
     */
    @Deprecated
     ...
     * {@inheritDoc}
     *
     * <p>Creates RrdNioBackend object for the given file path.
     */
    ...
    /**
     * This is a holder class as per the "initialisation on demand" Java idiom. The only purpose of
     * this holder class is to ensure that the thread pool is created lazily the first time that it
     * is needed, and not before.
     *
     * <p>In practice this thread pool will be used if clients rely on the factory returned by
