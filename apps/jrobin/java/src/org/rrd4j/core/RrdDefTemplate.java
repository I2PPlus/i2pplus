 ...
 * objects from a single XML template. XML template can be supplied as an XML InputSource, XML file
 * or XML formatted string.
 *
 * <p>Here is an example of a properly formatted XML template with all available options in it
 * (unwanted options can be removed):
 *
 * <pre>
 * &lt;rrd_def&gt;
 *     &lt;path&gt;test.rrd&lt;/path&gt;
 ...
 * </pre>
 *
 * <p>Notes on the template syntax:
 *
 * <ul>
 *   <li>There is a strong relation between the XML template syntax and the syntax of {@link
 *       org.rrd4j.core.RrdDef} class methods. If you are not sure what some XML tag means, check
 *       javadoc for the corresponding class.
 *   <li>the path to the rrd can be specified either as a file path using path element or as an uri
 *       using a url tag
 *   <li>starting timestamp can be supplied either as a long integer (like: 1000243567) or as an ISO
 *       formatted string (like: 2004-02-21 12:25:45)
 *   <li>whitespaces are not harmful
 *   <li>floating point values: anything that cannot be parsed will be treated as Double.NaN (like:
 *       U, unknown, 12r.23)
 *   <li>comments are allowed.
 * </ul>
 *
 * <p>Any template value (text between <code>&lt;some_tag&gt;</code> and <code>&lt;/some_tag&gt;
 * </code>) can be replaced with a variable of the following form: <code>${variable_name}</code>.
 * Use {@link org.rrd4j.core.XmlTemplate#setVariable(String, String) setVariable()} methods from the
 * base class to replace template variables with real values at runtime.
 *
 ...
 *       many different RRD files from the same XML template. For example, your XML template could
 *       start with:
 *       <pre>
 * &lt;rrd_def&gt;
 *     &lt;path&gt;${path}&lt;/path&gt;
 ...
 * </pre>
 *   <li>Once all template variables are set, just use the template object to create RrdDef object.
 *       This object is actually used to create Rrd4j RRD files:
 *       <pre>
 ...
 * object can be reused to create as many RrdDef objects as needed, with different values specified
 * for template variables. XML syntax check is performed only once - the first definition object
 * gets created relatively slowly, but it will be created much faster next time.
 */
public class RrdDefTemplate extends XmlTemplate {
     ...
     * @throws java.lang.IllegalArgumentException Thrown in case of XML related error (parsing
     *     error, for example)
     */
    ...
    /**
     * Creates RrdDefTemplate object from the string containing XML template. Read general
     * information for this class to see an example of a properly formatted XML source.
     *
    ...
    /**
     * Creates RrdDefTemplate object from the file containing XML template. Read general information
     * for this class to see an example of a properly formatted XML source.
     *
    ...
    /**
     * Returns RrdDef object constructed from the underlying XML template. Before this method is
     * called, values for all non-optional placeholders must be supplied. To specify placeholder
     * values at runtime, use some of the overloaded {@link
     ...
     *     object.
     * @throws java.lang.IllegalArgumentException Thrown (in most cases) if the value for some
     *     placeholder was not supplied through {@link
     *     org.rrd4j.core.XmlTemplate#setVariable(String, String) setVariable()} method call
     */
    public RrdDef getRrdDef() {
        ...
        validateTagsOnlyOnce(
                root, new String[] {"path*", "uri*", "start", "step", "datasource*", "archive*"});
