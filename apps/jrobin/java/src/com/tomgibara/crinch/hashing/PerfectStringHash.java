...
/**
 * A "minimal perfect hash" for Strings. After construction with an array of <em>n</em> unique
 * non-null strings, an instance of this class will return a unique hash value <em>h</em> (0 &lt;= h
 * &lt; n) for any string <em>s</em> in the array. A negative has value will typically be returned
 * for a string that is not in the array.
 *
 * <p>However, the supplied array is <em>not</em> retained. This means that the implementation
 * cannot necessarily confirm that a string is not in the supplied array. Where this implementation
 * cannot distinguish that a string is not in the array, a 'valid' hash value may be returned. Under
 * no circumstances will a hash value be returned that is greater than or equal to <em>n</em>.
 *
 * <p><strong>IMPORTANT NOTE:</strong> The array of strings supplied to the constructor will be
 * mutated: it is re-ordered so that <code>hash(a[i]) == i</code>. Application code must generally
 * use this information to map hash values back onto the appropriate string value.
 *
 * <p><strong>NOTE:</strong> Good performance of this algorithm is predicated on string hash values
 * being cached by the <code>String</code> class. Experience indicates that is is a good assumption.
 *
 * @author Tom Gibara
 */
public class PerfectStringHash implements Hash<String> {
    ...
    /**
     * Comparator used to order the supplied string array. Hashcodes take priority, we will do a
     * binary search on those. Otherwise, lengths take priority over character ordering because the
     * hash algorithm prefers to compare lengths, it's faster.
     */
    ...
    /**
     * Builds a (typically v. small) decision tree for distinguishing strings that share the same
     * hash value.
     *
     * @param values the string values to distinguish
     * @param start the index from which the values should be read
     * @param length the number of string values that need to be distinguished
     * @param pivots the array that will hold our decision nodes
     * @param pivotIndex the index at which the tree should be written
     */
    private static void generatePivots(
            String[] values, int start, int length, int[] pivots, int pivotIndex) {
                ...
                if (j >= length - 1) {
                    ...
                    if (l1 == l2) {
                        ...
                        for (int k = 0; k < l1; k++) {
                            ...
                            if (c1 == c2) continue;
                            if (c1 < c2) { // must occur at some point because we have already
                                // checked that the two strings are unequal
                                tPart = k;
                                tComp = c1;
                            } else {
                                // shouldn't be possible - we've sorted the strings to avoid this
                                // case
    ...
    /**
     * Stores two ints for every string, an offset into the pivot array (-1 if not necessary) and
     * the depth of the decision tree that is rooted there.
     */
    private final int[] offsets;

    /**
     * Stores two ints for every decision, the index at which a character comparison needs to be
     * made, followed by the character value to be compared against; or -1 to indicate a length
     * comparison, followed by the length to be compared against.
     */
     ...
     *
     * @param values an array of unique non-null strings that will be reordered such that <code>
     *     hash(values[i]) == i</code>.
     */
