...
class FilterDefinition {
    ...
    FilterDefinition(Threshold defaultThreshold, FilterDefinitionElement[] elements, Recorder[] recorders) {
        ...
        for (FilterDefinitionElement element : elements) maxSeconds = Math.max(maxSeconds, element.getThreshold().getSeconds());
        for (Recorder recorder : recorders) maxSeconds = Math.max(maxSeconds, recorder.getThreshold().getSeconds());
