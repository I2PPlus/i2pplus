...
public class I2PTunnelConnectClient extends I2PTunnelHTTPClientBase implements Runnable {
    ...
    private static final String ERR_BAD_PROTOCOL = "HTTP/1.1 405 Bad Method\r\n"
            ...
            + "Connection: close\r\n"
            + "\r\n"
            + "<html><body><H1>I2P ERROR: METHOD NOT ALLOWED</H1>The request uses a bad protocol. The Connect"
            + " Proxy supports CONNECT requests ONLY. Other methods such as GET are not allowed - Maybe you wanted"
            + " the HTTP Proxy?.<BR>";
    ...
    public void startRunning() {
        super.startRunning();
        if (open) _context.portMapper().register(PortMapper.SVC_HTTPS_PROXY, getTunnel().listenHost, getLocalPort());
    }
    ...
    protected void clientConnectionRun(Socket s) {
        ...
        try {
            ...
            while (true) {
                ...
                line = line.trim();
                if (_log.shouldDebug()) if (line != null) _log.debug(getPrefix(requestId) + "Request: " + line);
                ...
                if (method == null) { // first line CONNECT blah.i2p:80 HTTP/1.1
                    ...
                    if (_log.shouldDebug()) {
                        _log.debug(getPrefix(requestId) + "METHOD:" + method + ":\n" + "HOST:"
                                ...
                                + restofline + ":\n" + "DEST:"
                                + destination + ":\n" + "WWW Proxy? "
                                + usingWWWProxy + " Internal Proxy? " + usingInternalOutproxy);
                    }
                    ...
                    if (usingWWWProxy
                        ...
                        String user =
                                getTunnel().getClientOptions().getProperty(PROP_OUTPROXY_USER_PREFIX + currentProxy);
                        ...
                        if (user != null && pw != null) {
                            String proxyCreds = user + ':' + pw;
                            newRequest
                                    .append("Proxy-Authorization: Basic ")
                                    .append(Base64.encode(
                                            DataHelper.getUTF8(proxyCreds), true)) // true = use standard alphabet
                                    .append("\r\n");
                        }
                    }
                    newRequest.append("\r\n"); // HTTP spec
            ...
            if (usingInternalOutproxy) {
                ...
                OnTimeout onTimeout =
                        new OnTimeout(s, s.getOutputStream(), targetRequest, usingWWWProxy, currentProxy, requestId);
            ...
            if (!usingWWWProxy && remotePort > 0) sktOpts.setPort(remotePort);
