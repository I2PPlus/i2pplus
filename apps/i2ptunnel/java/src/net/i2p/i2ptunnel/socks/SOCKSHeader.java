...
public class SOCKSHeader {
    ...
    public String getHost() {
        ...
        if (addressType == AddressType.IPV4) return Addresses.toString(Arrays.copyOfRange(header, 4, 4));
        if (addressType == AddressType.IPV6) return Addresses.toString(Arrays.copyOfRange(header, 4, 16));
        return null;
    }
    ...
    public int getPort() {
        ...
        if (addressType == 3) namelen = 1 + (header[4] & 0xff);
        else if (addressType == 1) namelen = 4;
        else if (addressType == 4) namelen = 16;
        else return 0;
