...
public class IrcInboundFilter implements Runnable {
    ...
    public IrcInboundFilter(Socket lcl, I2PSocket rem, StringBuilder pong, Log log) {
     ...
     *  @since 0.8.9
     */
    public IrcInboundFilter(Socket lcl, I2PSocket rem, StringBuilder pong, Log log, DCCHelper helper) {
        local = lcl;
        remote = rem;
        expectedPong = pong;
        _log = log;
    ...
    public void run() {
        ...
        try {
            ...
            if (_log.shouldWarn()) _log.warn("[IRC Client] Inbound Filter: No streams", e);
            return;
        }
        if (_log.shouldDebug()) _log.debug("[IRC Client] Inbound Filter: Running");
        try {
            while (true) {
                try {
                    ...
                    if (outmsg != null) {
                        if (!inmsg.equals(outmsg)) {
                            ...
                            if (_log.shouldInfo()) _log.info("[IRC Client] Inbound message [" + outmsg + "]");
                        }
                        outmsg = outmsg + "\r\n"; // rfc1459 sec. 2.3
                        ...
                        if (_log.shouldWarn()) _log.warn("[IRC Client] Inbound message BLOCKED [" + inmsg + "]");
                    }
                } catch (IOException e1) {
                    if (_log.shouldInfo()) _log.info("[IRC Client] Inbound Filter: Disconnected", e1);
                    else if (_log.shouldWarn()) _log.warn("[IRC Client] Inbound Filter: Disconnected");
                    break;
                }
            }
        ...
        if (_log.shouldDebug()) _log.debug("[IRC Client] Inbound Filter: Stopped");
    }
}
