...
public class I2PTunnelHTTPServer extends I2PTunnelServer {
    ...
    private static final String EXPIRES_HEADER =
            "expires"; // not needed with cache-control max-age, and php inserts erroneous header
    ...
    private static final String X_PANTHEON_STYX_HOSTNAME_HEADER =
            "x-pantheon-styx-hostname"; // possible anonymity implications, informational
    ...
    private static final String X_STYX_REQ_ID_HEADER =
            "x-styx-req-id"; // possible anonymity implications, informational
    ...
    static final String ERR_NOT_FOUND = "HTTP/1.1 404 Not Found\r\n" + "Content-Type: text/html; charset=utf-8\r\n"
    ...
    private static final String ERR_UNAVAILABLE = "HTTP/1.1 503 Service Unavailable\r\n"
            ...
            + "<!doctype html>\n"
            + "<html>\n"
            + "<head><title>503 Service Temporarily Unavailable</title><meta name=color-scheme content=\"light"
            + " dark\"></head>\n"
            + "<body>\n"
    ...
    static final String ERR_FORBIDDEN = "HTTP/1.1 403 Denied\r\n" + "Content-Type: text/html; charset=utf-8\r\n"
            + "Cache-Control: no-cache\r\n"
            + "Connection: close\r\n"
            + "\r\n"
            + "<!doctype html>\n"
    ...
    */
    ...
    private static final String ERR_REQUEST_URI_TOO_LONG = "HTTP/1.1 414 Request URI too long\r\n"
            ...
            + "<!doctype html>\n"
            + "<html>\n"
            + "<head><title>414 Request URI Too Long</title><meta name=color-scheme content=\"light"
            + " dark\"></head>\n"
    ...
    private static final String ERR_HEADERS_TOO_LARGE = "HTTP/1.1 431 Request header fields too large\r\n"
            ...
            + "<!doctype html>\n"
            + "<html>\n"
            + "<head><title>431 Request Header Fields Too Large</title><meta name=color-scheme content=\"light"
            + " dark\"></head>\n"
    ...
    private void setupI2PTunnelHTTPServer(String spoofHost) {
        ...
        getTunnel()
                ...
                .createRateStat(
                        ...
                        "Tunnels [HTTPServer]",
                        new long[] {60 * 1000, 10 * 60 * 1000, 3 * 60 * 60 * 1000});
    ...
    private void setupPostThrottle() {
        ...
        synchronized (this) {
            if (pp != 0 || pt != 0 || _postThrottler != null) {
                ...
                if (_postThrottler == null) _postThrottler = new ConnThrottler(pp, pt, pw, pb, px, "POST/PUT", _log);
    ...
    private final boolean shouldAddResponseHeaderReferrerPolicy() {
        Properties opts = getTunnel().getClientOptions();
        boolean addReferrerPolicyHeader =
                Boolean.parseBoolean(opts.getProperty(OPT_ADD_RESPONSE_HEADER_REFERRER_POLICY));
    ...
    protected void blockingHandle(I2PSocket socket) {
        ...
        Hash peerHash = socket.getPeerDestination() != null
                ? socket.getPeerDestination().calculateHash()
                : null;
        if (peerHash == null) {
            return;
        }
        ...
        if (_log.shouldDebug()) {
            _log.debug("[HTTPServer] Incoming connection to " + toString().replace("/", "") + " (Port "
        ...
        try {
            if (socket.getLocalPort() == 443) {
                ...
                Runnable t =
                        new I2PTunnelRunner(s, socket, slock, null, null, null, (I2PTunnelRunner.FailCallback) null);
            ...
            do {
                ...
                try {
                     ...
                     */
                    long timeout = requestCount > 0
                            ? I2PTunnelHTTPClient.BROWSER_KEEPALIVE_TIMEOUT + 10 * 1000
                            : HEADER_TIMEOUT;
                    headers = readHeaders(
                            socket,
                            null,
                    ...
                    if (requestCount > 0) {
                        ...
                        if (_log.shouldWarn()) {
                            if (ste.getMessage() != null) {
                                _log.warn(
                                        "[HTTPServer] Request error: " + ste.getMessage() + " \n* Client: " + peerB32);
                            }
                        }
                    }
                    try {
                    ...
                    if (requestCount > 0) {
                        ...
                        if (_log.shouldWarn()) {
                            if (eofe.getMessage() != null) {
                                _log.warn(
                                        "[HTTPServer] Request error: " + eofe.getMessage() + " \n* Client: " + peerB32);
                            }
                        }
                    }
                    try {
                ...
                if (_log.shouldDebug()) {
                    _log.debug("[HTTPServer] Incoming request for: " + hostname + "\n* Client: " + peerB32);
                }
                if (hostname != null && !hostname.endsWith(".i2p") && !hostname.endsWith(".onion")) {
                    ...
                    if (address != null) {
                        if (address.isLinkLocalAddress()
                            ...
                            if (_log.shouldWarn()) {
                                _log.warn("[HTTPServer] WARNING! Attempt to access localhost or loopback address via ["
                                        + hostname + "]" + " -> Adding dest to clients blocklist file \n* Client: "
                                        + peerB32);
                            }
                            ...
                            if (_log.shouldInfo() && !hostname.equals(address.getHostAddress())) {
                                _log.info("[HTTPServer] Hostname " + hostname + " validated" + " -> Resolves to: "
                        ...
                        if (blockAgents != null) {
                            ...
                            for (int i = 0; i < agents.length; i++) {
                                ...
                                if (ag.equals("none")) {
                                    if (_log.shouldWarn()) {
                                        _log.warn(
                                                "[HTTPServer] Refusing access: User Agent header is blank \n* Client: "
                                                        + peerB32);
                                    }
                                    try {
                                        sendError(socket, ERR_FORBIDDEN);
                ...
                if (command.length() > 0) {
                    processBlocklist(socket, command);
                }
                ...
                if (_log.shouldDebug()) {
                    _log.debug("[HTTPServer] Modified headers\n\t" + modifiedHeader);
                        ...
                        && !command.toString().toLowerCase(Locale.ROOT).contains("head")) {
                ...
                if (requestCount == 0) {
                    ...
                    getTunnel()
                            .getContext()
                            .statManager()
                            .addRateData("i2ptunnel.httpserver.blockingHandleTime", timeToHandle);
                    if ((timeToHandle > 1500) && (_log.shouldDebug())) {
                        _log.info("[HTTPServer] Took a while (" + timeToHandle + "ms) to handle the request for "
                                + remoteHost + ':' + remotePort + "\n* Client: "
                                + peerB32 + "\n* Tasks: Read headers: "
                                + (afterHeaders - afterAccept) + "ms; " + "Socket create: "
                                + (afterSocket - afterHeaders) + "ms; " + "Start runners: "
                                + (afterHandle - afterSocket) + "ms");
                    }
                }
                if (keepalive) { // Since we are now running the CompressedRequestor inline, if keepalive is true, we
                                 // don't need to wait.
                    if (_log.shouldDebug()) {
                        ...
                        _log.debug("[HTTPServer] Waited " + timeToWait + "ms for response [#" + requestCount
                                + "] to complete -> " + code);
                    }
            ...
            if (_log.shouldError()) _log.error("[HTTPServer] Out of Memory error (" + oom.getMessage() + ")");
        }
    }
        ...
        public void run() {
            ...
            try {
                ...
                if (!(isGet || isHead) || _upgrade || browserin.available() > 0) { // just in case
                    ...
                    _browser.setReadTimeout(
                            isPost
                                    ? SERVER_READ_TIMEOUT_MEDIUM
                                    : // medium
                ...
                Map<String, List<String>> headers =
                        readHeaders(null, serverin, command, SERVER_SKIPHEADERS, _ctx, timeout);
                ...
                if (_log.shouldDebug())
                    _log.debug("[HTTPServer] Running server-to-browser Compressed? " + _shouldCompress + " KeepAlive? "
                            + _keepalive
                            + (req != null && !req.equals("") && !req.equals("Unknown request")
                                    ? "\n* URL: " + req
                                    : ""));
                ...
                if (_log.shouldDebug()) {
                    _log.debug(
                            "Finished server-to-browser: Compressed? " + _shouldCompress + " KeepAlive? " + _keepalive
                                    + (req != null && !req.equals("") && !req.equals("Unknown request")
                                            ? "\n* URL: " + req
                                            : ""));
                }
            ...
            try {
                ...
                if (ioe.getMessage() != null) {
                    if (ioe.getMessage().indexOf("Input stream closed") >= 0
                        ...
                        if (_log.shouldDebug()) {
                            _log.debug("[HTTPServer] Error sending " + _name + " -> " + ioe.getMessage());
                        }
                    } else {
                        if (_log.shouldWarn()) {
    ...
    private static class CompressedResponseOutputStream extends HTTPResponseOutputStream {
        ...
        protected void finishHeaders() throws IOException {
            ...
            if (shouldCompress()) out.write(DataHelper.getASCII("Content-Encoding: x-i2p-gzip\r\n"));
            super.finishHeaders();
        }
     ...
     */
    public static Map<String, List<String>> readHeaders(
            I2PSocket socket,
            InputStream in,
    ...
    private static Map<String, List<String>> readHeadersInternal(
            I2PSocket socket,
            InputStream in,
            StringBuilder command,
            Set<String> skipHeaders,
            ...
            if ((buf.length() == 0) || ((buf.charAt(0) == '\n') || (buf.charAt(0) == '\r'))) {
                ...
                if (ctx.clock().now() > expire) {
                    throw new SocketTimeoutException(
                            "Timeout (" + (initialTimeout + HEADER_FINISH_TIMEOUT / 1000) + "s) receiving headers");
                }
                int split = buf.indexOf(":");
                if (split <= 0)
                    throw new BadRequestException(
                            "Invalid HTTP header, missing colon: \"" + buf + "\" request: \"" + command + '"');
                totalSize += buf.length();
                if (totalSize > MAX_TOTAL_HEADER_SIZE) throw new LineTooLongException("Request + headers too big");
                ...
                if ("accept-encoding".equals(lcName)) {
                    ...
                    if (socket != null) {
                        ...
                        throw new BadRequestException(
                                "Invalid HTTP header: \"" + name + "\" -> Terminating connection...");
                    }
                }
            ...
            try {
                if (socket != null) {
                    try {
                        ...
                        _log.error("[HTTPServer] Error closing socket (" + ioe.getMessage() + ")");
                    }
                    throw new BadRequestException(
                            command.toString() + "-> Matches blocklist entry \"" + matchedString + "\"");
                }
            } catch (BadRequestException bre) {
                throw bre;
            }
