...
class HTTPResponseOutputStream extends FilterOutputStream {
    ...
    public HTTPResponseOutputStream(
            OutputStream raw, boolean allowKeepAliveIn, boolean allowKeepAliveOut, boolean isHead, DoneCallback cb) {
        this(raw, cb);
        _keepAliveIn = allowKeepAliveIn;
        _keepAliveOut = allowKeepAliveOut;
        if (isHead) _dataExpected = 0;
        if (_log.shouldInfo())
    ...
    private void ensureCapacity() throws IOException {
        ...
        if (valid >= MAX_HEADER_SIZE)
            throw new IOException(
                    "Max header size (" + MAX_HEADER_SIZE + "B) exceeded -> " + valid + "B size header detected");
        ...
        if (valid + 1 >= len) {
            ...
            if (len == CACHE_SIZE) _cache.release(_headerBuffer);
    ...
    private boolean headerReceived() {
        ...
        if (third != NL) return false;
        byte first = data[valid - 3];
        if (first == NL) return true; // \n\r\n
    ...
    private void writeHeader() throws IOException {
        ...
        for (int i = 0; i < valid; i++) {
            if (data[i] == NL) {
                if (lastEnd == -1) {
                    ...
                    if (_log.shouldInfo()) _log.info("Response: " + responseLine.trim());
                    ...
                    if (sp > 0) {
                        ...
                        if (s.startsWith("1") || s.startsWith("204") || s.startsWith("304")) _dataExpected = 0;
                    } else {
                        // no status?
                        _keepAliveIn = false;
                        _keepAliveOut = false;
                    ...
                    for (int j = lastEnd + 1; j < i; j++) {
                        if (data[j] == ':') {
                            ...
                            String val;
                            if (valLen == 0) val = "";
                            else val = DataHelper.getUTF8(data, j + 2, valLen).trim();
                            ...
                            if ("connection".equals(lcKey)) {
                                ...
                                // Nonstandard, strip
                                    ...
                                    && "x-i2p-gzip".equals(val.toLowerCase(Locale.US))) {
                                _gzip = true;
                                // client side only
                                ...
                                // filter this hop-by-hop header; outproxy authentication must be configured in
                                // I2PTunnelHTTPClient
                                // see e.g.
                                ...
                                } else if ("transfer-encoding".equals(lcKey)
                                        && val.toLowerCase(Locale.US).contains("chunked")) {
        ...
        if (_keepAliveOut) {
            ...
            if ((chunked && _dataExpected >= 0) || (!chunked && _dataExpected < 0)) _keepAliveOut = false;
        }
        if (_keepAliveIn) {
            // we need one but not both
            if ((chunked && _dataExpected >= 0) || (!chunked && _dataExpected < 0)) _keepAliveIn = false;
        }

        if (!connectionSent && !_keepAliveOut) out.write(CONNECTION_CLOSE);

        finishHeaders();

        boolean shouldCompress = shouldCompress();
        if (_log.shouldInfo())
            _log.info("After headers: GZIP? " + _gzip + " Compressed? " + shouldCompress + " KeepAliveIn? "
                    + _keepAliveIn + " KeepAliveOut? " + _keepAliveOut);

        if (data.length == CACHE_SIZE) _cache.release(_headerBuffer);
        _headerBuffer = null;
        ...
        if (_keepAliveIn && !shouldCompress) {
            if (_dataExpected > 0) {
                ...
                if (_callback != null) _callback.streamDone();
            } else {
                // -1, chunked
    ...
    public void close() throws IOException {
        if (_log.shouldInfo())
            _log.info("Closing " + out + " -> Compressed? " + shouldCompress() + " KeepAliveIn? " + _keepAliveIn
                    + " KeepAliveOut? " + _keepAliveOut);
        synchronized (this) {
