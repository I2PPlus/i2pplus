...
build_jbigi() {
    ...
    $CC -c -fPIC -Wall $MARCH_FLAG -I. -I./include -I./include/linux-aarch64 -I$GMP_DIR -o jbigi.o src/jbigi.c
    $CC -shared -fPIC -Wall $MARCH_FLAG -I. -I./include -I./include/linux-aarch64 -I$GMP_DIR -o libjbigi.so jbigi.o $GMP_DIR/.libs/libgmp.a -Wl,-soname,libjbigi.so
...
echo "Building ${SUFFIX#_}-bit ARM64 Linux .so files..."
echo ""
...
ls -1 "$OUTPUT_DIR/"*linux*"$SUFFIX"* 2>/dev/null | grep -v intel | grep -v amd | while read f; do
    printf "  "
    file "$f"
done
