...
public final class KeyStoreUtil {
    ...
    public static boolean logCertExpiration(KeyStore ks, String location, long expiresWithin) {
        ...
        try {
            ...
            if (count == 0) error("No certificates found in key store " + location, null);
        } catch (GeneralSecurityException e) {
                        ...
                        if (enc != null) {
                            ...
                            if (_blacklist.contains(new SHA1Hash(h))) {
                                ...
                                if (!_blacklistLogged) {
                                    ...
                                    warn(
                                            "Ignoring blacklisted certificate: " + alias + "\n* CN: " + cn + "\n* OU: "
                                                    + ou + "\n* S/N: " + serial.toString(16),
                                            null);
                                }
                            }
                        } else {
            ...
            if (files != null) {
                ...
                for (int i = 0; i < files.length; i++) {
                    ...
                    if (alias.endsWith(".crt")
                            ...
                            || alias.endsWith(".pfx")
                            || alias.endsWith(".p12")
                            || alias.endsWith(".cer")) alias = alias.substring(0, alias.length() - 4);
    ...
    public static boolean createKeys(File ks, String alias, String cname, String ou, String keyPW) {
        final boolean isCA =
                !cname.contains("@") && !cname.endsWith(".family.i2p.net") && SigType.ECDSA_SHA256_P256.isAvailable();
        ...
        return createKeys(
                ks, DEFAULT_KEYSTORE_PASSWORD, alias, cname, null, ou, DEFAULT_KEY_VALID_DAYS, alg, sz, keyPW);
    }
    ...
    public static boolean createKeys(
        ...
        final int sz = isCA ? DEFAULT_CA_KEY_SIZE : DEFAULT_KEY_SIZE;
        return createKeys(
                ks, DEFAULT_KEYSTORE_PASSWORD, alias, cname, altNames, ou, DEFAULT_KEY_VALID_DAYS, alg, sz, keyPW);
    }
    ...
    public static boolean createKeys(
        ...
        if (useKeytool) {
            if (altNames != null) throw new IllegalArgumentException("can't do SAN in keytool");
    ...
    public static X509Certificate renewPrivateKeyCertificate(
        ...
        try {
            ...
            if (alias == null) {
                ...
                if (e.hasMoreElements()) alias = e.nextElement();
                if (alias == null) throw new GeneralSecurityException("no private keys found");
            }
            PrivateKey pk = (PrivateKey) keyStore.getKey(alias, keypwchars);
            if (pk == null) throw new GeneralSecurityException("private key not found: " + alias);
            Certificate[] certs = keyStore.getCertificateChain(alias);
            if (certs.length != 1) throw new GeneralSecurityException("Bad cert chain length");
                ...
                if (ks != null) {
                    ...
                    if (ksf != null && ksf.isDirectory()) {
                        ...
                        if (count > 0) {
                            ...
                            if (count > 0) System.out.println("Found " + count + " blacklisted certs in " + ksf);
                        }
                    }
                } else {
    ...
    private static void usage() {
        System.err.println("Usage: KeyStoreUtil create keystore.ks (create empty keystore)\n"
                + "       KeyStoreUtil export keystore.ks alias keypw (exports private key from keystore)\n"
                + "       KeyStoreUtil import keystore.ks file.key alias keypw (imports private key from file to"
                + " keystore)\n"
    ...
    private static void testKeygen(String[] args) throws Exception {
        ...
        String alias = args[2];
        String pw = args[3];
        boolean ok = createKeys(
                ksf,
                DEFAULT_KEYSTORE_PASSWORD,
                alias + "-EC",
                "test cname",
                "test ou",
                DEFAULT_KEY_VALID_DAYS,
                "EC",
                256,
                pw);
    ...
    private static void listKeys(String[] args) {
        ...
        if (ksf.exists()) {
            ...
            try {
                ...
                for (Enumeration<String> e = ks.aliases(); e.hasMoreElements(); ) {
                    String alias = e.nextElement();
                    if (ks.isCertificateEntry(alias)) System.out.println(alias);
                }
                System.out.println("\nPrivate keys:");
                for (Enumeration<String> e = ks.aliases(); e.hasMoreElements(); ) {
                    String alias = e.nextElement();
                    if (ks.isKeyEntry(alias)) System.out.println(alias);
                }
            } catch (IOException ioe) {
