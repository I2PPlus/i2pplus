...
public final class Blinding {
    ...
    public static SigningPublicKey blind(SigningPublicKey key, SigningPrivateKey alpha) {
        ...
        if ((type != TYPE && type != TYPER) || alpha.getType() != TYPER) throw new IllegalArgumentException("Unsupported blinding from " + type + " to " + alpha.getType());
        try {
    ...
    public static SigType getDefaultBlindedType(SigType unblindedType) {
        if (unblindedType == TYPE) return TYPER;
    ...
    public static BlindData decode(I2PAppContext ctx, String address) throws IllegalArgumentException {
        ...
        if (!address.endsWith(".b32.i2p")) throw new IllegalArgumentException("Not a .b32.i2p address");
        byte[] b = Base32.decode(address.substring(0, address.length() - 8));
        if (b == null) throw new IllegalArgumentException("Corrupt b32 address");
        if (b.length < 35) throw new IllegalArgumentException("Not a new-format address");
        return decode(ctx, b);
    }
    ...
    public static BlindData decode(I2PAppContext ctx, byte[] b) throws IllegalArgumentException {
        ...
        b[2] ^= (byte) (check >> 16);
        int flag = b[0] & 0xff;
        if ((flag & 0xf8) != 0) throw new IllegalArgumentException("Corrupt b32 address (or unsupported options)");
        if ((flag & FLAG_TWOBYTE) != 0) throw new IllegalArgumentException("Two byte signature types unsupported");
        // TODO two-byte sigtypes
        int st1 = b[1] & 0xff;
        int st2 = b[2] & 0xff;
        SigType sigt1 = SigType.getByCode(st1);
        SigType sigt2 = SigType.getByCode(st2);
        if (sigt1 == null) throw new IllegalArgumentException("Unsupported signature type " + st1);
        if (!sigt1.isAvailable()) throw new IllegalArgumentException("Unavailable signature type " + sigt1);
        if (sigt2 == null) throw new IllegalArgumentException("Unsupported blinded signature type " + st2);
        if (!sigt2.isAvailable()) throw new IllegalArgumentException("Unavailable blinded signature type " + sigt2);
        // todo secret/privkey
        int spkLen = sigt1.getPubkeyLen();
        if (3 + spkLen > b.length) throw new IllegalArgumentException("b32 too short");
        byte[] spkData = new byte[spkLen];
        System.arraycopy(b, 3, spkData, 0, spkLen);
        SigningPublicKey spk = new SigningPublicKey(sigt1, spkData);
        if (3 + spkLen != b.length) throw new IllegalArgumentException("b32 too long");
        BlindData rv = new BlindData(ctx, spk, sigt2, null);
        if ((flag & FLAG_SECRET) != 0) rv.setSecretRequired();
        if ((flag & FLAG_AUTH) != 0) rv.setAuthRequired();
        return rv;
    }
    ...
    public static String encode(SigningPublicKey key, boolean requireSecret, boolean requireAuth) throws IllegalArgumentException {
        ...
        long check = crc.getValue();
        // TODO two-byte sigtypes
        if (requireSecret) b[0] = FLAG_SECRET;
        if (requireAuth) b[0] |= FLAG_AUTH;
