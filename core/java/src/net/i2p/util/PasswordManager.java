...
public class PasswordManager {
    ...
    public boolean check(String realm, String user, String pw) {
        return checkPlain(realm, user, pw) || checkB64(realm, user, pw) || checkHash(realm, user, pw);
    }
...
/**
    ...
    public boolean checkHash(String shash, String pw) {
        ...
        // Try new rounds (1M) first
        byte[] pwHash = _context.keyGenerator().generateSessionKey(salt, DataHelper.getUTF8(pw)).getData();
        if (DataHelper.eqCT(hash, 0, pwHash, 0, SessionKey.KEYSIZE_BYTES)) {return true;}
        // Fall back to legacy rounds (1K) for migration
        pwHash = _context.keyGenerator().generateSessionKey(salt, DataHelper.getUTF8(pw), _context.keyGenerator().getPBERTLegacy()).getData();
