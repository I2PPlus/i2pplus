...
public abstract class I2PSessionImpl implements I2PSession, I2CPMessageReader.I2CPMessageEventListener {
    ...
    public I2PSession addSubsession(InputStream privateKeyStream, Properties opts) throws I2PSessionException {
        ...
        synchronized (_stateLock) {
            if (_state == State.OPEN) {_producer.connect(sub);} // else will be called in connect()
        }
        return sub;
    }
    ...
    private final Properties loadConfig(Properties opts) {
        ...
        if ((!_context.isRouterContext()) && _context.getBooleanProperty("i2cp.auth") &&
    ...
    private Properties filter(Properties options) {
        ...
        for (String key : options.stringPropertyNames()) {
            if (key.startsWith("java.") ||
                ...
                (key.startsWith("i2p.streaming.") && !isAllowedStreamingProperty(key)) ||
                key.startsWith("user.") ||
                key.startsWith("os.") ||
                key.startsWith("sun.") ||
                key.startsWith("awt.") ||
     ...
     */
    private boolean isAllowedStreamingProperty(String key) {
        return key.equals("i2p.streaming.profile") ||
               key.equals("i2p.streaming.connectDelay") ||
               key.equals("i2p.streaming.readTimeout") ||
               key.equals("i2p.streaming.inactivityTimeout") ||
               key.equals("i2p.streaming.answerPings") ||
               key.equals("i2p.streaming.maxWindowSize") ||
               key.equals("i2p.streaming.congestionAvoid") ||
               key.equals("i2p.streaming.delayWindowed") ||
               key.equals("i2p.streaming.delayDirect") ||
               key.equals("i2p.streaming.maxConcurrentStreams") ||
               key.equals("i2p.streaming.initialWindowSize");
    }
    ...
    void setLeaseSet(LeaseSet ls) {
        _leaseSet = ls;
        if (ls != null) {
            synchronized (_leaseSetWait) {_leaseSetWait.notifyAll();}
            ...
            synchronized (_lookupCache) {_lookupCache.put(h, d);}
        }
    }
    ...
    private void readDestination(InputStream destKeyStream) throws DataFormatException, IOException {
        ...
        if (etype != EncType.ELGAMAL_2048) {_privateKey = new PrivateKey(etype);}
        ...
        if (_signingPrivateKey.isOffline()) {
            ...
            if (type == null) {throw new DataFormatException("Unsupported transient signature type: " + itype);}
    ...
    public void connect() throws I2PSessionException {
        ...
        try {
            ...
            Thread notifier = new I2PAppThread(_availabilityNotifier, "ClntNtfy." + getName(), true);
            notifier.start();
            startIdleMonitor();
            startVerifyUsage();
            success = true;
    ...
    void sendMessage_unchecked(I2CPMessage message) throws I2PSessionException {
        if (_queue != null) {
            ...
            else {writer.addMessage(message);}
        }
    }
    ...
    private void locked_closeSocket() {
        if (_reader != null) {_reader.stopReading(); _reader = null;}
        if (_queue != null) {_queue.close();} // internal
    ...
    protected void disconnect() {
        ...
        closeSocket();
        changeState(State.CLOSED);
        synchronized (_leaseSetWait) {_leaseSetWait.notifyAll();} // break out of wait for initial LS in connect()
    }
    ...
    private LookupWaiter x_lookupDest(String name, long maxWait) throws I2PSessionException {
        ...
        if (name.length() >= 256 && !_context.isRouterContext()) {return null;} // won't fit in Mapping
        synchronized (_lookupCache) {
