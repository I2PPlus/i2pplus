...
public class SendMessageDirectJob extends JobImpl {
     ...
     * @param msgIDBloomXor value to xor the messageID with before passing to the InNetMessagePool, may be 0
     */
    public SendMessageDirectJob(RouterContext ctx, I2NPMessage message, Hash toPeer, ReplyJob onSuccess, Job onFail, MessageSelector selector, int timeoutMs, int priority, long msgIDBloomXor) {
     ...
     * @param msgIDBloomXor value to xor the messageID with before passing to the InNetMessagePool, may be 0
     */
    public SendMessageDirectJob(RouterContext ctx, I2NPMessage message, Hash toPeer, Job onSend, ReplyJob onSuccess, Job onFail, MessageSelector selector, int timeoutMs, int priority, long msgIDBloomXor) {
        super(ctx);
        ...
        _targetHash = toPeer;
        if (timeoutMs < 5 * 1000) {
            if (_log.shouldWarn()) _log.warn("Very little time (" + timeoutMs + "ms) allocated to send direct message -> Setting to 10s...");
    ...
    public void runJob() {
        ...
        if (_expiration < now) {
            if (_log.shouldWarn()) _log.warn("Timed out sending direct message to [" + _targetHash.toBase64().substring(0, 6) + "]" + "\n* Expires: " + Instant.ofEpochMilli(_expiration) + "\n* " + _message);
            if (_onFail != null) getContext().jobQueue().addJob(_onFail);
            return;
        }

        if (_router != null) {
            if (_log.shouldDebug()) _log.debug("Router specified, sending direct message...");
            send();
        } else {
            _router = getContext().netDb().lookupRouterInfoLocally(_targetHash);
            if (_router != null) {
                if (_log.shouldDebug()) _log.debug("Router not specified but lookup found it");
                send();
            } else {
                if (!_alreadySearched) {
                    if (_log.shouldDebug()) _log.debug("Router not specified, so we're looking for it...");
                    ...
                    if (_log.shouldWarn()) _log.warn("Unable to find router [" + _targetHash.toBase64().substring(0, 6) + "] to send to after searching for " + (getContext().clock().now() - _searchOn) + "ms \n* " + _message);
                    if (_onFail != null) getContext().jobQueue().addJob(_onFail);
                }
            }
        }
    ...
    private void send() {
        if (_sent) {
            if (_log.shouldWarn()) _log.warn("Message already sent, not resending...", new Exception("blah"));
            return;
        }
        _sent = true;
        ...
        if (us.equals(to)) {
            ...
            if (_log.shouldDebug()) _log.debug("Adding " + _message.getClass().getName() + " to Inbound message pool as it was destined for ourselves");
            ...
            if (_log.shouldDebug()) _log.debug("Adding " + _message.getClass().getName() + " to Outbound message pool targeting [" + _router.getIdentity().getHash().toBase64().substring(0, 6) + "]");
