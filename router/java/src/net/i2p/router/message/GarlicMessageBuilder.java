...
public class GarlicMessageBuilder {
    ...
    static boolean needsTags(RouterContext ctx, PublicKey key, Hash local, int minTagOverride) {
        if (LeaseSetKeys.SET_EC_PQ_ALL.contains(key.getType())) return false;
    ...
    public static GarlicMessage buildMessage(RouterContext ctx, GarlicConfig config, SessionKey wrappedKey, Set<SessionTag> wrappedTags, int numTagsToDeliver, int lowTagsThreshold, SessionKeyManager skm) {
        ...
        if (key == null) {
            ...
            if (ri == null) throw new IllegalArgumentException("Null recipient specified");
            RouterIdentity ident = ri.getIdentity();
            if (ident == null) throw new IllegalArgumentException("Null recipient.identity specified");
            PublicKey pk = ident.getPublicKey();
            if (pk == null) throw new IllegalArgumentException("Null recipient.identity.publicKey specified");
            key = pk;
        }
        if (key.getType() != EncType.ELGAMAL_2048) throw new IllegalArgumentException();

        if (log.shouldInfo()) log.info("Encrypted with public key to expire on " + Instant.ofEpochMilli(config.getExpiration()));
        ...
        if (numTagsToDeliver > 0 && skm.shouldSendTags(key, curKey, lowTagsThreshold)) {
            ...
            if (log.shouldInfo()) log.info("Too few tags available so we're including " + numTagsToDeliver);
        }
    ...
    public static GarlicMessage buildMessage(RouterContext ctx, GarlicConfig config, Set<SessionTag> wrappedTags, PublicKey target, SessionKey encryptKey, SessionTag encryptTag) {
        ...
        if (encData == null) {
            if (log.shouldWarn()) log.warn("ElGamal encrypt fail");
            return null;
        }
        msg.setData(encData);
        ...
        if (timeFromNow < 1 * 1000) {
            if (log.shouldDebug()) log.debug("Building a message expiring in " + timeFromNow + "ms: " + config, new Exception("created by"));
            return null;
        }
        if (log.shouldDebug()) log.debug("Built ElGamal CloveSet (" + config.getCloveCount() + " cloves " + cloveSet.length + " bytes) in " + msg);
        return msg;
    }
    ...
    public static GarlicMessage buildMessage(RouterContext ctx, GarlicConfig config, SessionKey encryptKey, RatchetSessionTag encryptTag) {
        ...
        msg.setMessageExpiration(config.getExpiration());
        Log log = ctx.logManager().getLog(GarlicMessageBuilder.class);
        if (log.shouldDebug()) log.debug("Built ECIES CloveSet (" + config.getCloveCount() + " cloves) in " + msg);
        return msg;
    }
    ...
    static GarlicMessage buildECIESMessage(RouterContext ctx, GarlicConfig config, Hash from, Destination to, SessionKeyManager skm, ReplyCallback callback) {
        ...
        if (!LeaseSetKeys.SET_EC_PQ_ALL.contains(type)) throw new IllegalArgumentException();
        Log log = ctx.logManager().getLog(GarlicMessageBuilder.class);
        GarlicMessage msg = new GarlicMessage(ctx);
        CloveSet cloveSet = buildECIESCloveSet(ctx, config);
        LeaseSetKeys lsk = ctx.keyManager().getKeys(from);
        if (lsk == null) {
            if (log.shouldWarn()) log.warn("No LSK for " + from.toBase32());
            return null;
        }
        PrivateKey priv = lsk.getDecryptionKey(type);
        if (priv == null) {
            if (log.shouldWarn()) log.warn("No key for " + from.toBase32());
            return null;
        }
        ...
        if (skm instanceof RatchetSKM) {
            ...
            if (log.shouldWarn()) log.warn("No SKM for " + from.toBase32());
            return null;
        }
        byte encData[] = ctx.eciesEngine().encrypt(cloveSet, key, to, priv, rskm, callback);
        if (encData == null) {
            if (log.shouldWarn()) log.warn("Encrypt fail for " + from.toBase32());
            return null;
        }
        msg.setData(encData);
        msg.setMessageExpiration(config.getExpiration());
    ...
    public static GarlicMessage buildECIESMessage(RouterContext ctx, GarlicConfig config) {
        PublicKey key = config.getRecipientPublicKey();
        if (!LeaseSetKeys.SET_EC_PQ_ALL.contains(key.getType())) throw new IllegalArgumentException();
        Log log = ctx.logManager().getLog(GarlicMessageBuilder.class);
        GarlicMessage msg = new GarlicMessage(ctx);
        CloveSet cloveSet = buildECIESCloveSet(ctx, config);
        byte encData[] = ctx.eciesEngine().encrypt(cloveSet, key);
        if (encData == null) {
            if (log.shouldWarn()) log.warn("Encrypt fail for " + config);
            return null;
        }
        msg.setData(encData);
        msg.setMessageExpiration(config.getExpiration());
