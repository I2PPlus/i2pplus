...
public class TransportManager implements TransportEventListener {
    ...
    private void initializeAddress(Collection<Transport> ts) {
        ...
        // we only use these if we don't have a preferred address
    ...
    synchronized void startListening() {
        ...
        // What firewall supports UPnP and is configured with a public address on the LAN side? Unlikely.
        // Always start on Android, as we may have a cellular IPv4 address but are routing all traffic through WiFi.
    ...
    public void renderStatusHTML(Writer out, String urlBase, int sortFlags) throws IOException {
        if (SystemVersion.isAndroid()) {
            out.write("<div id=upnp class=tablewrap><p class=infohelp id=upnpstatus>" + _t("Cannot enumerate UPnP devices on Android.") +
                      "</p></div>\n");
        } else if (_upnpManager != null) {out.write(_upnpManager.renderStatusHTML());}
        else {
            out.write("<div id=upnp class=tablewrap><p class=infohelp id=upnpstatus>" + _t("UPnP is not enabled")
