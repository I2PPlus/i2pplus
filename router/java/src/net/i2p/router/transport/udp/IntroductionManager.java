...
class IntroductionManager {
    ...
    public IntroductionManager(RouterContext ctx, UDPTransport transport) {
        ...
        ctx.statManager()
                .createRateStat("udp.relayBadIP", "Received IP or port was bad", "Transport [UDP]", UDPTransport.RATES);
    }
    ...
    public void add(PeerState peer) {
        ...
        boolean added = id > 0;
        if (added) _outbound.put(Long.valueOf(id), peer);
    ...
    public int pickInbound(RouterAddress current, boolean ipv6, Properties ssuOptions, int howMany) {
        if (_log.shouldDebug()) _log.debug("Picking inbound out of " + _inbound.size());
        ...
        if (current != null) {
            ...
            for (int i = 0; i < ua.getIntroducerCount(); i++) {
                ...
                if (lexp > 0 && lexp < now + UDPTransport.INTRODUCER_EXPIRATION_MARGIN) continue;
                long tag = ua.getIntroducerTag(i);
                if (!isInboundTagValid(tag)) continue;
                ...
                ssu2count++;
                if (_log.shouldInfo()) _log.info("Reusing introducer: " + ua.getIntroducerHash(i));
        ...
        for (int i = 0; i < sz && found < howMany; i++) {
            ...
            for (Introducer intro : introducers) {
                if (b64.equals(intro.shash)) continue outerloop;
            }
            RouterInfo ri = _context.netDb().lookupRouterInfoLocally(hash);
            if (ri == null) {
                if (_log.shouldInfo()) _log.info("No local RouterInfo for selected peer [" + cur + "]");
            ...
            if (ras.isEmpty()) {
                if (_log.shouldInfo()) _log.info("Selected peer has no SSU address: " + ri);
                continue;
            }
            if (_context.banlist().isBanlisted(hash) || _transport.wasUnreachable(hash)) {
                if (_log.shouldInfo()) _log.info("Router is failing, blocklisted or was unreachable: " + cur);
                continue;
            }
            ...
            if (cur.getLastReceiveTime() < inactivityCutoff
                ...
                if (_log.shouldLog(Log.INFO)) _log.info("Router has been idle too long: " + cur);
                continue;
            }
            int oldFound = found;
            for (RouterAddress ra : ras) {
                ...
                int port = ra.getPort();
                if (!isValid(ip, port, true)) continue;
                ...
                if (((!ipv6 && ip.length == 16) || (ipv6 && ip.length == 4))
                    ...
                    if (_log.shouldInfo())
                        _log.info(
                                "Would have picked IPv6 introducer for IPv4 or IPv4 introducer for IPv6 but they don't"
                                        + " support it: "
                                        + cur);
                    continue;
                }
            ...
            if (oldFound != found && _log.shouldInfo()) _log.info("Selecting introducer: " + cur);
        }
    ...
    public void pingIntroducers() {
        ...
        for (Iterator<PeerState> iter = _inbound.values().iterator(); iter.hasNext(); ) {
            ...
            if (cur.getIntroducerTime() > pingCutoff && cur.getLastSendOrPingTime() < inactivityCutoff) {
                if (_log.shouldDebug()) _log.debug("Pinging introducer: " + cur);
    ...
    void receiveRelayRequest(PeerState2 alice, byte[] data) {
        ...
        if (skew > MAX_SKEW || skew < 0 - MAX_SKEW) {
            if (_log.shouldWarn()) _log.warn("Too skewed for relay req from " + alice);
            return;
        }
        int ver = data[12] & 0xff;
        if (ver != 2) {
            if (_log.shouldWarn()) _log.warn("Bad relay req version " + ver + " from " + alice);
            return;
        }
        ...
        int rcode;
        if (charlie == null) {
            if (_log.shouldDebug()) _log.debug("Relay tag not found " + tag + " from " + alice);
            rcode = SSU2Util.RELAY_REJECT_BOB_NO_TAG;
        } else if (charlie.getVersion() != 2) {
            if (_log.shouldWarn()) _log.warn("Receive SSU2 relay request from " + alice + " for SSU1 " + charlie);
            // add a code for this?
            rcode = SSU2Util.RELAY_REJECT_BOB_NO_TAG;
        } else {
            ...
            if (isDup) {
                ...
                if (_log.shouldInfo())
                    _log.info("Dropping dup relay request from " + alice + " for tag " + tag + " nonce " + nonce
                            + " time " + time + " and relaying with " + charlie);
                return;
            }
            ...
            if (aliceRI != null) {
                ...
                if (SSU2Util.validateSig(
                        _context,
                        SSU2Util.RELAY_REQUEST_PROLOGUE,
                        _context.routerHash(),
                    ...
                    if (_log.shouldWarn()) _log.warn("Signature failed relay request\n" + aliceRI);
                ...
                if (_log.shouldWarn()) _log.warn("Alice's RouterInfo not found " + alice);
        ...
        if (rcode == SSU2Util.RELAY_ACCEPT) {
            ...
            if (_log.shouldInfo())
                _log.info("Received relay request from " + alice + " for tag " + tag + " nonce " + nonce + " time "
                        + time + " and relaying with " + charlie);
            ...
            int avail = charlie.getMTU()
                    - ((charlie.isIPv6()
                                    ? PacketBuilder2.MIN_IPV6_DATA_PACKET_OVERHEAD
                                    : PacketBuilder2.MIN_DATA_PACKET_OVERHEAD)
                            + SSU2Payload.BLOCK_HEADER_SIZE
                            + // relay intro block header
                            idata.length
                            + // relay intro block payload
                            SSU2Payload.BLOCK_HEADER_SIZE
                            + // RI block header
        ...
        try {
            ...
            data = SSU2Util.createRelayResponseData(_context, _context.routerHash(), rcode, nonce, null, 0, spk, 0);
            if (data == null) {
                if (_log.shouldWarn()) {
    ...
    private boolean receiveRelayIntro(PeerState2 bob, Hash alice, byte[] data, RouterInfo aliceRI) {
        ...
        if (_transport.isSymNatted()) {
        ...
        } else if (_context.banlist().isBanlisted(alice) || _context.blocklist().isBlocklisted(testIP)) {
            ...
            // bob should have sent it to us. Don't bother to lookup remotely if he didn't, or it was lost.
            if (aliceRI != null) {
                // validate signed data
                SigningPublicKey spk = aliceRI.getIdentity().getSigningPublicKey();
                if (SSU2Util.validateSig(
                ...
                if (_log.shouldWarn()) {
                    _log.warn("Alice's RouterInfo  [" + alice + "] not found for relay intro from " + bob);
                }
        ...
        SigningPrivateKey spk = _context.keyManager().getSigningPrivateKey();
        data = SSU2Util.createRelayResponseData(
                _context, bob.getRemotePeer(), rcode, nonce, ourIP, ourPort, spk, token);
        if (data == null) {
            if (_log.shouldWarn()) {
        ...
        if (rcode == SSU2Util.RELAY_ACCEPT) {
            ...
            if (_log.shouldDebug()) {
                _log.debug("[SSU] Sending hole punch to "
                        + Addresses.toString(testIP, testPort).replace("/", ""));
            }
    ...
    void receiveRelayResponse(PeerState2 peer, int status, byte[] data) {
        ...
        if (alice != null) {
            ...
            if (charlie != null) {
                byte[] signedData;
                if (status == 0) signedData = Arrays.copyOfRange(data, 0, data.length - 8); // token
                else signedData = data;
                SigningPublicKey spk = charlie.getIdentity().getSigningPublicKey();
                if (SSU2Util.validateSig(
                        _context, SSU2Util.RELAY_RESPONSE_PROLOGUE, _context.routerHash(), null, signedData, spk)) {
                } else {
                    if (_log.shouldWarn()) _log.warn("Signature failed RelayReponse as Bob from Charlie:\n" + charlie);
                }
            } else {
                if (_log.shouldWarn()) _log.warn("Signer's RouterInfo not found " + peer);
            }
            ...
            try {
                ...
                if (_log.shouldDebug())
                    _log.debug("Received RelayReponse " + status + " as Bob, forwarding " + " nonce " + nonce
                            + " to Alice " + alice);
                _transport.send(packet);
                alice.setLastSendTime(now);
            } catch (IOException ioe) {
            ...
            if (isDup) {
                // very rare
                if (_log.shouldInfo())
                    _log.info("Dropping dup relay response as bob from charlie " + peer.getRemotePeer() + " for nonce "
                            + nonce);
                return;
            }
