...
public class PeerState {
    ...
    void removeACKMessage(Long messageId) {
            ...
            if (removed) {
                ...
                if (_log.shouldDebug())
                    _log.debug("Sent ACK [MsgID" + messageId + "] now " + _currentACKs.size() + " current and " +
    ...
    boolean acked(long messageId) {
        ...
        if (state != null) {
            ...
            if (_log.shouldDebug())
                _log.debug("Received ACK of [MsgID " + messageId + "] from [" + _remotePeer.toBase64().substring(0,6)
        ...
        if (state != null) {
            ...
            if (_log.shouldLog(Log.INFO))
                _log.info("Received partial ACK of [MsgID " + state.getMessageId() + "] from [" + _remotePeer.toBase64().substring(0,6)
                          + "] after " + lifetime + "ms and " + numSends + " send attempts " + bitfield + ": completely removed? "
                          + isComplete + ": " + state);
