    ...
    //}
        ...
        private void receivePacket(UDPPacketReader reader, UDPPacket packet, InboundEstablishState state, boolean allowFallback) {
            ...
            if (state.getMACKey() != null) {
                ...
                if (isValid) {
                    ...
                    if (_log.shouldLog(Log.WARN))
                        _log.warn("Invalid introduction packet received for Inbound connection, treating as non-establishment packet " + packet);
                    _state = 33;
                }
            }
            if (allowFallback) {
        ...
        private void receivePacket(UDPPacketReader reader, UDPPacket packet, OutboundEstablishState state) {
            ...
            if (isValid) {
                ...
                if (_log.shouldLog(Log.WARN))
                    _log.warn("Invalid introduction packet received for Outbound established connection with old intro key, treating as non-establishment packet " + packet);
            }
