...
public class TunnelPoolSettings {
    ...
    public boolean getAllowZeroHop() {
        return _allowZeroHop || _length + Math.min(_lengthVariance, 0) <= 0 || _lengthOverride == 0;
    }
    ...
    public void setAllowZeroHop(boolean ok) {
        if (!_isExploratory) _allowZeroHop = ok;
    }
        ...
        for (Map.Entry<Object, Object> e : props.entrySet()) {
            ...
            if (name.startsWith(prefix)) {
                ...
                //     _rebuildPeriod = getInt(value, DEFAULT_REBUILD_PERIOD);
                    ...
                    if (rk != null && rk.length == SessionKey.KEYSIZE_BYTES) _randomKey = new SessionKey(rk);
