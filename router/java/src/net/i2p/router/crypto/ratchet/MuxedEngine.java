...
final class MuxedEngine {
    ...
    public CloveSet decrypt(byte data[], PrivateKey elgKey, PrivateKey ecKey, MuxedSKM keyManager) throws DataFormatException {
        if (elgKey.getType() != EncType.ELGAMAL_2048 || ecKey.getType() != EncType.ECIES_X25519) throw new IllegalArgumentException();
        ...
        if (preferRatchet) {
            ...
            if (debug) _log.debug("Ratchet tag not found before AES");
        }
        // AES Tag
        if (data.length >= 128 && (data.length & 0x0f) == 0) {
            ...
            if (dec != null) {
                try {
                    ...
                    if (rv == null && _log.shouldInfo()) _log.info("AES cloveset error after AES? " + preferRatchet);
                } catch (DataFormatException dfe) {
                    if (_log.shouldInfo()) _log.info("AES cloveset error after AES? " + preferRatchet, dfe);
                }
                return rv;
            } else {
                if (debug) _log.debug("AES tag not found after Ratchet? " + preferRatchet);
            }
        }
        if (!preferRatchet) {
            // Ratchet Tag
            rv = _context.eciesEngine().decryptFast(data, ecKey, keyManager.getECSKM());
            if (rv != null) return rv;
            if (debug) _log.debug("Ratchet tag not found after AES");
        }
        ...
        if (preferRatchet) {
            ...
            if (ok) return rv;
            if (debug) _log.debug("Ratchet NextSession decrypt failed before ElGamal");
        }
        // ElG DH
        if (data.length >= 514 && (data.length & 0x0f) == 2) {
            ...
            if (dec != null) {
                try {
                    ...
                    if (ok) return rv;
                    if (_log.shouldInfo()) _log.info("ElGamal cloveset error after Ratchet? " + preferRatchet);
                } catch (DataFormatException dfe) {
                    if (_log.shouldInfo()) _log.info("ElGamal cloveset error after Ratchet? " + preferRatchet, dfe);
                }
            } else {
                if (_log.shouldInfo()) _log.info("ElGamal decrypt failed after Ratchet? " + preferRatchet);
            }
        ...
        if (!preferRatchet) {
            ...
            boolean ok = rv != null;
            keyManager.reportDecryptResult(true, ok);
            if (!ok && debug) _log.debug("Ratchet NextSession decrypt failed after ElGamal");
        }
        return rv;
    }
}
