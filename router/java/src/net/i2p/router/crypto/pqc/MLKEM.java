    ...
    /*
        private static void main2(String args[]) {
            ...
            for (int j = 0; j < 2; j++) {
                for (EncType type : toTest) {
                    if (type.getBaseAlgorithm() != EncAlgo.ECIES_MLKEM) continue;
                    if (!type.isAvailable()) {
        ...
        private static void testEnc(EncType type, int runs) throws GeneralSecurityException {
            ...
            for (int i = 0; i < runs; i++) {
                ...
                stime += mid - start;
                vtime += end - mid;
                if (!ok) throw new GeneralSecurityException(type + " shared key fail on run " + i);
            }
            stime /= 1000*1000;
            vtime /= 1000*1000;
            // we do two of each per run above
            int runs2 = runs * 2;
            System.out.println(type + " encap/decap " + runs + " times: " + (vtime+stime) + " ms = " + (((double) stime) / runs2) + " each encap, " + (((double) vtime) / runs2) + " each decap, " +
