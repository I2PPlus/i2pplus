...
public final class ElGamalAESEngine {
    ...
    public ElGamalAESEngine(I2PAppContext ctx) {
        ...
        _context.statManager().createFrequencyStat("crypto.elGamalAES.encryptNewSession", "How often we encrypt to a new ElGamal/AES+SessionTag session", "Encryption", new long[] {RateConstants.ONE_MINUTE});
        _context.statManager().createFrequencyStat("crypto.elGamalAES.encryptExistingSession", "How often we encrypt to an existing ElGamal/AES+SessionTag session", "Encryption", new long[] {RateConstants.ONE_MINUTE});
        _context.statManager().createFrequencyStat("crypto.elGamalAES.decryptNewSession", "How often we decrypt with a new ElGamal/AES+SessionTag session", "Encryption", new long[] {RateConstants.ONE_MINUTE});
        _context.statManager().createFrequencyStat("crypto.elGamalAES.decryptExistingSession", "How often we decrypt with an existing ElGamal/AES+SessionTag session", "Encryption", new long[] {RateConstants.ONE_MINUTE});
        _context.statManager().createFrequencyStat("crypto.elGamalAES.decryptFailed", "How often we fail to decrypt with ElGamal/AES+SessionTag", "Encryption", new long[] {RateConstants.ONE_MINUTE});
    }
    ...
    public byte[] decrypt(byte data[], PrivateKey targetPrivateKey, SessionKeyManager keyManager) throws DataFormatException {
        ...
        if (targetPrivateKey.getType() != EncType.ELGAMAL_2048) return null;
        ...
        if (key != null) {
            ...
            if (shouldDebug) _log.debug("Decrypting existing session " + "(" + data.length + " bytes)" + "\n* Tag: " + st.toString() + "\n* Key: " + key.toBase64());
            ...
            if (decrypted != null) {
                ...
                if (!foundTags.isEmpty()) {
                    for (SessionTag t : foundTags) {
                        buf.append("[").append(t.toString().substring(0, 6)).append("]");
                        buf.append(" ");
                    }
                    if (!foundTags.isEmpty() && shouldDebug) _log.debug("ElGamal/AES decrypt success with [" + st.toString().substring(0, 6) + "]\n* Found " + foundTags.size() + " Tags: " + buf.toString());
                    wasExisting = true;
                }
            } else {
            ...
            if (decrypted != null) {
                ...
                if (!foundTags.isEmpty()) {
                    ...
                    if (!foundTags.isEmpty() && shouldDebug) _log.debug("ElGamal decrypt success!\n* Found " + foundTags.size() + " Tags: " + buf.toString());
                }
            } else {
                _context.statManager().updateFrequency("crypto.elGamalAES.decryptFailed");
                if (_log.shouldWarn()) _log.warn("ElGamal decrypt fail: Unknown Tag " + st.toString());
            }
        } else {
            return null;
        }
        ...
        if (!foundTags.isEmpty()) {
            if (foundKey.getData() != null) {
                ...
                if (shouldDebug) _log.debug("Found key: " + foundKey.toBase64().substring(0, 6) + " wasExisting? " + wasExisting + "\n* Tags: " + buf.toString());
                ...
                if (shouldDebug) _log.debug("Used key [" + usedKey.toBase64().substring(0, 6) + "] wasExisting? " + wasExisting + "\n* Tags: " + buf.toString());
    ...
    public byte[] decryptFast(byte data[], PrivateKey targetPrivateKey, SessionKeyManager keyManager) throws DataFormatException {
        ...
        if (data.length < MIN_ENCRYPTED_SIZE) return null;
        byte tag[] = new byte[32];
        System.arraycopy(data, 0, tag, 0, 32);
        SessionTag st = new SessionTag(tag);
        SessionKey key = keyManager.consumeTag(st);
        ...
        Set<SessionTag> foundTags = new HashSet<SessionTag>();
        final boolean shouldDebug = _log.shouldDebug();
        if (shouldDebug) _log.debug("Decrypting existing session \n* Tag: " + st.toString() + "\n* Key: " + key.toBase64() + " (" + data.length + " bytes)");
        byte[] decrypted = decryptExistingSession(data, key, targetPrivateKey, foundTags, usedKey, foundKey);
        if (decrypted != null) {
            _context.statManager().updateFrequency("crypto.elGamalAES.decryptExistingSession");
            if (!foundTags.isEmpty() && shouldDebug) _log.debug("ElG/AES decrypt success with " + st + ": " + foundTags.size() + " found tags: " + foundTags);
            if (!foundTags.isEmpty()) {
                if (foundKey.getData() != null) {
                    if (shouldDebug) _log.debug("Found key: " + foundKey.toBase64() + " in existing session");
                    keyManager.tagsReceived(foundKey, foundTags);
                } else if (usedKey.getData() != null) {
                    if (shouldDebug) _log.debug("Used key: " + usedKey.toBase64() + " in existing session");
                    keyManager.tagsReceived(usedKey, foundTags);
                }
            }
        } else {
    ...
    public byte[] decryptSlow(byte data[], PrivateKey targetPrivateKey, SessionKeyManager keyManager) throws DataFormatException {
        if (data == null) return null;
        if (data.length < ELG_ENCRYPTED_LENGTH) return null;
        SessionKey foundKey = new SessionKey();
        SessionKey usedKey = new SessionKey();
        Set<SessionTag> foundTags = new HashSet<SessionTag>();
        ...
        if (decrypted != null) {
            ...
            _context.statManager().updateFrequency("crypto.elGamalAES.decryptFailed");
            if (_log.shouldWarn()) _log.warn("ElG decrypt failure as new session");
        }
        if (!foundTags.isEmpty()) {
            if (shouldDebug) _log.debug("ElG decrypt success: " + foundTags.size() + " found tags");
            if (foundKey.getData() != null) {
                if (shouldDebug) _log.debug("Found key: " + foundKey.toBase64() + " in new session");
                keyManager.tagsReceived(foundKey, foundTags);
            } else if (usedKey.getData() != null) {
                if (shouldDebug) _log.debug("Used key: " + usedKey.toBase64() + " in new session");
                keyManager.tagsReceived(usedKey, foundTags);
            }
        }
        return decrypted;
    ...
    private byte[] decryptNewSession(byte data[], PrivateKey targetPrivateKey, Set<SessionTag> foundTags, SessionKey usedKey, SessionKey foundKey) throws DataFormatException {
        ...
        byte aesDecr[] = decryptAESBlock(data, ELG_ENCRYPTED_LENGTH, data.length - ELG_ENCRYPTED_LENGTH, usedKey, iv, null, foundTags, foundKey);
    ...
    private byte[] decryptExistingSession(byte data[], SessionKey key, PrivateKey targetPrivateKey, Set<SessionTag> foundTags, SessionKey usedKey, SessionKey foundKey) throws DataFormatException {
        ...
        if (decrypted == null) {
            ...
            if (_log.shouldWarn()) _log.warn("Decrypting looks negative (existing key fails with existing tag) - let's try as a new one...");
            ...
            if (_log.shouldWarn()) {
                ...
                else _log.warn("Decrypting succeeded as a new session, even though it used an existing tag!");
            }
            return rv;
        }
    ...
    public byte[] encrypt(byte data[], PublicKey target, SessionKey key, Set<SessionTag> tagsForDelivery, SessionTag currentTag, SessionKey newKey, long paddedSize) {
        ...
        if (currentTag == null) {
            if (_log.shouldDebug()) _log.debug("Current tag is null - encrypting as new session");
        ...
        if (_log.shouldDebug()) _log.debug("Encrypting existing session " + "(" + rv.length + " bytes)" + "\n* Tag: " + currentTag.toString() + "\n* Key: " + key.toBase64() /* + ": " + Base64.encode(rv, 0, 64) */);
        return rv;
    }
