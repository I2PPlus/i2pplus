...
public class DecayingHashSet extends DecayingBloomFilter {
    ...
    public DecayingHashSet(I2PAppContext context, int durationMs, int entryBytes, String name) {
        ...
        if (_log.shouldDebug()) _log.debug("New DHS " + name + " entryBytes = " + entryBytes + " cycle (s) = " + (durationMs / 1000));
        // try to get a handle on memory usage vs. false positives
        context.statManager().createRateStat("router.decayingHashSet." + name + ".size", "Size", "Router [DecayingHashSet]", new long[] {10 * Math.max(60 * 1000, durationMs)});
        context.statManager().createRateStat("router.decayingHashSet." + name + ".dups", "1000000 * Duplicates/Size", "Router [DecayingHashSet]", new long[] {10 * Math.max(60 * 1000, durationMs)});
    }
    ...
    protected void decay() {
        ...
        if (_log.shouldDebug()) _log.debug("Decaying the " + _name + " filter after inserting " + currentCount + " elements and " + dups + " false positives");
    ...
    private static class ArrayWrapper {
        ...
        public boolean equals(Object o) {
            if (o == null || !(o instanceof ArrayWrapper)) return false;
