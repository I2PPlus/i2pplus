...
class InboundMessageDistributor implements GarlicMessageReceiver.CloveReceiver {
    ...
    public InboundMessageDistributor(RouterContext ctx, Hash client) {
        ...
        if (_client != null) {
            ...
            _msgIDBloomXor = clienttps != null ? clienttps.getMsgIdBloomXor() : RandomSource.getInstance().nextLong(I2NPMessage.MAX_ID_VALUE);
            if (_log.shouldLog(Log.DEBUG)) {
                _log.debug("Initializing client " + (clienttps != null ? clienttps.getDestinationNickname() : "UNKNOWN") +
                    ...
                    if (dsm.getEntry().isLeaseSet()) {
                        ...
                        if (_log.shouldInfo())
                            ...
                            if (sz > 0)
                                ...
                                if (_log.shouldLog(Log.INFO))
                                    _log.info("Store the LeaseSet in the correct DbId subDb: " + _client.toBase32());
                                FloodfillDatabaseStoreMessageHandler _FDSMH = new FloodfillDatabaseStoreMessageHandler(_context, (FloodfillNetworkDatabaseFacade) _context.clientNetDb(_client));
