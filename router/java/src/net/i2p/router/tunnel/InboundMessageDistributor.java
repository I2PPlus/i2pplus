...
class InboundMessageDistributor implements GarlicMessageReceiver.CloveReceiver {
    ...
    public InboundMessageDistributor(RouterContext ctx, Hash client) {
        ...
        if (_client != null) {
            ...
            _msgIDBloomXor = clienttps != null
                    ? clienttps.getMsgIdBloomXor()
            ...
            if (_log.shouldLog(Log.DEBUG)) _log.debug("Initializing NULL or Exploratory InboundMessageDistributor...");
        }
    }
    ...
    public void distribute(I2NPMessage msg, Hash target, TunnelId tunnel) {
        if (_log.shouldDebug()) {
            _log.debug("InboundMessageDistributor for " + _clientNickname + " ["
                    + ((_client != null) ? _client.toBase32().substring(0, 8) : "NULL") + "] to " + target + " / "
                    + tunnel + " : " + msg);
        }
            ...
            switch (type) {
                ...
                case DatabaseStoreMessage.MESSAGE_TYPE:
                    ...
                    if (dsm.getEntry().getType() == DatabaseEntry.KEY_TYPE_ROUTERINFO) {
                        ...
                        // Todo: if peer was ff and RI is not ff, queue for exploration in netdb (but that isn't part of
                        // the facade now)
                        if (_log.shouldWarn()) {
                        ...
                        if (oldri != null
                                && oldri.getPublished() < ri.getPublished()
                                && !FloodfillNetworkDatabaseFacade.isFloodfill(ri)) {
                            if (_log.shouldInfo()) {
                                _log.info("Updating capabilities for ["
                                        + key.toBase64().substring(0, 6) + "] from \"" + oldri.getCapabilities()
                                        + "\" to \"" + ri.getCapabilities() + '"');
                            }
                        ...
                        _log.error(
                                "Dropping DANGEROUS LeaseSet DbStoreMessage with reply token sent down a tunnel for ["
                ...
                default:
                    ...
                    _context.statManager().addRateData("tunnel.dropDangerousClientTunnelMessage", 1, type);
                    _log.error(
                            "Dropping DANGEROUS message [" + msg + "] sent down a tunnel for client ["
                                    + _client.toString().substring(0, 8) + "]",
                            new Exception("cause"));
                    return;
            } // switch
            ...
            switch (type) {
                case DatabaseStoreMessage.MESSAGE_TYPE:
                    ...
                    if (dsm.getReplyToken() != 0) {
                        ...
                        _log.error("Dropping DANGEROUS DbStoreMessage [" + msg
                                + "] with reply token sent down Exploratory tunnel");
                        return;
                    }
                    if (dsm.getEntry().isLeaseSet()) ((LeaseSet) dsm.getEntry()).setReceivedBy(_client);
                    break;
                ...
                default:
                    _context.statManager().addRateData("tunnel.dropDangerousExplTunnelMessage", 1, type);
                    _log.error(
                            "Dropping DANGEROUS message [" + msg + "] sent down Exploratory tunnel",
                            new Exception("cause"));
                    return;
            } // switch
        } // client != null
        ...
        if ((target == null) || ((tunnel == null) && (_context.routerHash().equals(target)))) {
            ...
            if (_log.shouldLog(Log.DEBUG))
                _log.debug(
                        "Distributing Inbound tunnel msg type " + type + " back out " + out + " targeting " + target);
    ...
    public void handleClove(DeliveryInstructions instructions, I2NPMessage data) {
        ...
        switch (instructions.getDeliveryMode()) {
            case DeliveryInstructions.DELIVERY_MODE_LOCAL:
                if (_log.shouldDebug())
                    _log.debug("Local delivery instructions for clove: "
                ...
                switch (type) {
                    ...
                    case DatabaseStoreMessage.MESSAGE_TYPE:
                        ...
                        if (dsm.getEntry().isLeaseSet()) {
                            ...
                            if (_client != null) {
                                ...
                                _log.error(
                                        "Dropping DANGEROUS message (" + dsm + ") sent down a tunnel for "
                                                + _clientNickname + " [" + truncDest + "]",
                                        new Exception("cause"));
                                return;
                            }
                            // Case 3:
                            ...
                            // We must send to the InNetMessagePool so the message can be matched
                            // and the search marked as successful.
                            // note that encrypted replies to RI lookups is currently disabled in ISJ, we won't get
                            // here.
                            // ... and inject it.
                            if (_log.shouldInfo()) {
                    ...
                    default:
                        ...
                        if (_client != null) {
                            _context.statManager()
                                    .addRateData("tunnel.dropDangerousClientTunnelMessage", 1, data.getType());
                            _log.error(
                                    "Dropping DANGEROUS message (" + data + ") down a tunnel for client ["
                                            + _client.toString().substring(0, 8) + "]",
                                    new Exception("cause"));
                        } else {
                            _log.error(
                                    "Dropped dangerous message received down an expl. tunnel " + data,
                                    new Exception("cause"));
                        }
                        break;
                } // switch (type)
            ...
            case DeliveryInstructions.DELIVERY_MODE_DESTINATION:
                ...
                if (type != DataMessage.MESSAGE_TYPE) {
                    ...
                    if (_log.shouldDebug()) {
                        _log.debug("Received data message for client ["
                                + _client.toString().substring(0, 8) + "]");
                    }
                    ...
                    if (tgt != null && _client.equals(tgt.getAliasOf())) {
                        ...
                        if (_log.shouldDebug()) {
                            _log.debug("Received data message for client ["
                                    + _client.toString().substring(0, 8) + "] targeting Shared Client destination ["
                                    + to.toString().substring(0, 8) + "]");
                        }
                        DataMessage dm = (DataMessage) data;
                        Payload payload = new Payload();
                        payload.setEncryptedData(dm.getData());
                        ...
                        if (_log.shouldError()) {
                            _log.error("Received data message for client ["
                                    + _client.toString().substring(0, 8) + "] -> Wrong target: ["
                                    + to.toString().substring(0, 8) + "]");
                        }
                    }
                } else {
                    if (_log.shouldError()) {
                        _log.error("Received data message via Exploratory tunnel targeting destination ["
                                + to.toString().substring(0, 8) + "]");
                    }
                }
                return;
            ...
            case DeliveryInstructions.DELIVERY_MODE_TUNNEL:
                ...
                if (_log.shouldInfo())
                    _log.info("Clove targeted ["
                            + instructions.getRouter().toBase64().substring(0, 6) + "] via " + "[TunnelId"
                            + instructions.getTunnelId() + "] -> Handling recursively to prevent leakage... \n* "
                            + "Message type: " + data.getClass().getSimpleName()
                            + ((_client != null)
