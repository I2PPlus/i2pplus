...
public class ConnectChecker {
    ...
    public boolean canConnect(Hash from, Hash to) {
        ...
        if (ust && ctx.commSystem().isEstablished(from))
            return true;
        RouterInfo rt = (RouterInfo) ctx.floodfillNetDb().lookupLocallyWithoutValidation(to);
        if (rt == null)
            return true;
        RouterInfo rf = (RouterInfo) ctx.floodfillNetDb().lookupLocallyWithoutValidation(from);
        if (rf == null)
            return true;
        int ct;
        if (ust) {
