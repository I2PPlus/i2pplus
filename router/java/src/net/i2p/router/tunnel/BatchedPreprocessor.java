            ...
            if (allocated > 0) {
                ...
                if ( (pending.size() > FORCE_BATCH_FLUSH) ||                    // enough msgs - or
                    ...
                    if (!pending.isEmpty()) {
                        ...
                        if (_log.shouldLog(Log.INFO))
                            display(allocated, pending, "\n* Flushed messages in buffer, some remain");
                        ...
                        if (_log.shouldLog(Log.INFO))
                            display(allocated, pending, "Flushed " + (beforeSize) + " messages, none remaining after " + delayAmount + "ms");

                        if (timingBuf != null) {
                            long now = System.currentTimeMillis();
                            timingBuf.append(" Flushed all messages, none remain (displayed to now: " + (now - afterDisplayed) + ")");
                            timingBuf.append(" Total time: " + (now - start));
                            _log.debug(timingBuf.toString());
                        }
                        return false;
                    ...
                    if (_log.shouldLog(Log.INFO))
                        display(allocated, pending, "Not flushing pending messages");
