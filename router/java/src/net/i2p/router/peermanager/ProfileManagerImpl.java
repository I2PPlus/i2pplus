...
public class ProfileManagerImpl implements ProfileManager {
    ...
    public void tunnelTimedOut(Hash peer) {
        PeerProfile data = getProfile(peer);
        if (data == null) return;
        // Timeouts are tracked separately from rejections — a peer that
        // is slow to respond is not the same as one that actively refuses.
        data.getTunnelHistory().incrementTimedOut();
    }
