...
class PeerManager {
    ...
    private boolean storeProfile(Hash peer, long cutoff) {
        ...
        // Persist profiles that have tunnel history or non-trivial DB history.
        // Floodfills we've queried have DB history (lookup/store success/failure)
        // but may never have accepted or rejected one of our tunnel builds.
        // Without persisting them, firstHeardAbout resets to "now" on every
        // restart and classifyFloodfillPeer() marks them BAD for 45 minutes.
        if (!prof.hasTunnelHistory() && !prof.hasDBActivity()) {return false;}
