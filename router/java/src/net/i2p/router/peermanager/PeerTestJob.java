...
public class PeerTestJob extends JobImpl {
     ...
     * to avoid false timeouts during normal operation.
     * Under attack (low build success), doubles timeout to account for network delays.</p>
     *
    ...
    private int getTestTimeout() {
        ...
        // Double timeout under attack to account for network delays @since 0.9.68+
        if (_aggressiveMode) {
            testTimeout = Math.min(testTimeout * 2, 1500);
        }
