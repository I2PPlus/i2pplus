...
public class PeerTestJob extends JobImpl {
    ...
    public void runJob() {
        ...
        long avgLag = getContext().jobQueue().getAvgLag();
        boolean keepTesting;
        PeerManager manager;
        synchronized(this) {
        ...
        if ((lag > 1000 && avgLag > 10) || SystemVersion.getCPULoadAvg() > 90) {
            requeue(getPeerTestDelay() * 2);
            if (_log.shouldWarn())
            if (lag > 1000 && avgLag > 10) {
                _log.info("High Job lag (Avg: " + avgLag + " / Max: " + lag + "ms) -> Increasing delay before next run to " + getPeerTestDelay() * 2 + "ms");
            } else {
