...
class CapacityCalculator {
    ...
    public static double calc(PeerProfile profile) {
        ...
        if (enableAgeChecks) {
            ...
            if (ago < 2 * 60 * 60 * 1000) capacity -= PENALTY_NEW * (2 * 60 * 60 * 1000 - ago) / 2 * 60 * 60 * 1000;
        }
        ...
        */
        ...
        if (ndb != null) {
            ...
            if (ri != null) {
                ...
                if (caps.indexOf(Router.CAPABILITY_REACHABLE) < 0) capacity -= PENALTY_NO_R_CAP;
                if (caps.indexOf(Router.CAPABILITY_UNREACHABLE) >= 0) capacity -= PENALTY_U_CAP;
                if (caps.indexOf(Router.CAPABILITY_BW32) >= 0) capacity -= PENALTY_L_CAP;
                if (caps.indexOf(Router.CAPABILITY_CONGESTION_MODERATE) >= 0) {
                    ...
                    if (ecapPenalty != null) {
                        ...
                        if (age < 30 * 60 * 1000) capacity -= PENALTY_CAP_E;
        ...
        if (lastBad > lastGood) {
            ...
            if (lastGood > now - 30 * 60 * 1000) capacity += PENALTY_RECENT_SEND_FAIL;
        } else if (lastGood > 0) {
            capacity += BONUS_LAST_SEND_SUCCESS;
            if (lastGood > now - 30 * 60 * 1000) capacity += BONUS_RECENT_SEND_SUCCESS;
        }
