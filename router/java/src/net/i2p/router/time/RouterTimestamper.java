...
public class RouterTimestamper extends Timestamper {
    ...
    public RouterTimestamper(I2PAppContext ctx, UpdateListener lsnr, boolean daemon) {
        ...
        _disabled = ctx.getProperty(PROP_DISABLED, DEFAULT_DISABLED) || ctx.getBooleanProperty("i2p.vmCommSystem");
        if (_disabled) {
            _initialized = true;
            _zones = null;
    ...
    public void waitForInitialization() {
        try {
            synchronized (this) {
                if (!_initialized) wait(MAX_WAIT_INITIALIZATION);
            }
    ...
    public void timestampNow() {
        if (_initialized && _isRunning && (!_disabled) && _timestamperThread != null) _timestamperThread.interrupt();
    }
    ...
    private class Shutdown implements Runnable {
        ...
        public void run() {
            _isRunning = false;
            if (_timestamperThread != null) _timestamperThread.interrupt();
        }
    }
