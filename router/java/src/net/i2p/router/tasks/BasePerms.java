...
public class BasePerms {
    ...
    public static void fix(RouterContext ctx) {
        if (!SystemVersion.isWindows()) return;
        if (ctx.getBooleanProperty(PROP_FIXED)) return;
        if (!ctx.router().getKillVMOnEnd()) return; // embedded
        File dir = ctx.getBaseDir();
        File f = new File(dir, "history.txt");
        // no permissions, nothing we can do
