...
class RouterDoSThrottle extends RouterThrottleImpl {
    public RouterDoSThrottle(RouterContext context) {
        super(context);
        context.statManager().createRateStat("router.throttleNetDbDoS", "NetDb lookup messages received during detected DoS", "Router [Throttle]", RateConstants.TUNNEL_RATES);
        ...
        if (_currentLookupPeriod + LOOKUP_THROTTLE_PERIOD > now) {
            ...
            if (cnt >= LOOKUP_THROTTLE_MAX) {
                ...
                if (cnt >= LOOKUP_THROTTLE_MAX * 2) {
                    ...
                    if (bl != null) {
                        bl.logBan(key, _context, "Excessive NetDb lookups", 5 * 60 * 1000L);
                    }
                    if (_log.shouldWarn()) {
                        _log.warn("Banning [" + key.toBase64().substring(0, 6) + "] for 5m for excessive NetDB lookups " + "(Limit (over 20s period): " + LOOKUP_THROTTLE_MAX + " -> Requests: " + cnt + ")");
                    }
                }
