...
class HandleFloodfillDatabaseStoreMessageJob extends JobImpl {
    ...
    public void runJob() {
        ...
        if (DatabaseEntry.isLeaseSet(type)) {
            ...
            // Extract first routable IP:port for ban logging
            String _riIP = "";
                ...
                if (ri.getReceivedAsPublished()) {
                    ...
                    if (isBanned) {
                        ...
                        if (_log.shouldWarn()) {
                            ...
                            _banLogger.logBan(key, _riIP, "Invalid Router version (" + v + ")", 24L * 60 * 60 * 1000, ri);
                        }
                        ...
                        if (!ri.getIdentity().getPublicKey().equals(prevNetDb.getIdentity().getPublicKey())) {
                            ...
                            _banLogger.logBan(key, _riIP, "Inconsistent public keys", 8L * 60 * 60 * 1000, ri);
                            shouldStore = false;
                            ...
                            _banLogger.logBan(key, _riIP, "Inconsistent signing keys", 8L * 60 * 60 * 1000, ri);
                            shouldStore = false;
                        }
                    }
                }
        ...
        if (invalidMessage == null && _facade.floodfillEnabled() && _message.getReplyToken() > 0) {
            if (wasNew) {
                ...
                new I2PThread(floodTask, "FloodWorker", true).start();
            } else {
    ...
    private void sendAck(Hash storedKey) {
        ...
        if (_facade.floodfillEnabled() ||
            ...
            msg2.setEntry(me);
            if (_fromHash != null && _from != null && _log.shouldWarn()) {
