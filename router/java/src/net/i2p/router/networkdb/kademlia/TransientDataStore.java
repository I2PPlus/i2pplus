...
class TransientDataStore implements DataStore {
    ...
    public boolean put(Hash key, DatabaseEntry data) {
        ...
        if (_log.shouldDebug()) {
            _log.debug((isRI ? "Sending RouterInfo" : "Saving LeaseSet") + " [" + (isRI ? key.toBase64().substring(0, 6) : key.toBase32().substring(0, 8)) + "] to persistent datastore" + (isRI ? " for processing" : "") + "...");
        }
        ...
        if (isRI) {
            ...
            if (old != null) {
                ...
                if (ri.getPublished() > ori.getPublished()) {
                    if (_log.shouldInfo()) _log.info("Received updated RouterInfo [" + key.toBase64().substring(0, 6) + "] -> " + v + " / " + caps + "\n* Old: " + Instant.ofEpochMilli(ori.getPublished()) + "\n* New: " + Instant.ofEpochMilli(ri.getPublished()));
                    _data.put(key, data);
                    rv = true;
                }
            } else {
                if (_log.shouldInfo()) _log.info("Received new RouterInfo [" + key.toBase64().substring(0, 6) + "] -> " + v + " / " + caps + "\n* Published: " + Instant.ofEpochMilli(ri.getPublished()));
                rv = true;
            ...
            if (old != null) {
                ...
                if (type != DatabaseEntry.KEY_TYPE_LEASESET && ols.getType() != DatabaseEntry.KEY_TYPE_LEASESET) {
                    if (ls instanceof LeaseSet2 && ols instanceof LeaseSet2) {
                        ...
                        throw new IllegalArgumentException("Expected LeaseSet2 but got a different type");
                    }
                } else {
