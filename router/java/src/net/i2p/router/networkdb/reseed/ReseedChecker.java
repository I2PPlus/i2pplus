...
public class ReseedChecker {
    ...
    public boolean checkReseed(int known) {
        ...
        if (uptime < tenMinutes && known > 1 && !isHidden) {
            return false;
        }
    ...
    private void logOnceWarning(int peers, String reason) {
        String s = (peers > 0) ? "Only " + peers + " peers remaining but reseed " + reason : "No peers remaining but reseed " + reason;
    ...
    private boolean _hasNetworkConnection() {
        ...
        File[] noReseedFiles = {new File(userHome, ".i2pnoreseed"), new File(userHome, "noreseed.i2p"), new File(configDir, ".i2pnoreseed"), new File(configDir, "noreseed.i2p")};
    ...
    public boolean requestReseed() {
        if (_inProgress.compareAndSet(false, true)) {
            ...
            if (_log.shouldWarn()) _log.warn("Reseed already in progress...");
            return false;
        }
    }
    ...
    private class StatusCleaner implements SimpleTimer.TimedEvent {
        ...
        public void timeReached() {
            if (_status.equals(getStatus())) setStatus("");
            if (_error.equals(getError())) setError("");
        }
    }
}
