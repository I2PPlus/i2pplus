...
public class DatabaseStoreMessage extends FastI2NPMessageImpl {
    ...
    public void readMessage(byte data[], int offset, int dataSize, int type) throws I2NPMessageException {
        ...
        if (DatabaseEntry.isLeaseSet(dbType)) {
            ...
            if (compressedSize <= 0 || curIndex + compressedSize > data.length || curIndex + compressedSize > dataSize + offset) {
                throw new I2NPMessageException("Compressed RouterInfo length: " + compressedSize + " but remaining bytes: " + Math.min(data.length - curIndex, dataSize + offset - curIndex));
            }
            try {
                ...
                throw new I2NPMessageException("Could not read RouterInfo -> Decompression failed", dfe);
            } catch (IOException ioe) {
    ...
    protected int calculateWrittenLength() {
        ...
        if (_replyToken > 0) {
            len += 4 + Hash.HASH_LENGTH;
        } // replyTunnel+replyGateway
    ...
    @Override
    public int hashCode() {
        return DataHelper.hashCode(getKey()) + DataHelper.hashCode(_dbEntry) + (int) _replyToken + DataHelper.hashCode(_replyTunnel) + DataHelper.hashCode(_replyGateway);
    }
    ...
    public String toString() {
        ...
        buf.append("\n* DatabaseStore Message").append("\n* Expiration: ").append(Instant.ofEpochMilli(_expiration)).append("\n* Unique ID: ").append(getUniqueId());
        if (_log.shouldInfo()) {
            if (_replyToken != 0) {
                buf.append("\n* Reply token: ").append(_replyToken).append("\n* Reply tunnel: ").append(_replyTunnel).append("\n* Reply gateway: ").append(_replyGateway);
            }
