...
class LocalClientManager extends ClientManager {
    ...
    private static class DelayedSend extends SimpleTimer2.TimedEvent {
        ...
        public DelayedSend(RouterContext ctx, ClientConnectionRunner sender, ClientConnectionRunner runner, Destination fromDest, Destination toDest, Payload payload, MessageId msgId, long messageNonce) {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            while ((c = g.getopt()) != -1) {
                switch (c) {
                    case 'd': dropX1000 = (int) (1000 * Double.parseDouble(g.getOptarg()));
                        if (dropX1000 < 0 || dropX1000 >= 100 * 1000) error = true;
                        break;

                    case 'j': jitter = Integer.parseInt(g.getOptarg());
                        if (jitter < 0) error = true;
                        break;

                    case 'l': latency = Integer.parseInt(g.getOptarg());
                        if (latency < 0) error = true;
                        break;

                    case 'p': port = Integer.parseInt(g.getOptarg());
                        if (port < 1024 || port > 65535) error = true;
                        break;
