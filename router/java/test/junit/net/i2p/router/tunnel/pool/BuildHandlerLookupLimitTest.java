...
public class BuildHandlerLookupLimitTest {
    ...
    public void startupBoostUsesFullCeiling() {
        ...
        assertEquals(MAX, BuildHandler.lookupLimit(5, MIN, MAX, PCT, THIRTY_MIN - 1));
    }

    @Test
    public void boostExpiresAfterWindow() {
        long justAfter = THIRTY_MIN;
    ...
    public void proportionalWithinCeiling() {
        assertEquals(26, BuildHandler.lookupLimit(65, MIN, MAX, PCT, THIRTY_MIN * 2));
    }

    @Test
    public void ceilingCapsLargePools() {
        assertEquals(MAX, BuildHandler.lookupLimit(500, MIN, MAX, PCT, THIRTY_MIN * 2));
    }

    @Test
    public void floorKeepsDrainedPoolLookingUp() {
        // a drained pool must not zero out next-hop resolution
        assertEquals(MIN, BuildHandler.lookupLimit(0, MIN, MAX, PCT, THIRTY_MIN * 2));
    }

    @Test
    public void slowBoxParametersRespected() {
        // IS_SLOW profile: min 4, max 10, pct 15
        assertEquals(10, BuildHandler.lookupLimit(500, 4, 10, 15, THIRTY_MIN * 2));
        assertEquals(4, BuildHandler.lookupLimit(0, 4, 10, 15, THIRTY_MIN * 2));
        assertEquals(10, BuildHandler.lookupLimit(3, 4, 10, 15, THIRTY_MIN / 2));
    }
}
